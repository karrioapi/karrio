#!/usr/bin/env python
# -*- coding: utf-8 -*-

#
# Generated Thu Dec 13 14:06:25 2018 by generateDS.py version 2.29.5.
# Python 3.6.5 (default, May 19 2018, 11:27:13)  [GCC 4.2.1 Compatible Apple LLVM 9.0.0 (clang-900.0.39.2)]
#
# Command line options:
#   ('-o', '../python/ShippingService.xsd.py')
#
# Command line arguments:
#   ShippingService.xsd
#
# Command line:
#   /Users/danielkobina/Documents/Open/bin/generateDS -o "../python/ShippingService.xsd.py" ShippingService.xsd
#
# Current working directory (os.getcwd()):
#   schemas
#

import sys
import re as re_
import base64
import datetime as datetime_
import warnings as warnings_
try:
    from lxml import etree as etree_
except ImportError:
    from xml.etree import ElementTree as etree_


Validate_simpletypes_ = True
if sys.version_info.major == 2:
    BaseStrType_ = basestring
else:
    BaseStrType_ = str


def parsexml_(infile, parser=None, **kwargs):
    if parser is None:
        # Use the lxml ElementTree compatible parser so that, e.g.,
        #   we ignore comments.
        try:
            parser = etree_.ETCompatXMLParser()
        except AttributeError:
            # fallback to xml.etree
            parser = etree_.XMLParser()
    doc = etree_.parse(infile, parser=parser, **kwargs)
    return doc

def parsexmlstring_(instring, parser=None, **kwargs):
    if parser is None:
        # Use the lxml ElementTree compatible parser so that, e.g.,
        #   we ignore comments.
        try:
            parser = etree_.ETCompatXMLParser()
        except AttributeError:
            # fallback to xml.etree
            parser = etree_.XMLParser()
    element = etree_.fromstring(instring, parser=parser, **kwargs)
    return element

#
# Namespace prefix definition table (and other attributes, too)
#
# The module generatedsnamespaces, if it is importable, must contain
# a dictionary named GeneratedsNamespaceDefs.  This Python dictionary
# should map element type names (strings) to XML schema namespace prefix
# definitions.  The export method for any class for which there is
# a namespace prefix definition, will export that definition in the
# XML representation of that element.  See the export method of
# any generated element type class for a example of the use of this
# table.
# A sample table is:
#
#     # File: generatedsnamespaces.py
#
#     GenerateDSNamespaceDefs = {
#         "ElementtypeA": "http://www.xxx.com/namespaceA",
#         "ElementtypeB": "http://www.xxx.com/namespaceB",
#     }
#

try:
    from generatedsnamespaces import GenerateDSNamespaceDefs as GenerateDSNamespaceDefs_
except ImportError:
    GenerateDSNamespaceDefs_ = {}

#
# The root super-class for element type classes
#
# Calls to the methods in these classes are generated by generateDS.py.
# You can replace these methods by re-implementing the following class
#   in a module named generatedssuper.py.

try:
    from generatedssuper import GeneratedsSuper
except ImportError as exp:
    
    class GeneratedsSuper(object):
        tzoff_pattern = re_.compile(r'(\+|-)((0\d|1[0-3]):[0-5]\d|14:00)$')
        class _FixedOffsetTZ(datetime_.tzinfo):
            def __init__(self, offset, name):
                self.__offset = datetime_.timedelta(minutes=offset)
                self.__name = name
            def utcoffset(self, dt):
                return self.__offset
            def tzname(self, dt):
                return self.__name
            def dst(self, dt):
                return None
        def gds_format_string(self, input_data, input_name=''):
            return input_data
        def gds_validate_string(self, input_data, node=None, input_name=''):
            if not input_data:
                return ''
            else:
                return input_data
        def gds_format_base64(self, input_data, input_name=''):
            return base64.b64encode(input_data)
        def gds_validate_base64(self, input_data, node=None, input_name=''):
            return input_data
        def gds_format_integer(self, input_data, input_name=''):
            return '%d' % input_data
        def gds_validate_integer(self, input_data, node=None, input_name=''):
            return input_data
        def gds_format_integer_list(self, input_data, input_name=''):
            return '%s' % ' '.join(input_data)
        def gds_validate_integer_list(
                self, input_data, node=None, input_name=''):
            values = input_data.split()
            for value in values:
                try:
                    int(value)
                except (TypeError, ValueError):
                    raise_parse_error(node, 'Requires sequence of integers')
            return values
        def gds_format_float(self, input_data, input_name=''):
            return ('%.15f' % input_data).rstrip('0')
        def gds_validate_float(self, input_data, node=None, input_name=''):
            return input_data
        def gds_format_float_list(self, input_data, input_name=''):
            return '%s' % ' '.join(input_data)
        def gds_validate_float_list(
                self, input_data, node=None, input_name=''):
            values = input_data.split()
            for value in values:
                try:
                    float(value)
                except (TypeError, ValueError):
                    raise_parse_error(node, 'Requires sequence of floats')
            return values
        def gds_format_double(self, input_data, input_name=''):
            return '%e' % input_data
        def gds_validate_double(self, input_data, node=None, input_name=''):
            return input_data
        def gds_format_double_list(self, input_data, input_name=''):
            return '%s' % ' '.join(input_data)
        def gds_validate_double_list(
                self, input_data, node=None, input_name=''):
            values = input_data.split()
            for value in values:
                try:
                    float(value)
                except (TypeError, ValueError):
                    raise_parse_error(node, 'Requires sequence of doubles')
            return values
        def gds_format_boolean(self, input_data, input_name=''):
            return ('%s' % input_data).lower()
        def gds_validate_boolean(self, input_data, node=None, input_name=''):
            return input_data
        def gds_format_boolean_list(self, input_data, input_name=''):
            return '%s' % ' '.join(input_data)
        def gds_validate_boolean_list(
                self, input_data, node=None, input_name=''):
            values = input_data.split()
            for value in values:
                if value not in ('true', '1', 'false', '0', ):
                    raise_parse_error(
                        node,
                        'Requires sequence of booleans '
                        '("true", "1", "false", "0")')
            return values
        def gds_validate_datetime(self, input_data, node=None, input_name=''):
            return input_data
        def gds_format_datetime(self, input_data, input_name=''):
            if input_data.microsecond == 0:
                _svalue = '%04d-%02d-%02dT%02d:%02d:%02d' % (
                    input_data.year,
                    input_data.month,
                    input_data.day,
                    input_data.hour,
                    input_data.minute,
                    input_data.second,
                )
            else:
                _svalue = '%04d-%02d-%02dT%02d:%02d:%02d.%s' % (
                    input_data.year,
                    input_data.month,
                    input_data.day,
                    input_data.hour,
                    input_data.minute,
                    input_data.second,
                    ('%f' % (float(input_data.microsecond) / 1000000))[2:],
                )
            if input_data.tzinfo is not None:
                tzoff = input_data.tzinfo.utcoffset(input_data)
                if tzoff is not None:
                    total_seconds = tzoff.seconds + (86400 * tzoff.days)
                    if total_seconds == 0:
                        _svalue += 'Z'
                    else:
                        if total_seconds < 0:
                            _svalue += '-'
                            total_seconds *= -1
                        else:
                            _svalue += '+'
                        hours = total_seconds // 3600
                        minutes = (total_seconds - (hours * 3600)) // 60
                        _svalue += '{0:02d}:{1:02d}'.format(hours, minutes)
            return _svalue
        @classmethod
        def gds_parse_datetime(cls, input_data):
            tz = None
            if input_data[-1] == 'Z':
                tz = GeneratedsSuper._FixedOffsetTZ(0, 'UTC')
                input_data = input_data[:-1]
            else:
                results = GeneratedsSuper.tzoff_pattern.search(input_data)
                if results is not None:
                    tzoff_parts = results.group(2).split(':')
                    tzoff = int(tzoff_parts[0]) * 60 + int(tzoff_parts[1])
                    if results.group(1) == '-':
                        tzoff *= -1
                    tz = GeneratedsSuper._FixedOffsetTZ(
                        tzoff, results.group(0))
                    input_data = input_data[:-6]
            time_parts = input_data.split('.')
            if len(time_parts) > 1:
                micro_seconds = int(float('0.' + time_parts[1]) * 1000000)
                input_data = '%s.%s' % (time_parts[0], micro_seconds, )
                dt = datetime_.datetime.strptime(
                    input_data, '%Y-%m-%dT%H:%M:%S.%f')
            else:
                dt = datetime_.datetime.strptime(
                    input_data, '%Y-%m-%dT%H:%M:%S')
            dt = dt.replace(tzinfo=tz)
            return dt
        def gds_validate_date(self, input_data, node=None, input_name=''):
            return input_data
        def gds_format_date(self, input_data, input_name=''):
            _svalue = '%04d-%02d-%02d' % (
                input_data.year,
                input_data.month,
                input_data.day,
            )
            try:
                if input_data.tzinfo is not None:
                    tzoff = input_data.tzinfo.utcoffset(input_data)
                    if tzoff is not None:
                        total_seconds = tzoff.seconds + (86400 * tzoff.days)
                        if total_seconds == 0:
                            _svalue += 'Z'
                        else:
                            if total_seconds < 0:
                                _svalue += '-'
                                total_seconds *= -1
                            else:
                                _svalue += '+'
                            hours = total_seconds // 3600
                            minutes = (total_seconds - (hours * 3600)) // 60
                            _svalue += '{0:02d}:{1:02d}'.format(
                                hours, minutes)
            except AttributeError:
                pass
            return _svalue
        @classmethod
        def gds_parse_date(cls, input_data):
            tz = None
            if input_data[-1] == 'Z':
                tz = GeneratedsSuper._FixedOffsetTZ(0, 'UTC')
                input_data = input_data[:-1]
            else:
                results = GeneratedsSuper.tzoff_pattern.search(input_data)
                if results is not None:
                    tzoff_parts = results.group(2).split(':')
                    tzoff = int(tzoff_parts[0]) * 60 + int(tzoff_parts[1])
                    if results.group(1) == '-':
                        tzoff *= -1
                    tz = GeneratedsSuper._FixedOffsetTZ(
                        tzoff, results.group(0))
                    input_data = input_data[:-6]
            dt = datetime_.datetime.strptime(input_data, '%Y-%m-%d')
            dt = dt.replace(tzinfo=tz)
            return dt.date()
        def gds_validate_time(self, input_data, node=None, input_name=''):
            return input_data
        def gds_format_time(self, input_data, input_name=''):
            if input_data.microsecond == 0:
                _svalue = '%02d:%02d:%02d' % (
                    input_data.hour,
                    input_data.minute,
                    input_data.second,
                )
            else:
                _svalue = '%02d:%02d:%02d.%s' % (
                    input_data.hour,
                    input_data.minute,
                    input_data.second,
                    ('%f' % (float(input_data.microsecond) / 1000000))[2:],
                )
            if input_data.tzinfo is not None:
                tzoff = input_data.tzinfo.utcoffset(input_data)
                if tzoff is not None:
                    total_seconds = tzoff.seconds + (86400 * tzoff.days)
                    if total_seconds == 0:
                        _svalue += 'Z'
                    else:
                        if total_seconds < 0:
                            _svalue += '-'
                            total_seconds *= -1
                        else:
                            _svalue += '+'
                        hours = total_seconds // 3600
                        minutes = (total_seconds - (hours * 3600)) // 60
                        _svalue += '{0:02d}:{1:02d}'.format(hours, minutes)
            return _svalue
        def gds_validate_simple_patterns(self, patterns, target):
            # pat is a list of lists of strings/patterns.  We should:
            # - AND the outer elements
            # - OR the inner elements
            found1 = True
            for patterns1 in patterns:
                found2 = False
                for patterns2 in patterns1:
                    if re_.search(patterns2, target) is not None:
                        found2 = True
                        break
                if not found2:
                    found1 = False
                    break
            return found1
        @classmethod
        def gds_parse_time(cls, input_data):
            tz = None
            if input_data[-1] == 'Z':
                tz = GeneratedsSuper._FixedOffsetTZ(0, 'UTC')
                input_data = input_data[:-1]
            else:
                results = GeneratedsSuper.tzoff_pattern.search(input_data)
                if results is not None:
                    tzoff_parts = results.group(2).split(':')
                    tzoff = int(tzoff_parts[0]) * 60 + int(tzoff_parts[1])
                    if results.group(1) == '-':
                        tzoff *= -1
                    tz = GeneratedsSuper._FixedOffsetTZ(
                        tzoff, results.group(0))
                    input_data = input_data[:-6]
            if len(input_data.split('.')) > 1:
                dt = datetime_.datetime.strptime(input_data, '%H:%M:%S.%f')
            else:
                dt = datetime_.datetime.strptime(input_data, '%H:%M:%S')
            dt = dt.replace(tzinfo=tz)
            return dt.time()
        def gds_str_lower(self, instring):
            return instring.lower()
        def get_path_(self, node):
            path_list = []
            self.get_path_list_(node, path_list)
            path_list.reverse()
            path = '/'.join(path_list)
            return path
        Tag_strip_pattern_ = re_.compile(r'\{.*\}')
        def get_path_list_(self, node, path_list):
            if node is None:
                return
            tag = GeneratedsSuper.Tag_strip_pattern_.sub('', node.tag)
            if tag:
                path_list.append(tag)
            self.get_path_list_(node.getparent(), path_list)
        def get_class_obj_(self, node, default_class=None):
            class_obj1 = default_class
            if 'xsi' in node.nsmap:
                classname = node.get('{%s}type' % node.nsmap['xsi'])
                if classname is not None:
                    names = classname.split(':')
                    if len(names) == 2:
                        classname = names[1]
                    class_obj2 = globals().get(classname)
                    if class_obj2 is not None:
                        class_obj1 = class_obj2
            return class_obj1
        def gds_build_any(self, node, type_name=None):
            return None
        @classmethod
        def gds_reverse_node_mapping(cls, mapping):
            return dict(((v, k) for k, v in mapping.iteritems()))
        @staticmethod
        def gds_encode(instring):
            if sys.version_info.major == 2:
                return instring.encode(ExternalEncoding)
            else:
                return instring
        @staticmethod
        def convert_unicode(instring):
            if isinstance(instring, str):
                result = quote_xml(instring)
            elif sys.version_info.major == 2 and isinstance(instring, unicode):
                result = quote_xml(instring).encode('utf8')
            else:
                result = GeneratedsSuper.gds_encode(str(instring))
            return result
        def __eq__(self, other):
            if type(self) != type(other):
                return False
            return self.__dict__ == other.__dict__
        def __ne__(self, other):
            return not self.__eq__(other)
    
    def getSubclassFromModule_(module, class_):
        '''Get the subclass of a class from a specific module.'''
        name = class_.__name__ + 'Sub'
        if hasattr(module, name):
            return getattr(module, name)
        else:
            return None


#
# If you have installed IPython you can uncomment and use the following.
# IPython is available from http://ipython.scipy.org/.
#

## from IPython.Shell import IPShellEmbed
## args = ''
## ipshell = IPShellEmbed(args,
##     banner = 'Dropping into IPython',
##     exit_msg = 'Leaving Interpreter, back to program.')

# Then use the following line where and when you want to drop into the
# IPython shell:
#    ipshell('<some message> -- Entering ipshell.\nHit Ctrl-D to exit')

#
# Globals
#

ExternalEncoding = 'utf-8'
Tag_pattern_ = re_.compile(r'({.*})?(.*)')
String_cleanup_pat_ = re_.compile(r"[\n\r\s]+")
Namespace_extract_pat_ = re_.compile(r'{(.*)}(.*)')
CDATA_pattern_ = re_.compile(r"<!\[CDATA\[.*?\]\]>", re_.DOTALL)

# Change this to redirect the generated superclass module to use a
# specific subclass module.
CurrentSubclassModule_ = None

#
# Support/utility functions.
#


def showIndent(outfile, level, pretty_print=True):
    if pretty_print:
        for idx in range(level):
            outfile.write('    ')


def quote_xml(inStr):
    "Escape markup chars, but do not modify CDATA sections."
    if not inStr:
        return ''
    s1 = (isinstance(inStr, BaseStrType_) and inStr or '%s' % inStr)
    s2 = ''
    pos = 0
    matchobjects = CDATA_pattern_.finditer(s1)
    for mo in matchobjects:
        s3 = s1[pos:mo.start()]
        s2 += quote_xml_aux(s3)
        s2 += s1[mo.start():mo.end()]
        pos = mo.end()
    s3 = s1[pos:]
    s2 += quote_xml_aux(s3)
    return s2


def quote_xml_aux(inStr):
    s1 = inStr.replace('&', '&amp;')
    s1 = s1.replace('<', '&lt;')
    s1 = s1.replace('>', '&gt;')
    return s1


def quote_attrib(inStr):
    s1 = (isinstance(inStr, BaseStrType_) and inStr or '%s' % inStr)
    s1 = s1.replace('&', '&amp;')
    s1 = s1.replace('<', '&lt;')
    s1 = s1.replace('>', '&gt;')
    if '"' in s1:
        if "'" in s1:
            s1 = '"%s"' % s1.replace('"', "&quot;")
        else:
            s1 = "'%s'" % s1
    else:
        s1 = '"%s"' % s1
    return s1


def quote_python(inStr):
    s1 = inStr
    if s1.find("'") == -1:
        if s1.find('\n') == -1:
            return "'%s'" % s1
        else:
            return "'''%s'''" % s1
    else:
        if s1.find('"') != -1:
            s1 = s1.replace('"', '\\"')
        if s1.find('\n') == -1:
            return '"%s"' % s1
        else:
            return '"""%s"""' % s1


def get_all_text_(node):
    if node.text is not None:
        text = node.text
    else:
        text = ''
    for child in node:
        if child.tail is not None:
            text += child.tail
    return text


def find_attr_value_(attr_name, node):
    attrs = node.attrib
    attr_parts = attr_name.split(':')
    value = None
    if len(attr_parts) == 1:
        value = attrs.get(attr_name)
    elif len(attr_parts) == 2:
        prefix, name = attr_parts
        namespace = node.nsmap.get(prefix)
        if namespace is not None:
            value = attrs.get('{%s}%s' % (namespace, name, ))
    return value


class GDSParseError(Exception):
    pass


def raise_parse_error(node, msg):
    msg = '%s (element %s/line %d)' % (msg, node.tag, node.sourceline, )
    raise GDSParseError(msg)


class MixedContainer:
    # Constants for category:
    CategoryNone = 0
    CategoryText = 1
    CategorySimple = 2
    CategoryComplex = 3
    # Constants for content_type:
    TypeNone = 0
    TypeText = 1
    TypeString = 2
    TypeInteger = 3
    TypeFloat = 4
    TypeDecimal = 5
    TypeDouble = 6
    TypeBoolean = 7
    TypeBase64 = 8
    def __init__(self, category, content_type, name, value):
        self.category = category
        self.content_type = content_type
        self.name = name
        self.value = value
    def getCategory(self):
        return self.category
    def getContenttype(self, content_type):
        return self.content_type
    def getValue(self):
        return self.value
    def getName(self):
        return self.name
    def export(self, outfile, level, name, namespace,
               pretty_print=True):
        if self.category == MixedContainer.CategoryText:
            # Prevent exporting empty content as empty lines.
            if self.value.strip():
                outfile.write(self.value)
        elif self.category == MixedContainer.CategorySimple:
            self.exportSimple(outfile, level, name)
        else:    # category == MixedContainer.CategoryComplex
            self.value.export(
                outfile, level, namespace, name,
                pretty_print=pretty_print)
    def exportSimple(self, outfile, level, name):
        if self.content_type == MixedContainer.TypeString:
            outfile.write('<%s>%s</%s>' % (
                self.name, self.value, self.name))
        elif self.content_type == MixedContainer.TypeInteger or \
                self.content_type == MixedContainer.TypeBoolean:
            outfile.write('<%s>%d</%s>' % (
                self.name, self.value, self.name))
        elif self.content_type == MixedContainer.TypeFloat or \
                self.content_type == MixedContainer.TypeDecimal:
            outfile.write('<%s>%f</%s>' % (
                self.name, self.value, self.name))
        elif self.content_type == MixedContainer.TypeDouble:
            outfile.write('<%s>%g</%s>' % (
                self.name, self.value, self.name))
        elif self.content_type == MixedContainer.TypeBase64:
            outfile.write('<%s>%s</%s>' % (
                self.name,
                base64.b64encode(self.value),
                self.name))
    def to_etree(self, element):
        if self.category == MixedContainer.CategoryText:
            # Prevent exporting empty content as empty lines.
            if self.value.strip():
                if len(element) > 0:
                    if element[-1].tail is None:
                        element[-1].tail = self.value
                    else:
                        element[-1].tail += self.value
                else:
                    if element.text is None:
                        element.text = self.value
                    else:
                        element.text += self.value
        elif self.category == MixedContainer.CategorySimple:
            subelement = etree_.SubElement(
                element, '%s' % self.name)
            subelement.text = self.to_etree_simple()
        else:    # category == MixedContainer.CategoryComplex
            self.value.to_etree(element)
    def to_etree_simple(self):
        if self.content_type == MixedContainer.TypeString:
            text = self.value
        elif (self.content_type == MixedContainer.TypeInteger or
                self.content_type == MixedContainer.TypeBoolean):
            text = '%d' % self.value
        elif (self.content_type == MixedContainer.TypeFloat or
                self.content_type == MixedContainer.TypeDecimal):
            text = '%f' % self.value
        elif self.content_type == MixedContainer.TypeDouble:
            text = '%g' % self.value
        elif self.content_type == MixedContainer.TypeBase64:
            text = '%s' % base64.b64encode(self.value)
        return text
    def exportLiteral(self, outfile, level, name):
        if self.category == MixedContainer.CategoryText:
            showIndent(outfile, level)
            outfile.write(
                'model_.MixedContainer(%d, %d, "%s", "%s"),\n' % (
                    self.category, self.content_type,
                    self.name, self.value))
        elif self.category == MixedContainer.CategorySimple:
            showIndent(outfile, level)
            outfile.write(
                'model_.MixedContainer(%d, %d, "%s", "%s"),\n' % (
                    self.category, self.content_type,
                    self.name, self.value))
        else:    # category == MixedContainer.CategoryComplex
            showIndent(outfile, level)
            outfile.write(
                'model_.MixedContainer(%d, %d, "%s",\n' % (
                    self.category, self.content_type, self.name,))
            self.value.exportLiteral(outfile, level + 1)
            showIndent(outfile, level)
            outfile.write(')\n')


class MemberSpec_(object):
    def __init__(self, name='', data_type='', container=0,
            optional=0, child_attrs=None, choice=None):
        self.name = name
        self.data_type = data_type
        self.container = container
        self.child_attrs = child_attrs
        self.choice = choice
        self.optional = optional
    def set_name(self, name): self.name = name
    def get_name(self): return self.name
    def set_data_type(self, data_type): self.data_type = data_type
    def get_data_type_chain(self): return self.data_type
    def get_data_type(self):
        if isinstance(self.data_type, list):
            if len(self.data_type) > 0:
                return self.data_type[-1]
            else:
                return 'xs:string'
        else:
            return self.data_type
    def set_container(self, container): self.container = container
    def get_container(self): return self.container
    def set_child_attrs(self, child_attrs): self.child_attrs = child_attrs
    def get_child_attrs(self): return self.child_attrs
    def set_choice(self, choice): self.choice = choice
    def get_choice(self): return self.choice
    def set_optional(self, optional): self.optional = optional
    def get_optional(self): return self.optional


def _cast(typ, value):
    if typ is None or value is None:
        return value
    return typ(value)

#
# Data representation classes.
#


class RequestContainer(GeneratedsSuper):
    subclass = None
    superclass = None
    def __init__(self):
        self.original_tagname_ = None
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, RequestContainer)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if RequestContainer.subclass:
            return RequestContainer.subclass(*args_, **kwargs_)
        else:
            return RequestContainer(*args_, **kwargs_)
    factory = staticmethod(factory)
    def hasContent_(self):
        if (

        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='RequestContainer', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('RequestContainer')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='RequestContainer')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='RequestContainer', pretty_print=pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='RequestContainer'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='RequestContainer', fromsubclass_=False, pretty_print=True):
        pass
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        pass
# end class RequestContainer


class Shipment(GeneratedsSuper):
    """Shipment"""
    subclass = None
    superclass = None
    def __init__(self, SenderInformation=None, ReceiverInformation=None, FromOnLabelIndicator=None, FromOnLabelInformation=None, ShipmentDate=None, PackageInformation=None, InternationalInformation=None, ReturnShipmentInformation=None, PaymentInformation=None, PickupInformation=None, NotificationInformation=None, TrackingReferenceInformation=None, OtherInformation=None, ProactiveNotification=None):
        self.original_tagname_ = None
        self.SenderInformation = SenderInformation
        self.ReceiverInformation = ReceiverInformation
        self.FromOnLabelIndicator = FromOnLabelIndicator
        self.FromOnLabelInformation = FromOnLabelInformation
        self.ShipmentDate = ShipmentDate
        self.PackageInformation = PackageInformation
        self.InternationalInformation = InternationalInformation
        self.ReturnShipmentInformation = ReturnShipmentInformation
        self.PaymentInformation = PaymentInformation
        self.PickupInformation = PickupInformation
        self.NotificationInformation = NotificationInformation
        self.TrackingReferenceInformation = TrackingReferenceInformation
        self.OtherInformation = OtherInformation
        self.ProactiveNotification = ProactiveNotification
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, Shipment)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if Shipment.subclass:
            return Shipment.subclass(*args_, **kwargs_)
        else:
            return Shipment(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_SenderInformation(self): return self.SenderInformation
    def set_SenderInformation(self, SenderInformation): self.SenderInformation = SenderInformation
    def get_ReceiverInformation(self): return self.ReceiverInformation
    def set_ReceiverInformation(self, ReceiverInformation): self.ReceiverInformation = ReceiverInformation
    def get_FromOnLabelIndicator(self): return self.FromOnLabelIndicator
    def set_FromOnLabelIndicator(self, FromOnLabelIndicator): self.FromOnLabelIndicator = FromOnLabelIndicator
    def get_FromOnLabelInformation(self): return self.FromOnLabelInformation
    def set_FromOnLabelInformation(self, FromOnLabelInformation): self.FromOnLabelInformation = FromOnLabelInformation
    def get_ShipmentDate(self): return self.ShipmentDate
    def set_ShipmentDate(self, ShipmentDate): self.ShipmentDate = ShipmentDate
    def get_PackageInformation(self): return self.PackageInformation
    def set_PackageInformation(self, PackageInformation): self.PackageInformation = PackageInformation
    def get_InternationalInformation(self): return self.InternationalInformation
    def set_InternationalInformation(self, InternationalInformation): self.InternationalInformation = InternationalInformation
    def get_ReturnShipmentInformation(self): return self.ReturnShipmentInformation
    def set_ReturnShipmentInformation(self, ReturnShipmentInformation): self.ReturnShipmentInformation = ReturnShipmentInformation
    def get_PaymentInformation(self): return self.PaymentInformation
    def set_PaymentInformation(self, PaymentInformation): self.PaymentInformation = PaymentInformation
    def get_PickupInformation(self): return self.PickupInformation
    def set_PickupInformation(self, PickupInformation): self.PickupInformation = PickupInformation
    def get_NotificationInformation(self): return self.NotificationInformation
    def set_NotificationInformation(self, NotificationInformation): self.NotificationInformation = NotificationInformation
    def get_TrackingReferenceInformation(self): return self.TrackingReferenceInformation
    def set_TrackingReferenceInformation(self, TrackingReferenceInformation): self.TrackingReferenceInformation = TrackingReferenceInformation
    def get_OtherInformation(self): return self.OtherInformation
    def set_OtherInformation(self, OtherInformation): self.OtherInformation = OtherInformation
    def get_ProactiveNotification(self): return self.ProactiveNotification
    def set_ProactiveNotification(self, ProactiveNotification): self.ProactiveNotification = ProactiveNotification
    def hasContent_(self):
        if (
            self.SenderInformation is not None or
            self.ReceiverInformation is not None or
            self.FromOnLabelIndicator is not None or
            self.FromOnLabelInformation is not None or
            self.ShipmentDate is not None or
            self.PackageInformation is not None or
            self.InternationalInformation is not None or
            self.ReturnShipmentInformation is not None or
            self.PaymentInformation is not None or
            self.PickupInformation is not None or
            self.NotificationInformation is not None or
            self.TrackingReferenceInformation is not None or
            self.OtherInformation is not None or
            self.ProactiveNotification is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='Shipment', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('Shipment')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='Shipment')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='Shipment', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='Shipment'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='Shipment', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.SenderInformation is not None:
            self.SenderInformation.export(outfile, level, namespace_, name_='SenderInformation', pretty_print=pretty_print)
        if self.ReceiverInformation is not None:
            self.ReceiverInformation.export(outfile, level, namespace_, name_='ReceiverInformation', pretty_print=pretty_print)
        if self.FromOnLabelIndicator is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:FromOnLabelIndicator>%s</tns:FromOnLabelIndicator>%s' % (self.gds_format_boolean(self.FromOnLabelIndicator, input_name='FromOnLabelIndicator'), eol_))
        if self.FromOnLabelInformation is not None:
            self.FromOnLabelInformation.export(outfile, level, namespace_, name_='FromOnLabelInformation', pretty_print=pretty_print)
        if self.ShipmentDate is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:ShipmentDate>%s</tns:ShipmentDate>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.ShipmentDate), input_name='ShipmentDate')), eol_))
        if self.PackageInformation is not None:
            self.PackageInformation.export(outfile, level, namespace_, name_='PackageInformation', pretty_print=pretty_print)
        if self.InternationalInformation is not None:
            self.InternationalInformation.export(outfile, level, namespace_, name_='InternationalInformation', pretty_print=pretty_print)
        if self.ReturnShipmentInformation is not None:
            self.ReturnShipmentInformation.export(outfile, level, namespace_, name_='ReturnShipmentInformation', pretty_print=pretty_print)
        if self.PaymentInformation is not None:
            self.PaymentInformation.export(outfile, level, namespace_, name_='PaymentInformation', pretty_print=pretty_print)
        if self.PickupInformation is not None:
            self.PickupInformation.export(outfile, level, namespace_, name_='PickupInformation', pretty_print=pretty_print)
        if self.NotificationInformation is not None:
            self.NotificationInformation.export(outfile, level, namespace_, name_='NotificationInformation', pretty_print=pretty_print)
        if self.TrackingReferenceInformation is not None:
            self.TrackingReferenceInformation.export(outfile, level, namespace_, name_='TrackingReferenceInformation', pretty_print=pretty_print)
        if self.OtherInformation is not None:
            self.OtherInformation.export(outfile, level, namespace_, name_='OtherInformation', pretty_print=pretty_print)
        if self.ProactiveNotification is not None:
            self.ProactiveNotification.export(outfile, level, namespace_, name_='ProactiveNotification', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'SenderInformation':
            obj_ = SenderInformation.factory()
            obj_.build(child_)
            self.SenderInformation = obj_
            obj_.original_tagname_ = 'SenderInformation'
        elif nodeName_ == 'ReceiverInformation':
            obj_ = ReceiverInformation.factory()
            obj_.build(child_)
            self.ReceiverInformation = obj_
            obj_.original_tagname_ = 'ReceiverInformation'
        elif nodeName_ == 'FromOnLabelIndicator':
            sval_ = child_.text
            if sval_ in ('true', '1'):
                ival_ = True
            elif sval_ in ('false', '0'):
                ival_ = False
            else:
                raise_parse_error(child_, 'requires boolean')
            ival_ = self.gds_validate_boolean(ival_, node, 'FromOnLabelIndicator')
            self.FromOnLabelIndicator = ival_
        elif nodeName_ == 'FromOnLabelInformation':
            obj_ = FromOnLabelInformation.factory()
            obj_.build(child_)
            self.FromOnLabelInformation = obj_
            obj_.original_tagname_ = 'FromOnLabelInformation'
        elif nodeName_ == 'ShipmentDate':
            ShipmentDate_ = child_.text
            ShipmentDate_ = self.gds_validate_string(ShipmentDate_, node, 'ShipmentDate')
            self.ShipmentDate = ShipmentDate_
        elif nodeName_ == 'PackageInformation':
            obj_ = PackageInformation.factory()
            obj_.build(child_)
            self.PackageInformation = obj_
            obj_.original_tagname_ = 'PackageInformation'
        elif nodeName_ == 'InternationalInformation':
            obj_ = InternationalInformation.factory()
            obj_.build(child_)
            self.InternationalInformation = obj_
            obj_.original_tagname_ = 'InternationalInformation'
        elif nodeName_ == 'ReturnShipmentInformation':
            obj_ = ReturnShipmentInformation.factory()
            obj_.build(child_)
            self.ReturnShipmentInformation = obj_
            obj_.original_tagname_ = 'ReturnShipmentInformation'
        elif nodeName_ == 'PaymentInformation':
            obj_ = PaymentInformation.factory()
            obj_.build(child_)
            self.PaymentInformation = obj_
            obj_.original_tagname_ = 'PaymentInformation'
        elif nodeName_ == 'PickupInformation':
            obj_ = PickupInformation.factory()
            obj_.build(child_)
            self.PickupInformation = obj_
            obj_.original_tagname_ = 'PickupInformation'
        elif nodeName_ == 'NotificationInformation':
            obj_ = NotificationInformation.factory()
            obj_.build(child_)
            self.NotificationInformation = obj_
            obj_.original_tagname_ = 'NotificationInformation'
        elif nodeName_ == 'TrackingReferenceInformation':
            obj_ = TrackingReferenceInformation.factory()
            obj_.build(child_)
            self.TrackingReferenceInformation = obj_
            obj_.original_tagname_ = 'TrackingReferenceInformation'
        elif nodeName_ == 'OtherInformation':
            obj_ = OtherInformation.factory()
            obj_.build(child_)
            self.OtherInformation = obj_
            obj_.original_tagname_ = 'OtherInformation'
        elif nodeName_ == 'ProactiveNotification':
            obj_ = ProactiveNotification.factory()
            obj_.build(child_)
            self.ProactiveNotification = obj_
            obj_.original_tagname_ = 'ProactiveNotification'
# end class Shipment


class SenderInformation(GeneratedsSuper):
    """SenderInformation"""
    subclass = None
    superclass = None
    def __init__(self, Address=None, TaxNumber=None):
        self.original_tagname_ = None
        self.Address = Address
        self.TaxNumber = TaxNumber
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, SenderInformation)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if SenderInformation.subclass:
            return SenderInformation.subclass(*args_, **kwargs_)
        else:
            return SenderInformation(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Address(self): return self.Address
    def set_Address(self, Address): self.Address = Address
    def get_TaxNumber(self): return self.TaxNumber
    def set_TaxNumber(self, TaxNumber): self.TaxNumber = TaxNumber
    def hasContent_(self):
        if (
            self.Address is not None or
            self.TaxNumber is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='SenderInformation', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('SenderInformation')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='SenderInformation')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='SenderInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='SenderInformation'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='SenderInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Address is not None:
            self.Address.export(outfile, level, namespace_, name_='Address', pretty_print=pretty_print)
        if self.TaxNumber is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:TaxNumber>%s</tns:TaxNumber>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.TaxNumber), input_name='TaxNumber')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Address':
            obj_ = Address.factory()
            obj_.build(child_)
            self.Address = obj_
            obj_.original_tagname_ = 'Address'
        elif nodeName_ == 'TaxNumber':
            TaxNumber_ = child_.text
            TaxNumber_ = self.gds_validate_string(TaxNumber_, node, 'TaxNumber')
            self.TaxNumber = TaxNumber_
# end class SenderInformation


class Address(GeneratedsSuper):
    """Address"""
    subclass = None
    superclass = None
    def __init__(self, Name=None, Company=None, Department=None, StreetNumber=None, StreetSuffix=None, StreetName=None, StreetType=None, StreetDirection=None, Suite=None, Floor=None, StreetAddress2=None, StreetAddress3=None, City=None, Province=None, Country=None, PostalCode=None, PhoneNumber=None, FaxNumber=None):
        self.original_tagname_ = None
        self.Name = Name
        self.Company = Company
        self.Department = Department
        self.StreetNumber = StreetNumber
        self.StreetSuffix = StreetSuffix
        self.StreetName = StreetName
        self.StreetType = StreetType
        self.StreetDirection = StreetDirection
        self.Suite = Suite
        self.Floor = Floor
        self.StreetAddress2 = StreetAddress2
        self.StreetAddress3 = StreetAddress3
        self.City = City
        self.Province = Province
        self.Country = Country
        self.PostalCode = PostalCode
        self.PhoneNumber = PhoneNumber
        self.FaxNumber = FaxNumber
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, Address)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if Address.subclass:
            return Address.subclass(*args_, **kwargs_)
        else:
            return Address(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Name(self): return self.Name
    def set_Name(self, Name): self.Name = Name
    def get_Company(self): return self.Company
    def set_Company(self, Company): self.Company = Company
    def get_Department(self): return self.Department
    def set_Department(self, Department): self.Department = Department
    def get_StreetNumber(self): return self.StreetNumber
    def set_StreetNumber(self, StreetNumber): self.StreetNumber = StreetNumber
    def get_StreetSuffix(self): return self.StreetSuffix
    def set_StreetSuffix(self, StreetSuffix): self.StreetSuffix = StreetSuffix
    def get_StreetName(self): return self.StreetName
    def set_StreetName(self, StreetName): self.StreetName = StreetName
    def get_StreetType(self): return self.StreetType
    def set_StreetType(self, StreetType): self.StreetType = StreetType
    def get_StreetDirection(self): return self.StreetDirection
    def set_StreetDirection(self, StreetDirection): self.StreetDirection = StreetDirection
    def get_Suite(self): return self.Suite
    def set_Suite(self, Suite): self.Suite = Suite
    def get_Floor(self): return self.Floor
    def set_Floor(self, Floor): self.Floor = Floor
    def get_StreetAddress2(self): return self.StreetAddress2
    def set_StreetAddress2(self, StreetAddress2): self.StreetAddress2 = StreetAddress2
    def get_StreetAddress3(self): return self.StreetAddress3
    def set_StreetAddress3(self, StreetAddress3): self.StreetAddress3 = StreetAddress3
    def get_City(self): return self.City
    def set_City(self, City): self.City = City
    def get_Province(self): return self.Province
    def set_Province(self, Province): self.Province = Province
    def get_Country(self): return self.Country
    def set_Country(self, Country): self.Country = Country
    def get_PostalCode(self): return self.PostalCode
    def set_PostalCode(self, PostalCode): self.PostalCode = PostalCode
    def get_PhoneNumber(self): return self.PhoneNumber
    def set_PhoneNumber(self, PhoneNumber): self.PhoneNumber = PhoneNumber
    def get_FaxNumber(self): return self.FaxNumber
    def set_FaxNumber(self, FaxNumber): self.FaxNumber = FaxNumber
    def hasContent_(self):
        if (
            self.Name is not None or
            self.Company is not None or
            self.Department is not None or
            self.StreetNumber is not None or
            self.StreetSuffix is not None or
            self.StreetName is not None or
            self.StreetType is not None or
            self.StreetDirection is not None or
            self.Suite is not None or
            self.Floor is not None or
            self.StreetAddress2 is not None or
            self.StreetAddress3 is not None or
            self.City is not None or
            self.Province is not None or
            self.Country is not None or
            self.PostalCode is not None or
            self.PhoneNumber is not None or
            self.FaxNumber is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='Address', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('Address')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='Address')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='Address', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='Address'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='Address', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Name is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Name>%s</tns:Name>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Name), input_name='Name')), eol_))
        if self.Company is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Company>%s</tns:Company>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Company), input_name='Company')), eol_))
        if self.Department is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Department>%s</tns:Department>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Department), input_name='Department')), eol_))
        if self.StreetNumber is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:StreetNumber>%s</tns:StreetNumber>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.StreetNumber), input_name='StreetNumber')), eol_))
        if self.StreetSuffix is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:StreetSuffix>%s</tns:StreetSuffix>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.StreetSuffix), input_name='StreetSuffix')), eol_))
        if self.StreetName is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:StreetName>%s</tns:StreetName>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.StreetName), input_name='StreetName')), eol_))
        if self.StreetType is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:StreetType>%s</tns:StreetType>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.StreetType), input_name='StreetType')), eol_))
        if self.StreetDirection is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:StreetDirection>%s</tns:StreetDirection>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.StreetDirection), input_name='StreetDirection')), eol_))
        if self.Suite is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Suite>%s</tns:Suite>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Suite), input_name='Suite')), eol_))
        if self.Floor is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Floor>%s</tns:Floor>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Floor), input_name='Floor')), eol_))
        if self.StreetAddress2 is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:StreetAddress2>%s</tns:StreetAddress2>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.StreetAddress2), input_name='StreetAddress2')), eol_))
        if self.StreetAddress3 is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:StreetAddress3>%s</tns:StreetAddress3>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.StreetAddress3), input_name='StreetAddress3')), eol_))
        if self.City is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:City>%s</tns:City>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.City), input_name='City')), eol_))
        if self.Province is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Province>%s</tns:Province>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Province), input_name='Province')), eol_))
        if self.Country is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Country>%s</tns:Country>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Country), input_name='Country')), eol_))
        if self.PostalCode is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:PostalCode>%s</tns:PostalCode>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.PostalCode), input_name='PostalCode')), eol_))
        if self.PhoneNumber is not None:
            self.PhoneNumber.export(outfile, level, namespace_, name_='PhoneNumber', pretty_print=pretty_print)
        if self.FaxNumber is not None:
            self.FaxNumber.export(outfile, level, namespace_, name_='FaxNumber', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Name':
            Name_ = child_.text
            Name_ = self.gds_validate_string(Name_, node, 'Name')
            self.Name = Name_
        elif nodeName_ == 'Company':
            Company_ = child_.text
            Company_ = self.gds_validate_string(Company_, node, 'Company')
            self.Company = Company_
        elif nodeName_ == 'Department':
            Department_ = child_.text
            Department_ = self.gds_validate_string(Department_, node, 'Department')
            self.Department = Department_
        elif nodeName_ == 'StreetNumber':
            StreetNumber_ = child_.text
            StreetNumber_ = self.gds_validate_string(StreetNumber_, node, 'StreetNumber')
            self.StreetNumber = StreetNumber_
        elif nodeName_ == 'StreetSuffix':
            StreetSuffix_ = child_.text
            StreetSuffix_ = self.gds_validate_string(StreetSuffix_, node, 'StreetSuffix')
            self.StreetSuffix = StreetSuffix_
        elif nodeName_ == 'StreetName':
            StreetName_ = child_.text
            StreetName_ = self.gds_validate_string(StreetName_, node, 'StreetName')
            self.StreetName = StreetName_
        elif nodeName_ == 'StreetType':
            StreetType_ = child_.text
            StreetType_ = self.gds_validate_string(StreetType_, node, 'StreetType')
            self.StreetType = StreetType_
        elif nodeName_ == 'StreetDirection':
            StreetDirection_ = child_.text
            StreetDirection_ = self.gds_validate_string(StreetDirection_, node, 'StreetDirection')
            self.StreetDirection = StreetDirection_
        elif nodeName_ == 'Suite':
            Suite_ = child_.text
            Suite_ = self.gds_validate_string(Suite_, node, 'Suite')
            self.Suite = Suite_
        elif nodeName_ == 'Floor':
            Floor_ = child_.text
            Floor_ = self.gds_validate_string(Floor_, node, 'Floor')
            self.Floor = Floor_
        elif nodeName_ == 'StreetAddress2':
            StreetAddress2_ = child_.text
            StreetAddress2_ = self.gds_validate_string(StreetAddress2_, node, 'StreetAddress2')
            self.StreetAddress2 = StreetAddress2_
        elif nodeName_ == 'StreetAddress3':
            StreetAddress3_ = child_.text
            StreetAddress3_ = self.gds_validate_string(StreetAddress3_, node, 'StreetAddress3')
            self.StreetAddress3 = StreetAddress3_
        elif nodeName_ == 'City':
            City_ = child_.text
            City_ = self.gds_validate_string(City_, node, 'City')
            self.City = City_
        elif nodeName_ == 'Province':
            Province_ = child_.text
            Province_ = self.gds_validate_string(Province_, node, 'Province')
            self.Province = Province_
        elif nodeName_ == 'Country':
            Country_ = child_.text
            Country_ = self.gds_validate_string(Country_, node, 'Country')
            self.Country = Country_
        elif nodeName_ == 'PostalCode':
            PostalCode_ = child_.text
            PostalCode_ = self.gds_validate_string(PostalCode_, node, 'PostalCode')
            self.PostalCode = PostalCode_
        elif nodeName_ == 'PhoneNumber':
            obj_ = PhoneNumber.factory()
            obj_.build(child_)
            self.PhoneNumber = obj_
            obj_.original_tagname_ = 'PhoneNumber'
        elif nodeName_ == 'FaxNumber':
            obj_ = PhoneNumber.factory()
            obj_.build(child_)
            self.FaxNumber = obj_
            obj_.original_tagname_ = 'FaxNumber'
# end class Address


class PhoneNumber(GeneratedsSuper):
    """PhoneNumber"""
    subclass = None
    superclass = None
    def __init__(self, CountryCode=None, AreaCode=None, Phone=None, Extension=None):
        self.original_tagname_ = None
        self.CountryCode = CountryCode
        self.AreaCode = AreaCode
        self.Phone = Phone
        self.Extension = Extension
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, PhoneNumber)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if PhoneNumber.subclass:
            return PhoneNumber.subclass(*args_, **kwargs_)
        else:
            return PhoneNumber(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_CountryCode(self): return self.CountryCode
    def set_CountryCode(self, CountryCode): self.CountryCode = CountryCode
    def get_AreaCode(self): return self.AreaCode
    def set_AreaCode(self, AreaCode): self.AreaCode = AreaCode
    def get_Phone(self): return self.Phone
    def set_Phone(self, Phone): self.Phone = Phone
    def get_Extension(self): return self.Extension
    def set_Extension(self, Extension): self.Extension = Extension
    def hasContent_(self):
        if (
            self.CountryCode is not None or
            self.AreaCode is not None or
            self.Phone is not None or
            self.Extension is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='PhoneNumber', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('PhoneNumber')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='PhoneNumber')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='PhoneNumber', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='PhoneNumber'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='PhoneNumber', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.CountryCode is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:CountryCode>%s</tns:CountryCode>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.CountryCode), input_name='CountryCode')), eol_))
        if self.AreaCode is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:AreaCode>%s</tns:AreaCode>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.AreaCode), input_name='AreaCode')), eol_))
        if self.Phone is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Phone>%s</tns:Phone>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Phone), input_name='Phone')), eol_))
        if self.Extension is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Extension>%s</tns:Extension>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Extension), input_name='Extension')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'CountryCode':
            CountryCode_ = child_.text
            CountryCode_ = self.gds_validate_string(CountryCode_, node, 'CountryCode')
            self.CountryCode = CountryCode_
        elif nodeName_ == 'AreaCode':
            AreaCode_ = child_.text
            AreaCode_ = self.gds_validate_string(AreaCode_, node, 'AreaCode')
            self.AreaCode = AreaCode_
        elif nodeName_ == 'Phone':
            Phone_ = child_.text
            Phone_ = self.gds_validate_string(Phone_, node, 'Phone')
            self.Phone = Phone_
        elif nodeName_ == 'Extension':
            Extension_ = child_.text
            Extension_ = self.gds_validate_string(Extension_, node, 'Extension')
            self.Extension = Extension_
# end class PhoneNumber


class ReceiverInformation(GeneratedsSuper):
    """ReceiverInformation"""
    subclass = None
    superclass = None
    def __init__(self, Address=None, TaxNumber=None):
        self.original_tagname_ = None
        self.Address = Address
        self.TaxNumber = TaxNumber
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ReceiverInformation)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ReceiverInformation.subclass:
            return ReceiverInformation.subclass(*args_, **kwargs_)
        else:
            return ReceiverInformation(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Address(self): return self.Address
    def set_Address(self, Address): self.Address = Address
    def get_TaxNumber(self): return self.TaxNumber
    def set_TaxNumber(self, TaxNumber): self.TaxNumber = TaxNumber
    def hasContent_(self):
        if (
            self.Address is not None or
            self.TaxNumber is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ReceiverInformation', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ReceiverInformation')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ReceiverInformation')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ReceiverInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ReceiverInformation'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ReceiverInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Address is not None:
            self.Address.export(outfile, level, namespace_, name_='Address', pretty_print=pretty_print)
        if self.TaxNumber is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:TaxNumber>%s</tns:TaxNumber>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.TaxNumber), input_name='TaxNumber')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Address':
            obj_ = Address.factory()
            obj_.build(child_)
            self.Address = obj_
            obj_.original_tagname_ = 'Address'
        elif nodeName_ == 'TaxNumber':
            TaxNumber_ = child_.text
            TaxNumber_ = self.gds_validate_string(TaxNumber_, node, 'TaxNumber')
            self.TaxNumber = TaxNumber_
# end class ReceiverInformation


class FromOnLabelInformation(GeneratedsSuper):
    """ReceiverInformation"""
    subclass = None
    superclass = None
    def __init__(self, Address=None):
        self.original_tagname_ = None
        self.Address = Address
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, FromOnLabelInformation)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if FromOnLabelInformation.subclass:
            return FromOnLabelInformation.subclass(*args_, **kwargs_)
        else:
            return FromOnLabelInformation(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Address(self): return self.Address
    def set_Address(self, Address): self.Address = Address
    def hasContent_(self):
        if (
            self.Address is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='FromOnLabelInformation', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('FromOnLabelInformation')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='FromOnLabelInformation')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='FromOnLabelInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='FromOnLabelInformation'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='FromOnLabelInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Address is not None:
            self.Address.export(outfile, level, namespace_, name_='Address', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Address':
            obj_ = Address.factory()
            obj_.build(child_)
            self.Address = obj_
            obj_.original_tagname_ = 'Address'
# end class FromOnLabelInformation


class PackageInformation(GeneratedsSuper):
    """PackageInformation"""
    subclass = None
    superclass = None
    def __init__(self, ServiceID=None, Description=None, TotalWeight=None, TotalPieces=None, PiecesInformation=None, DangerousGoodsDeclarationDocumentIndicator=None, OptionsInformation=None):
        self.original_tagname_ = None
        self.ServiceID = ServiceID
        self.Description = Description
        self.TotalWeight = TotalWeight
        self.TotalPieces = TotalPieces
        self.PiecesInformation = PiecesInformation
        self.DangerousGoodsDeclarationDocumentIndicator = DangerousGoodsDeclarationDocumentIndicator
        self.OptionsInformation = OptionsInformation
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, PackageInformation)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if PackageInformation.subclass:
            return PackageInformation.subclass(*args_, **kwargs_)
        else:
            return PackageInformation(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_ServiceID(self): return self.ServiceID
    def set_ServiceID(self, ServiceID): self.ServiceID = ServiceID
    def get_Description(self): return self.Description
    def set_Description(self, Description): self.Description = Description
    def get_TotalWeight(self): return self.TotalWeight
    def set_TotalWeight(self, TotalWeight): self.TotalWeight = TotalWeight
    def get_TotalPieces(self): return self.TotalPieces
    def set_TotalPieces(self, TotalPieces): self.TotalPieces = TotalPieces
    def get_PiecesInformation(self): return self.PiecesInformation
    def set_PiecesInformation(self, PiecesInformation): self.PiecesInformation = PiecesInformation
    def get_DangerousGoodsDeclarationDocumentIndicator(self): return self.DangerousGoodsDeclarationDocumentIndicator
    def set_DangerousGoodsDeclarationDocumentIndicator(self, DangerousGoodsDeclarationDocumentIndicator): self.DangerousGoodsDeclarationDocumentIndicator = DangerousGoodsDeclarationDocumentIndicator
    def get_OptionsInformation(self): return self.OptionsInformation
    def set_OptionsInformation(self, OptionsInformation): self.OptionsInformation = OptionsInformation
    def hasContent_(self):
        if (
            self.ServiceID is not None or
            self.Description is not None or
            self.TotalWeight is not None or
            self.TotalPieces is not None or
            self.PiecesInformation is not None or
            self.DangerousGoodsDeclarationDocumentIndicator is not None or
            self.OptionsInformation is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='PackageInformation', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('PackageInformation')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='PackageInformation')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='PackageInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='PackageInformation'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='PackageInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.ServiceID is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:ServiceID>%s</tns:ServiceID>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.ServiceID), input_name='ServiceID')), eol_))
        if self.Description is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Description>%s</tns:Description>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Description), input_name='Description')), eol_))
        if self.TotalWeight is not None:
            self.TotalWeight.export(outfile, level, namespace_, name_='TotalWeight', pretty_print=pretty_print)
        if self.TotalPieces is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:TotalPieces>%s</tns:TotalPieces>%s' % (self.gds_format_integer(self.TotalPieces, input_name='TotalPieces'), eol_))
        if self.PiecesInformation is not None:
            self.PiecesInformation.export(outfile, level, namespace_, name_='PiecesInformation', pretty_print=pretty_print)
        if self.DangerousGoodsDeclarationDocumentIndicator is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:DangerousGoodsDeclarationDocumentIndicator>%s</tns:DangerousGoodsDeclarationDocumentIndicator>%s' % (self.gds_format_boolean(self.DangerousGoodsDeclarationDocumentIndicator, input_name='DangerousGoodsDeclarationDocumentIndicator'), eol_))
        if self.OptionsInformation is not None:
            self.OptionsInformation.export(outfile, level, namespace_, name_='OptionsInformation', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'ServiceID':
            ServiceID_ = child_.text
            ServiceID_ = self.gds_validate_string(ServiceID_, node, 'ServiceID')
            self.ServiceID = ServiceID_
        elif nodeName_ == 'Description':
            Description_ = child_.text
            Description_ = self.gds_validate_string(Description_, node, 'Description')
            self.Description = Description_
        elif nodeName_ == 'TotalWeight':
            obj_ = TotalWeight.factory()
            obj_.build(child_)
            self.TotalWeight = obj_
            obj_.original_tagname_ = 'TotalWeight'
        elif nodeName_ == 'TotalPieces':
            sval_ = child_.text
            try:
                ival_ = int(sval_)
            except (TypeError, ValueError) as exp:
                raise_parse_error(child_, 'requires integer: %s' % exp)
            ival_ = self.gds_validate_integer(ival_, node, 'TotalPieces')
            self.TotalPieces = ival_
        elif nodeName_ == 'PiecesInformation':
            obj_ = ArrayOfPiece.factory()
            obj_.build(child_)
            self.PiecesInformation = obj_
            obj_.original_tagname_ = 'PiecesInformation'
        elif nodeName_ == 'DangerousGoodsDeclarationDocumentIndicator':
            sval_ = child_.text
            if sval_ in ('true', '1'):
                ival_ = True
            elif sval_ in ('false', '0'):
                ival_ = False
            else:
                raise_parse_error(child_, 'requires boolean')
            ival_ = self.gds_validate_boolean(ival_, node, 'DangerousGoodsDeclarationDocumentIndicator')
            self.DangerousGoodsDeclarationDocumentIndicator = ival_
        elif nodeName_ == 'OptionsInformation':
            obj_ = OptionsInformation.factory()
            obj_.build(child_)
            self.OptionsInformation = obj_
            obj_.original_tagname_ = 'OptionsInformation'
# end class PackageInformation


class TotalWeight(GeneratedsSuper):
    """TotalWeight"""
    subclass = None
    superclass = None
    def __init__(self, Value=None, WeightUnit=None):
        self.original_tagname_ = None
        self.Value = Value
        self.WeightUnit = WeightUnit
        self.validate_WeightUnit(self.WeightUnit)
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, TotalWeight)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if TotalWeight.subclass:
            return TotalWeight.subclass(*args_, **kwargs_)
        else:
            return TotalWeight(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Value(self): return self.Value
    def set_Value(self, Value): self.Value = Value
    def get_WeightUnit(self): return self.WeightUnit
    def set_WeightUnit(self, WeightUnit): self.WeightUnit = WeightUnit
    def validate_WeightUnit(self, value):
        # Validate type WeightUnit, a restriction on xs:string.
        if value is not None and Validate_simpletypes_:
            value = str(value)
            enumerations = ['lb', 'kg']
            enumeration_respectee = False
            for enum in enumerations:
                if value == enum:
                    enumeration_respectee = True
                    break
            if not enumeration_respectee:
                warnings_.warn('Value "%(value)s" does not match xsd enumeration restriction on WeightUnit' % {"value" : value.encode("utf-8")} )
    def hasContent_(self):
        if (
            self.Value is not None or
            self.WeightUnit is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='TotalWeight', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('TotalWeight')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='TotalWeight')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='TotalWeight', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='TotalWeight'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='TotalWeight', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Value is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Value>%s</tns:Value>%s' % (self.gds_format_integer(self.Value, input_name='Value'), eol_))
        if self.WeightUnit is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:WeightUnit>%s</tns:WeightUnit>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.WeightUnit), input_name='WeightUnit')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Value':
            sval_ = child_.text
            try:
                ival_ = int(sval_)
            except (TypeError, ValueError) as exp:
                raise_parse_error(child_, 'requires integer: %s' % exp)
            ival_ = self.gds_validate_integer(ival_, node, 'Value')
            self.Value = ival_
        elif nodeName_ == 'WeightUnit':
            WeightUnit_ = child_.text
            WeightUnit_ = self.gds_validate_string(WeightUnit_, node, 'WeightUnit')
            self.WeightUnit = WeightUnit_
            # validate type WeightUnit
            self.validate_WeightUnit(self.WeightUnit)
# end class TotalWeight


class ArrayOfPiece(GeneratedsSuper):
    subclass = None
    superclass = None
    def __init__(self, Piece=None):
        self.original_tagname_ = None
        if Piece is None:
            self.Piece = []
        else:
            self.Piece = Piece
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ArrayOfPiece)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ArrayOfPiece.subclass:
            return ArrayOfPiece.subclass(*args_, **kwargs_)
        else:
            return ArrayOfPiece(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Piece(self): return self.Piece
    def set_Piece(self, Piece): self.Piece = Piece
    def add_Piece(self, value): self.Piece.append(value)
    def insert_Piece_at(self, index, value): self.Piece.insert(index, value)
    def replace_Piece_at(self, index, value): self.Piece[index] = value
    def hasContent_(self):
        if (
            self.Piece
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ArrayOfPiece', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ArrayOfPiece')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ArrayOfPiece')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ArrayOfPiece', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ArrayOfPiece'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ArrayOfPiece', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        for Piece_ in self.Piece:
            Piece_.export(outfile, level, namespace_, name_='Piece', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Piece':
            obj_ = Piece.factory()
            obj_.build(child_)
            self.Piece.append(obj_)
            obj_.original_tagname_ = 'Piece'
# end class ArrayOfPiece


class Piece(GeneratedsSuper):
    """Piece"""
    subclass = None
    superclass = None
    def __init__(self, Weight=None, Length=None, Width=None, Height=None, Options=None):
        self.original_tagname_ = None
        self.Weight = Weight
        self.Length = Length
        self.Width = Width
        self.Height = Height
        self.Options = Options
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, Piece)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if Piece.subclass:
            return Piece.subclass(*args_, **kwargs_)
        else:
            return Piece(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Weight(self): return self.Weight
    def set_Weight(self, Weight): self.Weight = Weight
    def get_Length(self): return self.Length
    def set_Length(self, Length): self.Length = Length
    def get_Width(self): return self.Width
    def set_Width(self, Width): self.Width = Width
    def get_Height(self): return self.Height
    def set_Height(self, Height): self.Height = Height
    def get_Options(self): return self.Options
    def set_Options(self, Options): self.Options = Options
    def hasContent_(self):
        if (
            self.Weight is not None or
            self.Length is not None or
            self.Width is not None or
            self.Height is not None or
            self.Options is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='Piece', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('Piece')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='Piece')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='Piece', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='Piece'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='Piece', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Weight is not None:
            self.Weight.export(outfile, level, namespace_, name_='Weight', pretty_print=pretty_print)
        if self.Length is not None:
            self.Length.export(outfile, level, namespace_, name_='Length', pretty_print=pretty_print)
        if self.Width is not None:
            self.Width.export(outfile, level, namespace_, name_='Width', pretty_print=pretty_print)
        if self.Height is not None:
            self.Height.export(outfile, level, namespace_, name_='Height', pretty_print=pretty_print)
        if self.Options is not None:
            self.Options.export(outfile, level, namespace_, name_='Options', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Weight':
            obj_ = Weight.factory()
            obj_.build(child_)
            self.Weight = obj_
            obj_.original_tagname_ = 'Weight'
        elif nodeName_ == 'Length':
            obj_ = Dimension.factory()
            obj_.build(child_)
            self.Length = obj_
            obj_.original_tagname_ = 'Length'
        elif nodeName_ == 'Width':
            obj_ = Dimension.factory()
            obj_.build(child_)
            self.Width = obj_
            obj_.original_tagname_ = 'Width'
        elif nodeName_ == 'Height':
            obj_ = Dimension.factory()
            obj_.build(child_)
            self.Height = obj_
            obj_.original_tagname_ = 'Height'
        elif nodeName_ == 'Options':
            obj_ = ArrayOfOptionIDValuePair.factory()
            obj_.build(child_)
            self.Options = obj_
            obj_.original_tagname_ = 'Options'
# end class Piece


class Weight(GeneratedsSuper):
    """Weight"""
    subclass = None
    superclass = None
    def __init__(self, Value=None, WeightUnit=None):
        self.original_tagname_ = None
        self.Value = Value
        self.WeightUnit = WeightUnit
        self.validate_WeightUnit(self.WeightUnit)
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, Weight)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if Weight.subclass:
            return Weight.subclass(*args_, **kwargs_)
        else:
            return Weight(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Value(self): return self.Value
    def set_Value(self, Value): self.Value = Value
    def get_WeightUnit(self): return self.WeightUnit
    def set_WeightUnit(self, WeightUnit): self.WeightUnit = WeightUnit
    def validate_WeightUnit(self, value):
        # Validate type WeightUnit, a restriction on xs:string.
        if value is not None and Validate_simpletypes_:
            value = str(value)
            enumerations = ['lb', 'kg']
            enumeration_respectee = False
            for enum in enumerations:
                if value == enum:
                    enumeration_respectee = True
                    break
            if not enumeration_respectee:
                warnings_.warn('Value "%(value)s" does not match xsd enumeration restriction on WeightUnit' % {"value" : value.encode("utf-8")} )
    def hasContent_(self):
        if (
            self.Value is not None or
            self.WeightUnit is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='Weight', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('Weight')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='Weight')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='Weight', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='Weight'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='Weight', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Value is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Value>%s</tns:Value>%s' % (self.gds_format_float(self.Value, input_name='Value'), eol_))
        if self.WeightUnit is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:WeightUnit>%s</tns:WeightUnit>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.WeightUnit), input_name='WeightUnit')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Value':
            sval_ = child_.text
            try:
                fval_ = float(sval_)
            except (TypeError, ValueError) as exp:
                raise_parse_error(child_, 'requires float or double: %s' % exp)
            fval_ = self.gds_validate_float(fval_, node, 'Value')
            self.Value = fval_
        elif nodeName_ == 'WeightUnit':
            WeightUnit_ = child_.text
            WeightUnit_ = self.gds_validate_string(WeightUnit_, node, 'WeightUnit')
            self.WeightUnit = WeightUnit_
            # validate type WeightUnit
            self.validate_WeightUnit(self.WeightUnit)
# end class Weight


class Dimension(GeneratedsSuper):
    """Dimension"""
    subclass = None
    superclass = None
    def __init__(self, Value=None, DimensionUnit=None):
        self.original_tagname_ = None
        self.Value = Value
        self.DimensionUnit = DimensionUnit
        self.validate_DimensionUnit(self.DimensionUnit)
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, Dimension)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if Dimension.subclass:
            return Dimension.subclass(*args_, **kwargs_)
        else:
            return Dimension(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Value(self): return self.Value
    def set_Value(self, Value): self.Value = Value
    def get_DimensionUnit(self): return self.DimensionUnit
    def set_DimensionUnit(self, DimensionUnit): self.DimensionUnit = DimensionUnit
    def validate_DimensionUnit(self, value):
        # Validate type DimensionUnit, a restriction on xs:string.
        if value is not None and Validate_simpletypes_:
            value = str(value)
            enumerations = ['in', 'cm']
            enumeration_respectee = False
            for enum in enumerations:
                if value == enum:
                    enumeration_respectee = True
                    break
            if not enumeration_respectee:
                warnings_.warn('Value "%(value)s" does not match xsd enumeration restriction on DimensionUnit' % {"value" : value.encode("utf-8")} )
    def hasContent_(self):
        if (
            self.Value is not None or
            self.DimensionUnit is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='Dimension', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('Dimension')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='Dimension')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='Dimension', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='Dimension'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='Dimension', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Value is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Value>%s</tns:Value>%s' % (self.gds_format_float(self.Value, input_name='Value'), eol_))
        if self.DimensionUnit is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:DimensionUnit>%s</tns:DimensionUnit>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.DimensionUnit), input_name='DimensionUnit')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Value':
            sval_ = child_.text
            try:
                fval_ = float(sval_)
            except (TypeError, ValueError) as exp:
                raise_parse_error(child_, 'requires float or double: %s' % exp)
            fval_ = self.gds_validate_float(fval_, node, 'Value')
            self.Value = fval_
        elif nodeName_ == 'DimensionUnit':
            DimensionUnit_ = child_.text
            DimensionUnit_ = self.gds_validate_string(DimensionUnit_, node, 'DimensionUnit')
            self.DimensionUnit = DimensionUnit_
            # validate type DimensionUnit
            self.validate_DimensionUnit(self.DimensionUnit)
# end class Dimension


class ArrayOfOptionIDValuePair(GeneratedsSuper):
    subclass = None
    superclass = None
    def __init__(self, OptionIDValuePair=None):
        self.original_tagname_ = None
        if OptionIDValuePair is None:
            self.OptionIDValuePair = []
        else:
            self.OptionIDValuePair = OptionIDValuePair
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ArrayOfOptionIDValuePair)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ArrayOfOptionIDValuePair.subclass:
            return ArrayOfOptionIDValuePair.subclass(*args_, **kwargs_)
        else:
            return ArrayOfOptionIDValuePair(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_OptionIDValuePair(self): return self.OptionIDValuePair
    def set_OptionIDValuePair(self, OptionIDValuePair): self.OptionIDValuePair = OptionIDValuePair
    def add_OptionIDValuePair(self, value): self.OptionIDValuePair.append(value)
    def insert_OptionIDValuePair_at(self, index, value): self.OptionIDValuePair.insert(index, value)
    def replace_OptionIDValuePair_at(self, index, value): self.OptionIDValuePair[index] = value
    def hasContent_(self):
        if (
            self.OptionIDValuePair
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ArrayOfOptionIDValuePair', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ArrayOfOptionIDValuePair')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ArrayOfOptionIDValuePair')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ArrayOfOptionIDValuePair', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ArrayOfOptionIDValuePair'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ArrayOfOptionIDValuePair', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        for OptionIDValuePair_ in self.OptionIDValuePair:
            OptionIDValuePair_.export(outfile, level, namespace_, name_='OptionIDValuePair', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'OptionIDValuePair':
            obj_ = OptionIDValuePair.factory()
            obj_.build(child_)
            self.OptionIDValuePair.append(obj_)
            obj_.original_tagname_ = 'OptionIDValuePair'
# end class ArrayOfOptionIDValuePair


class OptionIDValuePair(GeneratedsSuper):
    """OptionIDValuePair"""
    subclass = None
    superclass = None
    def __init__(self, ID=None, Value=None):
        self.original_tagname_ = None
        self.ID = ID
        self.Value = Value
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, OptionIDValuePair)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if OptionIDValuePair.subclass:
            return OptionIDValuePair.subclass(*args_, **kwargs_)
        else:
            return OptionIDValuePair(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_ID(self): return self.ID
    def set_ID(self, ID): self.ID = ID
    def get_Value(self): return self.Value
    def set_Value(self, Value): self.Value = Value
    def hasContent_(self):
        if (
            self.ID is not None or
            self.Value is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='OptionIDValuePair', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('OptionIDValuePair')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='OptionIDValuePair')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='OptionIDValuePair', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='OptionIDValuePair'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='OptionIDValuePair', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.ID is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:ID>%s</tns:ID>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.ID), input_name='ID')), eol_))
        if self.Value is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Value>%s</tns:Value>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Value), input_name='Value')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'ID':
            ID_ = child_.text
            ID_ = self.gds_validate_string(ID_, node, 'ID')
            self.ID = ID_
        elif nodeName_ == 'Value':
            Value_ = child_.text
            Value_ = self.gds_validate_string(Value_, node, 'Value')
            self.Value = Value_
# end class OptionIDValuePair


class OptionsInformation(GeneratedsSuper):
    """OptionsInformation"""
    subclass = None
    superclass = None
    def __init__(self, Options=None, ExpressChequeAddress=None):
        self.original_tagname_ = None
        self.Options = Options
        self.ExpressChequeAddress = ExpressChequeAddress
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, OptionsInformation)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if OptionsInformation.subclass:
            return OptionsInformation.subclass(*args_, **kwargs_)
        else:
            return OptionsInformation(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Options(self): return self.Options
    def set_Options(self, Options): self.Options = Options
    def get_ExpressChequeAddress(self): return self.ExpressChequeAddress
    def set_ExpressChequeAddress(self, ExpressChequeAddress): self.ExpressChequeAddress = ExpressChequeAddress
    def hasContent_(self):
        if (
            self.Options is not None or
            self.ExpressChequeAddress is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='OptionsInformation', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('OptionsInformation')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='OptionsInformation')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='OptionsInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='OptionsInformation'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='OptionsInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Options is not None:
            self.Options.export(outfile, level, namespace_, name_='Options', pretty_print=pretty_print)
        if self.ExpressChequeAddress is not None:
            self.ExpressChequeAddress.export(outfile, level, namespace_, name_='ExpressChequeAddress', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Options':
            obj_ = ArrayOfOptionIDValuePair.factory()
            obj_.build(child_)
            self.Options = obj_
            obj_.original_tagname_ = 'Options'
        elif nodeName_ == 'ExpressChequeAddress':
            obj_ = Address.factory()
            obj_.build(child_)
            self.ExpressChequeAddress = obj_
            obj_.original_tagname_ = 'ExpressChequeAddress'
# end class OptionsInformation


class InternationalInformation(GeneratedsSuper):
    """InternationalInformation"""
    subclass = None
    superclass = None
    def __init__(self, DocumentsOnlyIndicator=None, ContentDetails=None, BuyerInformation=None, PreferredCustomsBroker=None, DutyInformation=None, ImportExportType=None, CustomsInvoiceDocumentIndicator=None):
        self.original_tagname_ = None
        self.DocumentsOnlyIndicator = DocumentsOnlyIndicator
        self.ContentDetails = ContentDetails
        self.BuyerInformation = BuyerInformation
        self.PreferredCustomsBroker = PreferredCustomsBroker
        self.DutyInformation = DutyInformation
        self.ImportExportType = ImportExportType
        self.validate_ImportExportType(self.ImportExportType)
        self.CustomsInvoiceDocumentIndicator = CustomsInvoiceDocumentIndicator
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, InternationalInformation)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if InternationalInformation.subclass:
            return InternationalInformation.subclass(*args_, **kwargs_)
        else:
            return InternationalInformation(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_DocumentsOnlyIndicator(self): return self.DocumentsOnlyIndicator
    def set_DocumentsOnlyIndicator(self, DocumentsOnlyIndicator): self.DocumentsOnlyIndicator = DocumentsOnlyIndicator
    def get_ContentDetails(self): return self.ContentDetails
    def set_ContentDetails(self, ContentDetails): self.ContentDetails = ContentDetails
    def get_BuyerInformation(self): return self.BuyerInformation
    def set_BuyerInformation(self, BuyerInformation): self.BuyerInformation = BuyerInformation
    def get_PreferredCustomsBroker(self): return self.PreferredCustomsBroker
    def set_PreferredCustomsBroker(self, PreferredCustomsBroker): self.PreferredCustomsBroker = PreferredCustomsBroker
    def get_DutyInformation(self): return self.DutyInformation
    def set_DutyInformation(self, DutyInformation): self.DutyInformation = DutyInformation
    def get_ImportExportType(self): return self.ImportExportType
    def set_ImportExportType(self, ImportExportType): self.ImportExportType = ImportExportType
    def get_CustomsInvoiceDocumentIndicator(self): return self.CustomsInvoiceDocumentIndicator
    def set_CustomsInvoiceDocumentIndicator(self, CustomsInvoiceDocumentIndicator): self.CustomsInvoiceDocumentIndicator = CustomsInvoiceDocumentIndicator
    def validate_ImportExportType(self, value):
        # Validate type ImportExportType, a restriction on xs:string.
        if value is not None and Validate_simpletypes_:
            value = str(value)
            enumerations = ['Permanent', 'Temporary', 'Repair', 'Return']
            enumeration_respectee = False
            for enum in enumerations:
                if value == enum:
                    enumeration_respectee = True
                    break
            if not enumeration_respectee:
                warnings_.warn('Value "%(value)s" does not match xsd enumeration restriction on ImportExportType' % {"value" : value.encode("utf-8")} )
    def hasContent_(self):
        if (
            self.DocumentsOnlyIndicator is not None or
            self.ContentDetails is not None or
            self.BuyerInformation is not None or
            self.PreferredCustomsBroker is not None or
            self.DutyInformation is not None or
            self.ImportExportType is not None or
            self.CustomsInvoiceDocumentIndicator is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='InternationalInformation', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('InternationalInformation')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='InternationalInformation')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='InternationalInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='InternationalInformation'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='InternationalInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.DocumentsOnlyIndicator is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:DocumentsOnlyIndicator>%s</tns:DocumentsOnlyIndicator>%s' % (self.gds_format_boolean(self.DocumentsOnlyIndicator, input_name='DocumentsOnlyIndicator'), eol_))
        if self.ContentDetails is not None:
            self.ContentDetails.export(outfile, level, namespace_, name_='ContentDetails', pretty_print=pretty_print)
        if self.BuyerInformation is not None:
            self.BuyerInformation.export(outfile, level, namespace_, name_='BuyerInformation', pretty_print=pretty_print)
        if self.PreferredCustomsBroker is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:PreferredCustomsBroker>%s</tns:PreferredCustomsBroker>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.PreferredCustomsBroker), input_name='PreferredCustomsBroker')), eol_))
        if self.DutyInformation is not None:
            self.DutyInformation.export(outfile, level, namespace_, name_='DutyInformation', pretty_print=pretty_print)
        if self.ImportExportType is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:ImportExportType>%s</tns:ImportExportType>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.ImportExportType), input_name='ImportExportType')), eol_))
        if self.CustomsInvoiceDocumentIndicator is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:CustomsInvoiceDocumentIndicator>%s</tns:CustomsInvoiceDocumentIndicator>%s' % (self.gds_format_boolean(self.CustomsInvoiceDocumentIndicator, input_name='CustomsInvoiceDocumentIndicator'), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'DocumentsOnlyIndicator':
            sval_ = child_.text
            if sval_ in ('true', '1'):
                ival_ = True
            elif sval_ in ('false', '0'):
                ival_ = False
            else:
                raise_parse_error(child_, 'requires boolean')
            ival_ = self.gds_validate_boolean(ival_, node, 'DocumentsOnlyIndicator')
            self.DocumentsOnlyIndicator = ival_
        elif nodeName_ == 'ContentDetails':
            obj_ = ArrayOfContentDetail.factory()
            obj_.build(child_)
            self.ContentDetails = obj_
            obj_.original_tagname_ = 'ContentDetails'
        elif nodeName_ == 'BuyerInformation':
            obj_ = BuyerInformation.factory()
            obj_.build(child_)
            self.BuyerInformation = obj_
            obj_.original_tagname_ = 'BuyerInformation'
        elif nodeName_ == 'PreferredCustomsBroker':
            PreferredCustomsBroker_ = child_.text
            PreferredCustomsBroker_ = self.gds_validate_string(PreferredCustomsBroker_, node, 'PreferredCustomsBroker')
            self.PreferredCustomsBroker = PreferredCustomsBroker_
        elif nodeName_ == 'DutyInformation':
            obj_ = DutyInformation.factory()
            obj_.build(child_)
            self.DutyInformation = obj_
            obj_.original_tagname_ = 'DutyInformation'
        elif nodeName_ == 'ImportExportType':
            ImportExportType_ = child_.text
            ImportExportType_ = self.gds_validate_string(ImportExportType_, node, 'ImportExportType')
            self.ImportExportType = ImportExportType_
            # validate type ImportExportType
            self.validate_ImportExportType(self.ImportExportType)
        elif nodeName_ == 'CustomsInvoiceDocumentIndicator':
            sval_ = child_.text
            if sval_ in ('true', '1'):
                ival_ = True
            elif sval_ in ('false', '0'):
                ival_ = False
            else:
                raise_parse_error(child_, 'requires boolean')
            ival_ = self.gds_validate_boolean(ival_, node, 'CustomsInvoiceDocumentIndicator')
            self.CustomsInvoiceDocumentIndicator = ival_
# end class InternationalInformation


class ArrayOfContentDetail(GeneratedsSuper):
    subclass = None
    superclass = None
    def __init__(self, ContentDetail=None):
        self.original_tagname_ = None
        if ContentDetail is None:
            self.ContentDetail = []
        else:
            self.ContentDetail = ContentDetail
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ArrayOfContentDetail)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ArrayOfContentDetail.subclass:
            return ArrayOfContentDetail.subclass(*args_, **kwargs_)
        else:
            return ArrayOfContentDetail(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_ContentDetail(self): return self.ContentDetail
    def set_ContentDetail(self, ContentDetail): self.ContentDetail = ContentDetail
    def add_ContentDetail(self, value): self.ContentDetail.append(value)
    def insert_ContentDetail_at(self, index, value): self.ContentDetail.insert(index, value)
    def replace_ContentDetail_at(self, index, value): self.ContentDetail[index] = value
    def hasContent_(self):
        if (
            self.ContentDetail
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ArrayOfContentDetail', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ArrayOfContentDetail')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ArrayOfContentDetail')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ArrayOfContentDetail', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ArrayOfContentDetail'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ArrayOfContentDetail', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        for ContentDetail_ in self.ContentDetail:
            ContentDetail_.export(outfile, level, namespace_, name_='ContentDetail', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'ContentDetail':
            obj_ = ContentDetail.factory()
            obj_.build(child_)
            self.ContentDetail.append(obj_)
            obj_.original_tagname_ = 'ContentDetail'
# end class ArrayOfContentDetail


class ContentDetail(GeneratedsSuper):
    """ContentDetail"""
    subclass = None
    superclass = None
    def __init__(self, Description=None, HarmonizedCode=None, CountryOfManufacture=None, ProductCode=None, UnitValue=None, Quantity=None, NAFTADocumentIndicator=None, FDADocumentIndicator=None, FCCDocumentIndicator=None, SenderIsProducerIndicator=None, TextileIndicator=None, TextileManufacturer=None):
        self.original_tagname_ = None
        self.Description = Description
        self.HarmonizedCode = HarmonizedCode
        self.CountryOfManufacture = CountryOfManufacture
        self.ProductCode = ProductCode
        self.UnitValue = UnitValue
        self.Quantity = Quantity
        self.NAFTADocumentIndicator = NAFTADocumentIndicator
        self.FDADocumentIndicator = FDADocumentIndicator
        self.FCCDocumentIndicator = FCCDocumentIndicator
        self.SenderIsProducerIndicator = SenderIsProducerIndicator
        self.TextileIndicator = TextileIndicator
        self.TextileManufacturer = TextileManufacturer
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ContentDetail)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ContentDetail.subclass:
            return ContentDetail.subclass(*args_, **kwargs_)
        else:
            return ContentDetail(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Description(self): return self.Description
    def set_Description(self, Description): self.Description = Description
    def get_HarmonizedCode(self): return self.HarmonizedCode
    def set_HarmonizedCode(self, HarmonizedCode): self.HarmonizedCode = HarmonizedCode
    def get_CountryOfManufacture(self): return self.CountryOfManufacture
    def set_CountryOfManufacture(self, CountryOfManufacture): self.CountryOfManufacture = CountryOfManufacture
    def get_ProductCode(self): return self.ProductCode
    def set_ProductCode(self, ProductCode): self.ProductCode = ProductCode
    def get_UnitValue(self): return self.UnitValue
    def set_UnitValue(self, UnitValue): self.UnitValue = UnitValue
    def get_Quantity(self): return self.Quantity
    def set_Quantity(self, Quantity): self.Quantity = Quantity
    def get_NAFTADocumentIndicator(self): return self.NAFTADocumentIndicator
    def set_NAFTADocumentIndicator(self, NAFTADocumentIndicator): self.NAFTADocumentIndicator = NAFTADocumentIndicator
    def get_FDADocumentIndicator(self): return self.FDADocumentIndicator
    def set_FDADocumentIndicator(self, FDADocumentIndicator): self.FDADocumentIndicator = FDADocumentIndicator
    def get_FCCDocumentIndicator(self): return self.FCCDocumentIndicator
    def set_FCCDocumentIndicator(self, FCCDocumentIndicator): self.FCCDocumentIndicator = FCCDocumentIndicator
    def get_SenderIsProducerIndicator(self): return self.SenderIsProducerIndicator
    def set_SenderIsProducerIndicator(self, SenderIsProducerIndicator): self.SenderIsProducerIndicator = SenderIsProducerIndicator
    def get_TextileIndicator(self): return self.TextileIndicator
    def set_TextileIndicator(self, TextileIndicator): self.TextileIndicator = TextileIndicator
    def get_TextileManufacturer(self): return self.TextileManufacturer
    def set_TextileManufacturer(self, TextileManufacturer): self.TextileManufacturer = TextileManufacturer
    def hasContent_(self):
        if (
            self.Description is not None or
            self.HarmonizedCode is not None or
            self.CountryOfManufacture is not None or
            self.ProductCode is not None or
            self.UnitValue is not None or
            self.Quantity is not None or
            self.NAFTADocumentIndicator is not None or
            self.FDADocumentIndicator is not None or
            self.FCCDocumentIndicator is not None or
            self.SenderIsProducerIndicator is not None or
            self.TextileIndicator is not None or
            self.TextileManufacturer is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ContentDetail', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ContentDetail')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ContentDetail')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ContentDetail', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ContentDetail'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ContentDetail', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Description is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Description>%s</tns:Description>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Description), input_name='Description')), eol_))
        if self.HarmonizedCode is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:HarmonizedCode>%s</tns:HarmonizedCode>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.HarmonizedCode), input_name='HarmonizedCode')), eol_))
        if self.CountryOfManufacture is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:CountryOfManufacture>%s</tns:CountryOfManufacture>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.CountryOfManufacture), input_name='CountryOfManufacture')), eol_))
        if self.ProductCode is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:ProductCode>%s</tns:ProductCode>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.ProductCode), input_name='ProductCode')), eol_))
        if self.UnitValue is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:UnitValue>%s</tns:UnitValue>%s' % (self.gds_format_float(self.UnitValue, input_name='UnitValue'), eol_))
        if self.Quantity is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Quantity>%s</tns:Quantity>%s' % (self.gds_format_integer(self.Quantity, input_name='Quantity'), eol_))
        if self.NAFTADocumentIndicator is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:NAFTADocumentIndicator>%s</tns:NAFTADocumentIndicator>%s' % (self.gds_format_boolean(self.NAFTADocumentIndicator, input_name='NAFTADocumentIndicator'), eol_))
        if self.FDADocumentIndicator is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:FDADocumentIndicator>%s</tns:FDADocumentIndicator>%s' % (self.gds_format_boolean(self.FDADocumentIndicator, input_name='FDADocumentIndicator'), eol_))
        if self.FCCDocumentIndicator is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:FCCDocumentIndicator>%s</tns:FCCDocumentIndicator>%s' % (self.gds_format_boolean(self.FCCDocumentIndicator, input_name='FCCDocumentIndicator'), eol_))
        if self.SenderIsProducerIndicator is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:SenderIsProducerIndicator>%s</tns:SenderIsProducerIndicator>%s' % (self.gds_format_boolean(self.SenderIsProducerIndicator, input_name='SenderIsProducerIndicator'), eol_))
        if self.TextileIndicator is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:TextileIndicator>%s</tns:TextileIndicator>%s' % (self.gds_format_boolean(self.TextileIndicator, input_name='TextileIndicator'), eol_))
        if self.TextileManufacturer is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:TextileManufacturer>%s</tns:TextileManufacturer>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.TextileManufacturer), input_name='TextileManufacturer')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Description':
            Description_ = child_.text
            Description_ = self.gds_validate_string(Description_, node, 'Description')
            self.Description = Description_
        elif nodeName_ == 'HarmonizedCode':
            HarmonizedCode_ = child_.text
            HarmonizedCode_ = self.gds_validate_string(HarmonizedCode_, node, 'HarmonizedCode')
            self.HarmonizedCode = HarmonizedCode_
        elif nodeName_ == 'CountryOfManufacture':
            CountryOfManufacture_ = child_.text
            CountryOfManufacture_ = self.gds_validate_string(CountryOfManufacture_, node, 'CountryOfManufacture')
            self.CountryOfManufacture = CountryOfManufacture_
        elif nodeName_ == 'ProductCode':
            ProductCode_ = child_.text
            ProductCode_ = self.gds_validate_string(ProductCode_, node, 'ProductCode')
            self.ProductCode = ProductCode_
        elif nodeName_ == 'UnitValue':
            sval_ = child_.text
            try:
                fval_ = float(sval_)
            except (TypeError, ValueError) as exp:
                raise_parse_error(child_, 'requires float or double: %s' % exp)
            fval_ = self.gds_validate_float(fval_, node, 'UnitValue')
            self.UnitValue = fval_
        elif nodeName_ == 'Quantity':
            sval_ = child_.text
            try:
                ival_ = int(sval_)
            except (TypeError, ValueError) as exp:
                raise_parse_error(child_, 'requires integer: %s' % exp)
            ival_ = self.gds_validate_integer(ival_, node, 'Quantity')
            self.Quantity = ival_
        elif nodeName_ == 'NAFTADocumentIndicator':
            sval_ = child_.text
            if sval_ in ('true', '1'):
                ival_ = True
            elif sval_ in ('false', '0'):
                ival_ = False
            else:
                raise_parse_error(child_, 'requires boolean')
            ival_ = self.gds_validate_boolean(ival_, node, 'NAFTADocumentIndicator')
            self.NAFTADocumentIndicator = ival_
        elif nodeName_ == 'FDADocumentIndicator':
            sval_ = child_.text
            if sval_ in ('true', '1'):
                ival_ = True
            elif sval_ in ('false', '0'):
                ival_ = False
            else:
                raise_parse_error(child_, 'requires boolean')
            ival_ = self.gds_validate_boolean(ival_, node, 'FDADocumentIndicator')
            self.FDADocumentIndicator = ival_
        elif nodeName_ == 'FCCDocumentIndicator':
            sval_ = child_.text
            if sval_ in ('true', '1'):
                ival_ = True
            elif sval_ in ('false', '0'):
                ival_ = False
            else:
                raise_parse_error(child_, 'requires boolean')
            ival_ = self.gds_validate_boolean(ival_, node, 'FCCDocumentIndicator')
            self.FCCDocumentIndicator = ival_
        elif nodeName_ == 'SenderIsProducerIndicator':
            sval_ = child_.text
            if sval_ in ('true', '1'):
                ival_ = True
            elif sval_ in ('false', '0'):
                ival_ = False
            else:
                raise_parse_error(child_, 'requires boolean')
            ival_ = self.gds_validate_boolean(ival_, node, 'SenderIsProducerIndicator')
            self.SenderIsProducerIndicator = ival_
        elif nodeName_ == 'TextileIndicator':
            sval_ = child_.text
            if sval_ in ('true', '1'):
                ival_ = True
            elif sval_ in ('false', '0'):
                ival_ = False
            else:
                raise_parse_error(child_, 'requires boolean')
            ival_ = self.gds_validate_boolean(ival_, node, 'TextileIndicator')
            self.TextileIndicator = ival_
        elif nodeName_ == 'TextileManufacturer':
            TextileManufacturer_ = child_.text
            TextileManufacturer_ = self.gds_validate_string(TextileManufacturer_, node, 'TextileManufacturer')
            self.TextileManufacturer = TextileManufacturer_
# end class ContentDetail


class BuyerInformation(GeneratedsSuper):
    """BuyerInformation"""
    subclass = None
    superclass = None
    def __init__(self, Address=None, TaxNumber=None):
        self.original_tagname_ = None
        self.Address = Address
        self.TaxNumber = TaxNumber
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, BuyerInformation)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if BuyerInformation.subclass:
            return BuyerInformation.subclass(*args_, **kwargs_)
        else:
            return BuyerInformation(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Address(self): return self.Address
    def set_Address(self, Address): self.Address = Address
    def get_TaxNumber(self): return self.TaxNumber
    def set_TaxNumber(self, TaxNumber): self.TaxNumber = TaxNumber
    def hasContent_(self):
        if (
            self.Address is not None or
            self.TaxNumber is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='BuyerInformation', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('BuyerInformation')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='BuyerInformation')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='BuyerInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='BuyerInformation'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='BuyerInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Address is not None:
            self.Address.export(outfile, level, namespace_, name_='Address', pretty_print=pretty_print)
        if self.TaxNumber is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:TaxNumber>%s</tns:TaxNumber>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.TaxNumber), input_name='TaxNumber')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Address':
            obj_ = Address.factory()
            obj_.build(child_)
            self.Address = obj_
            obj_.original_tagname_ = 'Address'
        elif nodeName_ == 'TaxNumber':
            TaxNumber_ = child_.text
            TaxNumber_ = self.gds_validate_string(TaxNumber_, node, 'TaxNumber')
            self.TaxNumber = TaxNumber_
# end class BuyerInformation


class DutyInformation(GeneratedsSuper):
    """DutyInformation"""
    subclass = None
    superclass = None
    def __init__(self, BillDutiesToParty=None, BusinessRelationship=None, Currency=None):
        self.original_tagname_ = None
        self.BillDutiesToParty = BillDutiesToParty
        self.validate_BillDutiesToParty(self.BillDutiesToParty)
        self.BusinessRelationship = BusinessRelationship
        self.validate_BusinessRelationship(self.BusinessRelationship)
        self.Currency = Currency
        self.validate_DutyCurrency(self.Currency)
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, DutyInformation)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if DutyInformation.subclass:
            return DutyInformation.subclass(*args_, **kwargs_)
        else:
            return DutyInformation(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_BillDutiesToParty(self): return self.BillDutiesToParty
    def set_BillDutiesToParty(self, BillDutiesToParty): self.BillDutiesToParty = BillDutiesToParty
    def get_BusinessRelationship(self): return self.BusinessRelationship
    def set_BusinessRelationship(self, BusinessRelationship): self.BusinessRelationship = BusinessRelationship
    def get_Currency(self): return self.Currency
    def set_Currency(self, Currency): self.Currency = Currency
    def validate_BillDutiesToParty(self, value):
        # Validate type BillDutiesToParty, a restriction on xs:string.
        if value is not None and Validate_simpletypes_:
            value = str(value)
            enumerations = ['Sender', 'Receiver', 'Buyer']
            enumeration_respectee = False
            for enum in enumerations:
                if value == enum:
                    enumeration_respectee = True
                    break
            if not enumeration_respectee:
                warnings_.warn('Value "%(value)s" does not match xsd enumeration restriction on BillDutiesToParty' % {"value" : value.encode("utf-8")} )
    def validate_BusinessRelationship(self, value):
        # Validate type BusinessRelationship, a restriction on xs:string.
        if value is not None and Validate_simpletypes_:
            value = str(value)
            enumerations = ['Related', 'NotRelated']
            enumeration_respectee = False
            for enum in enumerations:
                if value == enum:
                    enumeration_respectee = True
                    break
            if not enumeration_respectee:
                warnings_.warn('Value "%(value)s" does not match xsd enumeration restriction on BusinessRelationship' % {"value" : value.encode("utf-8")} )
    def validate_DutyCurrency(self, value):
        # Validate type DutyCurrency, a restriction on xs:string.
        if value is not None and Validate_simpletypes_:
            value = str(value)
            enumerations = ['CAD', 'USD']
            enumeration_respectee = False
            for enum in enumerations:
                if value == enum:
                    enumeration_respectee = True
                    break
            if not enumeration_respectee:
                warnings_.warn('Value "%(value)s" does not match xsd enumeration restriction on DutyCurrency' % {"value" : value.encode("utf-8")} )
    def hasContent_(self):
        if (
            self.BillDutiesToParty is not None or
            self.BusinessRelationship is not None or
            self.Currency is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='DutyInformation', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('DutyInformation')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='DutyInformation')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='DutyInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='DutyInformation'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='DutyInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.BillDutiesToParty is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:BillDutiesToParty>%s</tns:BillDutiesToParty>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.BillDutiesToParty), input_name='BillDutiesToParty')), eol_))
        if self.BusinessRelationship is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:BusinessRelationship>%s</tns:BusinessRelationship>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.BusinessRelationship), input_name='BusinessRelationship')), eol_))
        if self.Currency is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Currency>%s</tns:Currency>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Currency), input_name='Currency')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'BillDutiesToParty':
            BillDutiesToParty_ = child_.text
            BillDutiesToParty_ = self.gds_validate_string(BillDutiesToParty_, node, 'BillDutiesToParty')
            self.BillDutiesToParty = BillDutiesToParty_
            # validate type BillDutiesToParty
            self.validate_BillDutiesToParty(self.BillDutiesToParty)
        elif nodeName_ == 'BusinessRelationship':
            BusinessRelationship_ = child_.text
            BusinessRelationship_ = self.gds_validate_string(BusinessRelationship_, node, 'BusinessRelationship')
            self.BusinessRelationship = BusinessRelationship_
            # validate type BusinessRelationship
            self.validate_BusinessRelationship(self.BusinessRelationship)
        elif nodeName_ == 'Currency':
            Currency_ = child_.text
            Currency_ = self.gds_validate_string(Currency_, node, 'Currency')
            self.Currency = Currency_
            # validate type DutyCurrency
            self.validate_DutyCurrency(self.Currency)
# end class DutyInformation


class ReturnShipmentInformation(GeneratedsSuper):
    """ReturnShipmentInformation"""
    subclass = None
    superclass = None
    def __init__(self, NumberOfReturnShipments=None, ReturnShipment=None):
        self.original_tagname_ = None
        self.NumberOfReturnShipments = NumberOfReturnShipments
        self.ReturnShipment = ReturnShipment
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ReturnShipmentInformation)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ReturnShipmentInformation.subclass:
            return ReturnShipmentInformation.subclass(*args_, **kwargs_)
        else:
            return ReturnShipmentInformation(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_NumberOfReturnShipments(self): return self.NumberOfReturnShipments
    def set_NumberOfReturnShipments(self, NumberOfReturnShipments): self.NumberOfReturnShipments = NumberOfReturnShipments
    def get_ReturnShipment(self): return self.ReturnShipment
    def set_ReturnShipment(self, ReturnShipment): self.ReturnShipment = ReturnShipment
    def hasContent_(self):
        if (
            self.NumberOfReturnShipments is not None or
            self.ReturnShipment is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ReturnShipmentInformation', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ReturnShipmentInformation')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ReturnShipmentInformation')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ReturnShipmentInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ReturnShipmentInformation'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ReturnShipmentInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.NumberOfReturnShipments is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:NumberOfReturnShipments>%s</tns:NumberOfReturnShipments>%s' % (self.gds_format_integer(self.NumberOfReturnShipments, input_name='NumberOfReturnShipments'), eol_))
        if self.ReturnShipment is not None:
            self.ReturnShipment.export(outfile, level, namespace_, name_='ReturnShipment', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'NumberOfReturnShipments':
            sval_ = child_.text
            try:
                ival_ = int(sval_)
            except (TypeError, ValueError) as exp:
                raise_parse_error(child_, 'requires integer: %s' % exp)
            ival_ = self.gds_validate_integer(ival_, node, 'NumberOfReturnShipments')
            self.NumberOfReturnShipments = ival_
        elif nodeName_ == 'ReturnShipment':
            obj_ = ReturnShipment.factory()
            obj_.build(child_)
            self.ReturnShipment = obj_
            obj_.original_tagname_ = 'ReturnShipment'
# end class ReturnShipmentInformation


class ReturnShipment(GeneratedsSuper):
    """Shipment"""
    subclass = None
    superclass = None
    def __init__(self, SenderInformation=None, ReceiverInformation=None, PackageInformation=None, PaymentInformation=None, PickupInformation=None, NotificationInformation=None, TrackingReferenceInformation=None, OtherInformation=None):
        self.original_tagname_ = None
        self.SenderInformation = SenderInformation
        self.ReceiverInformation = ReceiverInformation
        self.PackageInformation = PackageInformation
        self.PaymentInformation = PaymentInformation
        self.PickupInformation = PickupInformation
        self.NotificationInformation = NotificationInformation
        self.TrackingReferenceInformation = TrackingReferenceInformation
        self.OtherInformation = OtherInformation
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ReturnShipment)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ReturnShipment.subclass:
            return ReturnShipment.subclass(*args_, **kwargs_)
        else:
            return ReturnShipment(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_SenderInformation(self): return self.SenderInformation
    def set_SenderInformation(self, SenderInformation): self.SenderInformation = SenderInformation
    def get_ReceiverInformation(self): return self.ReceiverInformation
    def set_ReceiverInformation(self, ReceiverInformation): self.ReceiverInformation = ReceiverInformation
    def get_PackageInformation(self): return self.PackageInformation
    def set_PackageInformation(self, PackageInformation): self.PackageInformation = PackageInformation
    def get_PaymentInformation(self): return self.PaymentInformation
    def set_PaymentInformation(self, PaymentInformation): self.PaymentInformation = PaymentInformation
    def get_PickupInformation(self): return self.PickupInformation
    def set_PickupInformation(self, PickupInformation): self.PickupInformation = PickupInformation
    def get_NotificationInformation(self): return self.NotificationInformation
    def set_NotificationInformation(self, NotificationInformation): self.NotificationInformation = NotificationInformation
    def get_TrackingReferenceInformation(self): return self.TrackingReferenceInformation
    def set_TrackingReferenceInformation(self, TrackingReferenceInformation): self.TrackingReferenceInformation = TrackingReferenceInformation
    def get_OtherInformation(self): return self.OtherInformation
    def set_OtherInformation(self, OtherInformation): self.OtherInformation = OtherInformation
    def hasContent_(self):
        if (
            self.SenderInformation is not None or
            self.ReceiverInformation is not None or
            self.PackageInformation is not None or
            self.PaymentInformation is not None or
            self.PickupInformation is not None or
            self.NotificationInformation is not None or
            self.TrackingReferenceInformation is not None or
            self.OtherInformation is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ReturnShipment', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ReturnShipment')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ReturnShipment')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ReturnShipment', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ReturnShipment'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ReturnShipment', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.SenderInformation is not None:
            self.SenderInformation.export(outfile, level, namespace_, name_='SenderInformation', pretty_print=pretty_print)
        if self.ReceiverInformation is not None:
            self.ReceiverInformation.export(outfile, level, namespace_, name_='ReceiverInformation', pretty_print=pretty_print)
        if self.PackageInformation is not None:
            self.PackageInformation.export(outfile, level, namespace_, name_='PackageInformation', pretty_print=pretty_print)
        if self.PaymentInformation is not None:
            self.PaymentInformation.export(outfile, level, namespace_, name_='PaymentInformation', pretty_print=pretty_print)
        if self.PickupInformation is not None:
            self.PickupInformation.export(outfile, level, namespace_, name_='PickupInformation', pretty_print=pretty_print)
        if self.NotificationInformation is not None:
            self.NotificationInformation.export(outfile, level, namespace_, name_='NotificationInformation', pretty_print=pretty_print)
        if self.TrackingReferenceInformation is not None:
            self.TrackingReferenceInformation.export(outfile, level, namespace_, name_='TrackingReferenceInformation', pretty_print=pretty_print)
        if self.OtherInformation is not None:
            self.OtherInformation.export(outfile, level, namespace_, name_='OtherInformation', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'SenderInformation':
            obj_ = SenderInformation.factory()
            obj_.build(child_)
            self.SenderInformation = obj_
            obj_.original_tagname_ = 'SenderInformation'
        elif nodeName_ == 'ReceiverInformation':
            obj_ = ReceiverInformation.factory()
            obj_.build(child_)
            self.ReceiverInformation = obj_
            obj_.original_tagname_ = 'ReceiverInformation'
        elif nodeName_ == 'PackageInformation':
            obj_ = PackageInformation.factory()
            obj_.build(child_)
            self.PackageInformation = obj_
            obj_.original_tagname_ = 'PackageInformation'
        elif nodeName_ == 'PaymentInformation':
            obj_ = PaymentInformation.factory()
            obj_.build(child_)
            self.PaymentInformation = obj_
            obj_.original_tagname_ = 'PaymentInformation'
        elif nodeName_ == 'PickupInformation':
            obj_ = PickupInformation.factory()
            obj_.build(child_)
            self.PickupInformation = obj_
            obj_.original_tagname_ = 'PickupInformation'
        elif nodeName_ == 'NotificationInformation':
            obj_ = NotificationInformation.factory()
            obj_.build(child_)
            self.NotificationInformation = obj_
            obj_.original_tagname_ = 'NotificationInformation'
        elif nodeName_ == 'TrackingReferenceInformation':
            obj_ = TrackingReferenceInformation.factory()
            obj_.build(child_)
            self.TrackingReferenceInformation = obj_
            obj_.original_tagname_ = 'TrackingReferenceInformation'
        elif nodeName_ == 'OtherInformation':
            obj_ = OtherInformation.factory()
            obj_.build(child_)
            self.OtherInformation = obj_
            obj_.original_tagname_ = 'OtherInformation'
# end class ReturnShipment


class PaymentInformation(GeneratedsSuper):
    """PaymentInformation"""
    subclass = None
    superclass = None
    def __init__(self, PaymentType=None, RegisteredAccountNumber=None, BillingAccountNumber=None, CreditCardInformation=None):
        self.original_tagname_ = None
        self.PaymentType = PaymentType
        self.validate_PaymentType(self.PaymentType)
        self.RegisteredAccountNumber = RegisteredAccountNumber
        self.BillingAccountNumber = BillingAccountNumber
        self.CreditCardInformation = CreditCardInformation
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, PaymentInformation)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if PaymentInformation.subclass:
            return PaymentInformation.subclass(*args_, **kwargs_)
        else:
            return PaymentInformation(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_PaymentType(self): return self.PaymentType
    def set_PaymentType(self, PaymentType): self.PaymentType = PaymentType
    def get_RegisteredAccountNumber(self): return self.RegisteredAccountNumber
    def set_RegisteredAccountNumber(self, RegisteredAccountNumber): self.RegisteredAccountNumber = RegisteredAccountNumber
    def get_BillingAccountNumber(self): return self.BillingAccountNumber
    def set_BillingAccountNumber(self, BillingAccountNumber): self.BillingAccountNumber = BillingAccountNumber
    def get_CreditCardInformation(self): return self.CreditCardInformation
    def set_CreditCardInformation(self, CreditCardInformation): self.CreditCardInformation = CreditCardInformation
    def validate_PaymentType(self, value):
        # Validate type PaymentType, a restriction on xs:string.
        if value is not None and Validate_simpletypes_:
            value = str(value)
            enumerations = ['Sender', 'Receiver', 'ThirdParty', 'CreditCard']
            enumeration_respectee = False
            for enum in enumerations:
                if value == enum:
                    enumeration_respectee = True
                    break
            if not enumeration_respectee:
                warnings_.warn('Value "%(value)s" does not match xsd enumeration restriction on PaymentType' % {"value" : value.encode("utf-8")} )
    def hasContent_(self):
        if (
            self.PaymentType is not None or
            self.RegisteredAccountNumber is not None or
            self.BillingAccountNumber is not None or
            self.CreditCardInformation is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='PaymentInformation', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('PaymentInformation')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='PaymentInformation')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='PaymentInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='PaymentInformation'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='PaymentInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.PaymentType is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:PaymentType>%s</tns:PaymentType>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.PaymentType), input_name='PaymentType')), eol_))
        if self.RegisteredAccountNumber is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:RegisteredAccountNumber>%s</tns:RegisteredAccountNumber>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.RegisteredAccountNumber), input_name='RegisteredAccountNumber')), eol_))
        if self.BillingAccountNumber is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:BillingAccountNumber>%s</tns:BillingAccountNumber>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.BillingAccountNumber), input_name='BillingAccountNumber')), eol_))
        if self.CreditCardInformation is not None:
            self.CreditCardInformation.export(outfile, level, namespace_, name_='CreditCardInformation', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'PaymentType':
            PaymentType_ = child_.text
            PaymentType_ = self.gds_validate_string(PaymentType_, node, 'PaymentType')
            self.PaymentType = PaymentType_
            # validate type PaymentType
            self.validate_PaymentType(self.PaymentType)
        elif nodeName_ == 'RegisteredAccountNumber':
            RegisteredAccountNumber_ = child_.text
            RegisteredAccountNumber_ = self.gds_validate_string(RegisteredAccountNumber_, node, 'RegisteredAccountNumber')
            self.RegisteredAccountNumber = RegisteredAccountNumber_
        elif nodeName_ == 'BillingAccountNumber':
            BillingAccountNumber_ = child_.text
            BillingAccountNumber_ = self.gds_validate_string(BillingAccountNumber_, node, 'BillingAccountNumber')
            self.BillingAccountNumber = BillingAccountNumber_
        elif nodeName_ == 'CreditCardInformation':
            obj_ = CreditCardInformation.factory()
            obj_.build(child_)
            self.CreditCardInformation = obj_
            obj_.original_tagname_ = 'CreditCardInformation'
# end class PaymentInformation


class CreditCardInformation(GeneratedsSuper):
    """CreditCardInformation"""
    subclass = None
    superclass = None
    def __init__(self, Type=None, Number=None, Name=None, ExpiryMonth=None, ExpiryYear=None, CVV=None, BillingPostalCode=None):
        self.original_tagname_ = None
        self.Type = Type
        self.validate_CreditCardType(self.Type)
        self.Number = Number
        self.Name = Name
        self.ExpiryMonth = ExpiryMonth
        self.ExpiryYear = ExpiryYear
        self.CVV = CVV
        self.BillingPostalCode = BillingPostalCode
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, CreditCardInformation)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if CreditCardInformation.subclass:
            return CreditCardInformation.subclass(*args_, **kwargs_)
        else:
            return CreditCardInformation(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Type(self): return self.Type
    def set_Type(self, Type): self.Type = Type
    def get_Number(self): return self.Number
    def set_Number(self, Number): self.Number = Number
    def get_Name(self): return self.Name
    def set_Name(self, Name): self.Name = Name
    def get_ExpiryMonth(self): return self.ExpiryMonth
    def set_ExpiryMonth(self, ExpiryMonth): self.ExpiryMonth = ExpiryMonth
    def get_ExpiryYear(self): return self.ExpiryYear
    def set_ExpiryYear(self, ExpiryYear): self.ExpiryYear = ExpiryYear
    def get_CVV(self): return self.CVV
    def set_CVV(self, CVV): self.CVV = CVV
    def get_BillingPostalCode(self): return self.BillingPostalCode
    def set_BillingPostalCode(self, BillingPostalCode): self.BillingPostalCode = BillingPostalCode
    def validate_CreditCardType(self, value):
        # Validate type CreditCardType, a restriction on xs:string.
        if value is not None and Validate_simpletypes_:
            value = str(value)
            enumerations = ['Visa', 'MasterCard', 'AmericanExpress']
            enumeration_respectee = False
            for enum in enumerations:
                if value == enum:
                    enumeration_respectee = True
                    break
            if not enumeration_respectee:
                warnings_.warn('Value "%(value)s" does not match xsd enumeration restriction on CreditCardType' % {"value" : value.encode("utf-8")} )
    def hasContent_(self):
        if (
            self.Type is not None or
            self.Number is not None or
            self.Name is not None or
            self.ExpiryMonth is not None or
            self.ExpiryYear is not None or
            self.CVV is not None or
            self.BillingPostalCode is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='CreditCardInformation', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('CreditCardInformation')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='CreditCardInformation')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='CreditCardInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='CreditCardInformation'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='CreditCardInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Type is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Type>%s</tns:Type>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Type), input_name='Type')), eol_))
        if self.Number is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Number>%s</tns:Number>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Number), input_name='Number')), eol_))
        if self.Name is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Name>%s</tns:Name>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Name), input_name='Name')), eol_))
        if self.ExpiryMonth is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:ExpiryMonth>%s</tns:ExpiryMonth>%s' % (self.gds_format_integer(self.ExpiryMonth, input_name='ExpiryMonth'), eol_))
        if self.ExpiryYear is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:ExpiryYear>%s</tns:ExpiryYear>%s' % (self.gds_format_integer(self.ExpiryYear, input_name='ExpiryYear'), eol_))
        if self.CVV is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:CVV>%s</tns:CVV>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.CVV), input_name='CVV')), eol_))
        if self.BillingPostalCode is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:BillingPostalCode>%s</tns:BillingPostalCode>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.BillingPostalCode), input_name='BillingPostalCode')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Type':
            Type_ = child_.text
            Type_ = self.gds_validate_string(Type_, node, 'Type')
            self.Type = Type_
            # validate type CreditCardType
            self.validate_CreditCardType(self.Type)
        elif nodeName_ == 'Number':
            Number_ = child_.text
            Number_ = self.gds_validate_string(Number_, node, 'Number')
            self.Number = Number_
        elif nodeName_ == 'Name':
            Name_ = child_.text
            Name_ = self.gds_validate_string(Name_, node, 'Name')
            self.Name = Name_
        elif nodeName_ == 'ExpiryMonth':
            sval_ = child_.text
            try:
                ival_ = int(sval_)
            except (TypeError, ValueError) as exp:
                raise_parse_error(child_, 'requires integer: %s' % exp)
            ival_ = self.gds_validate_integer(ival_, node, 'ExpiryMonth')
            self.ExpiryMonth = ival_
        elif nodeName_ == 'ExpiryYear':
            sval_ = child_.text
            try:
                ival_ = int(sval_)
            except (TypeError, ValueError) as exp:
                raise_parse_error(child_, 'requires integer: %s' % exp)
            ival_ = self.gds_validate_integer(ival_, node, 'ExpiryYear')
            self.ExpiryYear = ival_
        elif nodeName_ == 'CVV':
            CVV_ = child_.text
            CVV_ = self.gds_validate_string(CVV_, node, 'CVV')
            self.CVV = CVV_
        elif nodeName_ == 'BillingPostalCode':
            BillingPostalCode_ = child_.text
            BillingPostalCode_ = self.gds_validate_string(BillingPostalCode_, node, 'BillingPostalCode')
            self.BillingPostalCode = BillingPostalCode_
# end class CreditCardInformation


class PickupInformation(GeneratedsSuper):
    """PickupInformation"""
    subclass = None
    superclass = None
    def __init__(self, PickupType=None):
        self.original_tagname_ = None
        self.PickupType = PickupType
        self.validate_PickupType(self.PickupType)
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, PickupInformation)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if PickupInformation.subclass:
            return PickupInformation.subclass(*args_, **kwargs_)
        else:
            return PickupInformation(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_PickupType(self): return self.PickupType
    def set_PickupType(self, PickupType): self.PickupType = PickupType
    def validate_PickupType(self, value):
        # Validate type PickupType, a restriction on xs:string.
        if value is not None and Validate_simpletypes_:
            value = str(value)
            enumerations = ['DropOff', 'PreScheduled']
            enumeration_respectee = False
            for enum in enumerations:
                if value == enum:
                    enumeration_respectee = True
                    break
            if not enumeration_respectee:
                warnings_.warn('Value "%(value)s" does not match xsd enumeration restriction on PickupType' % {"value" : value.encode("utf-8")} )
    def hasContent_(self):
        if (
            self.PickupType is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='PickupInformation', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('PickupInformation')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='PickupInformation')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='PickupInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='PickupInformation'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='PickupInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.PickupType is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:PickupType>%s</tns:PickupType>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.PickupType), input_name='PickupType')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'PickupType':
            PickupType_ = child_.text
            PickupType_ = self.gds_validate_string(PickupType_, node, 'PickupType')
            self.PickupType = PickupType_
            # validate type PickupType
            self.validate_PickupType(self.PickupType)
# end class PickupInformation


class NotificationInformation(GeneratedsSuper):
    """NotificationInformation"""
    subclass = None
    superclass = None
    def __init__(self, ConfirmationEmailAddress=None, AdvancedShippingNotificationEmailAddress1=None, AdvancedShippingNotificationEmailAddress2=None, AdvancedShippingNotificationMessage=None):
        self.original_tagname_ = None
        self.ConfirmationEmailAddress = ConfirmationEmailAddress
        self.AdvancedShippingNotificationEmailAddress1 = AdvancedShippingNotificationEmailAddress1
        self.AdvancedShippingNotificationEmailAddress2 = AdvancedShippingNotificationEmailAddress2
        self.AdvancedShippingNotificationMessage = AdvancedShippingNotificationMessage
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, NotificationInformation)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if NotificationInformation.subclass:
            return NotificationInformation.subclass(*args_, **kwargs_)
        else:
            return NotificationInformation(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_ConfirmationEmailAddress(self): return self.ConfirmationEmailAddress
    def set_ConfirmationEmailAddress(self, ConfirmationEmailAddress): self.ConfirmationEmailAddress = ConfirmationEmailAddress
    def get_AdvancedShippingNotificationEmailAddress1(self): return self.AdvancedShippingNotificationEmailAddress1
    def set_AdvancedShippingNotificationEmailAddress1(self, AdvancedShippingNotificationEmailAddress1): self.AdvancedShippingNotificationEmailAddress1 = AdvancedShippingNotificationEmailAddress1
    def get_AdvancedShippingNotificationEmailAddress2(self): return self.AdvancedShippingNotificationEmailAddress2
    def set_AdvancedShippingNotificationEmailAddress2(self, AdvancedShippingNotificationEmailAddress2): self.AdvancedShippingNotificationEmailAddress2 = AdvancedShippingNotificationEmailAddress2
    def get_AdvancedShippingNotificationMessage(self): return self.AdvancedShippingNotificationMessage
    def set_AdvancedShippingNotificationMessage(self, AdvancedShippingNotificationMessage): self.AdvancedShippingNotificationMessage = AdvancedShippingNotificationMessage
    def hasContent_(self):
        if (
            self.ConfirmationEmailAddress is not None or
            self.AdvancedShippingNotificationEmailAddress1 is not None or
            self.AdvancedShippingNotificationEmailAddress2 is not None or
            self.AdvancedShippingNotificationMessage is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='NotificationInformation', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('NotificationInformation')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='NotificationInformation')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='NotificationInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='NotificationInformation'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='NotificationInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.ConfirmationEmailAddress is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:ConfirmationEmailAddress>%s</tns:ConfirmationEmailAddress>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.ConfirmationEmailAddress), input_name='ConfirmationEmailAddress')), eol_))
        if self.AdvancedShippingNotificationEmailAddress1 is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:AdvancedShippingNotificationEmailAddress1>%s</tns:AdvancedShippingNotificationEmailAddress1>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.AdvancedShippingNotificationEmailAddress1), input_name='AdvancedShippingNotificationEmailAddress1')), eol_))
        if self.AdvancedShippingNotificationEmailAddress2 is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:AdvancedShippingNotificationEmailAddress2>%s</tns:AdvancedShippingNotificationEmailAddress2>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.AdvancedShippingNotificationEmailAddress2), input_name='AdvancedShippingNotificationEmailAddress2')), eol_))
        if self.AdvancedShippingNotificationMessage is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:AdvancedShippingNotificationMessage>%s</tns:AdvancedShippingNotificationMessage>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.AdvancedShippingNotificationMessage), input_name='AdvancedShippingNotificationMessage')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'ConfirmationEmailAddress':
            ConfirmationEmailAddress_ = child_.text
            ConfirmationEmailAddress_ = self.gds_validate_string(ConfirmationEmailAddress_, node, 'ConfirmationEmailAddress')
            self.ConfirmationEmailAddress = ConfirmationEmailAddress_
        elif nodeName_ == 'AdvancedShippingNotificationEmailAddress1':
            AdvancedShippingNotificationEmailAddress1_ = child_.text
            AdvancedShippingNotificationEmailAddress1_ = self.gds_validate_string(AdvancedShippingNotificationEmailAddress1_, node, 'AdvancedShippingNotificationEmailAddress1')
            self.AdvancedShippingNotificationEmailAddress1 = AdvancedShippingNotificationEmailAddress1_
        elif nodeName_ == 'AdvancedShippingNotificationEmailAddress2':
            AdvancedShippingNotificationEmailAddress2_ = child_.text
            AdvancedShippingNotificationEmailAddress2_ = self.gds_validate_string(AdvancedShippingNotificationEmailAddress2_, node, 'AdvancedShippingNotificationEmailAddress2')
            self.AdvancedShippingNotificationEmailAddress2 = AdvancedShippingNotificationEmailAddress2_
        elif nodeName_ == 'AdvancedShippingNotificationMessage':
            AdvancedShippingNotificationMessage_ = child_.text
            AdvancedShippingNotificationMessage_ = self.gds_validate_string(AdvancedShippingNotificationMessage_, node, 'AdvancedShippingNotificationMessage')
            self.AdvancedShippingNotificationMessage = AdvancedShippingNotificationMessage_
# end class NotificationInformation


class TrackingReferenceInformation(GeneratedsSuper):
    """TrackingReferenceInformation"""
    subclass = None
    superclass = None
    def __init__(self, Reference1=None, Reference2=None, Reference3=None, Reference4=None):
        self.original_tagname_ = None
        self.Reference1 = Reference1
        self.Reference2 = Reference2
        self.Reference3 = Reference3
        self.Reference4 = Reference4
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, TrackingReferenceInformation)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if TrackingReferenceInformation.subclass:
            return TrackingReferenceInformation.subclass(*args_, **kwargs_)
        else:
            return TrackingReferenceInformation(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Reference1(self): return self.Reference1
    def set_Reference1(self, Reference1): self.Reference1 = Reference1
    def get_Reference2(self): return self.Reference2
    def set_Reference2(self, Reference2): self.Reference2 = Reference2
    def get_Reference3(self): return self.Reference3
    def set_Reference3(self, Reference3): self.Reference3 = Reference3
    def get_Reference4(self): return self.Reference4
    def set_Reference4(self, Reference4): self.Reference4 = Reference4
    def hasContent_(self):
        if (
            self.Reference1 is not None or
            self.Reference2 is not None or
            self.Reference3 is not None or
            self.Reference4 is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='TrackingReferenceInformation', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('TrackingReferenceInformation')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='TrackingReferenceInformation')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='TrackingReferenceInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='TrackingReferenceInformation'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='TrackingReferenceInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Reference1 is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Reference1>%s</tns:Reference1>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Reference1), input_name='Reference1')), eol_))
        if self.Reference2 is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Reference2>%s</tns:Reference2>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Reference2), input_name='Reference2')), eol_))
        if self.Reference3 is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Reference3>%s</tns:Reference3>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Reference3), input_name='Reference3')), eol_))
        if self.Reference4 is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Reference4>%s</tns:Reference4>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Reference4), input_name='Reference4')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Reference1':
            Reference1_ = child_.text
            Reference1_ = self.gds_validate_string(Reference1_, node, 'Reference1')
            self.Reference1 = Reference1_
        elif nodeName_ == 'Reference2':
            Reference2_ = child_.text
            Reference2_ = self.gds_validate_string(Reference2_, node, 'Reference2')
            self.Reference2 = Reference2_
        elif nodeName_ == 'Reference3':
            Reference3_ = child_.text
            Reference3_ = self.gds_validate_string(Reference3_, node, 'Reference3')
            self.Reference3 = Reference3_
        elif nodeName_ == 'Reference4':
            Reference4_ = child_.text
            Reference4_ = self.gds_validate_string(Reference4_, node, 'Reference4')
            self.Reference4 = Reference4_
# end class TrackingReferenceInformation


class OtherInformation(GeneratedsSuper):
    """OtherInformation"""
    subclass = None
    superclass = None
    def __init__(self, CostCentre=None, SpecialInstructions=None):
        self.original_tagname_ = None
        self.CostCentre = CostCentre
        self.SpecialInstructions = SpecialInstructions
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, OtherInformation)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if OtherInformation.subclass:
            return OtherInformation.subclass(*args_, **kwargs_)
        else:
            return OtherInformation(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_CostCentre(self): return self.CostCentre
    def set_CostCentre(self, CostCentre): self.CostCentre = CostCentre
    def get_SpecialInstructions(self): return self.SpecialInstructions
    def set_SpecialInstructions(self, SpecialInstructions): self.SpecialInstructions = SpecialInstructions
    def hasContent_(self):
        if (
            self.CostCentre is not None or
            self.SpecialInstructions is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='OtherInformation', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('OtherInformation')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='OtherInformation')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='OtherInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='OtherInformation'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='OtherInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.CostCentre is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:CostCentre>%s</tns:CostCentre>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.CostCentre), input_name='CostCentre')), eol_))
        if self.SpecialInstructions is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:SpecialInstructions>%s</tns:SpecialInstructions>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.SpecialInstructions), input_name='SpecialInstructions')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'CostCentre':
            CostCentre_ = child_.text
            CostCentre_ = self.gds_validate_string(CostCentre_, node, 'CostCentre')
            self.CostCentre = CostCentre_
        elif nodeName_ == 'SpecialInstructions':
            SpecialInstructions_ = child_.text
            SpecialInstructions_ = self.gds_validate_string(SpecialInstructions_, node, 'SpecialInstructions')
            self.SpecialInstructions = SpecialInstructions_
# end class OtherInformation


class ProactiveNotification(GeneratedsSuper):
    """ProactiveNotification"""
    subclass = None
    superclass = None
    def __init__(self, RequestorName=None, RequestorEmail=None, Subscriptions=None):
        self.original_tagname_ = None
        self.RequestorName = RequestorName
        self.RequestorEmail = RequestorEmail
        self.Subscriptions = Subscriptions
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ProactiveNotification)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ProactiveNotification.subclass:
            return ProactiveNotification.subclass(*args_, **kwargs_)
        else:
            return ProactiveNotification(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_RequestorName(self): return self.RequestorName
    def set_RequestorName(self, RequestorName): self.RequestorName = RequestorName
    def get_RequestorEmail(self): return self.RequestorEmail
    def set_RequestorEmail(self, RequestorEmail): self.RequestorEmail = RequestorEmail
    def get_Subscriptions(self): return self.Subscriptions
    def set_Subscriptions(self, Subscriptions): self.Subscriptions = Subscriptions
    def hasContent_(self):
        if (
            self.RequestorName is not None or
            self.RequestorEmail is not None or
            self.Subscriptions is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ProactiveNotification', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ProactiveNotification')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ProactiveNotification')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ProactiveNotification', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ProactiveNotification'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ProactiveNotification', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.RequestorName is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:RequestorName>%s</tns:RequestorName>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.RequestorName), input_name='RequestorName')), eol_))
        if self.RequestorEmail is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:RequestorEmail>%s</tns:RequestorEmail>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.RequestorEmail), input_name='RequestorEmail')), eol_))
        if self.Subscriptions is not None:
            self.Subscriptions.export(outfile, level, namespace_, name_='Subscriptions', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'RequestorName':
            RequestorName_ = child_.text
            RequestorName_ = self.gds_validate_string(RequestorName_, node, 'RequestorName')
            self.RequestorName = RequestorName_
        elif nodeName_ == 'RequestorEmail':
            RequestorEmail_ = child_.text
            RequestorEmail_ = self.gds_validate_string(RequestorEmail_, node, 'RequestorEmail')
            self.RequestorEmail = RequestorEmail_
        elif nodeName_ == 'Subscriptions':
            obj_ = ArrayOfSubscription.factory()
            obj_.build(child_)
            self.Subscriptions = obj_
            obj_.original_tagname_ = 'Subscriptions'
# end class ProactiveNotification


class ArrayOfSubscription(GeneratedsSuper):
    subclass = None
    superclass = None
    def __init__(self, Subscription=None):
        self.original_tagname_ = None
        if Subscription is None:
            self.Subscription = []
        else:
            self.Subscription = Subscription
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ArrayOfSubscription)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ArrayOfSubscription.subclass:
            return ArrayOfSubscription.subclass(*args_, **kwargs_)
        else:
            return ArrayOfSubscription(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Subscription(self): return self.Subscription
    def set_Subscription(self, Subscription): self.Subscription = Subscription
    def add_Subscription(self, value): self.Subscription.append(value)
    def insert_Subscription_at(self, index, value): self.Subscription.insert(index, value)
    def replace_Subscription_at(self, index, value): self.Subscription[index] = value
    def hasContent_(self):
        if (
            self.Subscription
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ArrayOfSubscription', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ArrayOfSubscription')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ArrayOfSubscription')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ArrayOfSubscription', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ArrayOfSubscription'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ArrayOfSubscription', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        for Subscription_ in self.Subscription:
            Subscription_.export(outfile, level, namespace_, name_='Subscription', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Subscription':
            obj_ = Subscription.factory()
            obj_.build(child_)
            self.Subscription.append(obj_)
            obj_.original_tagname_ = 'Subscription'
# end class ArrayOfSubscription


class Subscription(GeneratedsSuper):
    """Proactive Notification"""
    subclass = None
    superclass = None
    def __init__(self, Name=None, Email=None, NotifyWhenExceptionOccurs=None, NotifyWhenDeliveryOccurs=None):
        self.original_tagname_ = None
        self.Name = Name
        self.Email = Email
        self.NotifyWhenExceptionOccurs = NotifyWhenExceptionOccurs
        self.NotifyWhenDeliveryOccurs = NotifyWhenDeliveryOccurs
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, Subscription)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if Subscription.subclass:
            return Subscription.subclass(*args_, **kwargs_)
        else:
            return Subscription(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Name(self): return self.Name
    def set_Name(self, Name): self.Name = Name
    def get_Email(self): return self.Email
    def set_Email(self, Email): self.Email = Email
    def get_NotifyWhenExceptionOccurs(self): return self.NotifyWhenExceptionOccurs
    def set_NotifyWhenExceptionOccurs(self, NotifyWhenExceptionOccurs): self.NotifyWhenExceptionOccurs = NotifyWhenExceptionOccurs
    def get_NotifyWhenDeliveryOccurs(self): return self.NotifyWhenDeliveryOccurs
    def set_NotifyWhenDeliveryOccurs(self, NotifyWhenDeliveryOccurs): self.NotifyWhenDeliveryOccurs = NotifyWhenDeliveryOccurs
    def hasContent_(self):
        if (
            self.Name is not None or
            self.Email is not None or
            self.NotifyWhenExceptionOccurs is not None or
            self.NotifyWhenDeliveryOccurs is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='Subscription', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('Subscription')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='Subscription')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='Subscription', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='Subscription'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='Subscription', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Name is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Name>%s</tns:Name>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Name), input_name='Name')), eol_))
        if self.Email is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Email>%s</tns:Email>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Email), input_name='Email')), eol_))
        if self.NotifyWhenExceptionOccurs is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:NotifyWhenExceptionOccurs>%s</tns:NotifyWhenExceptionOccurs>%s' % (self.gds_format_boolean(self.NotifyWhenExceptionOccurs, input_name='NotifyWhenExceptionOccurs'), eol_))
        if self.NotifyWhenDeliveryOccurs is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:NotifyWhenDeliveryOccurs>%s</tns:NotifyWhenDeliveryOccurs>%s' % (self.gds_format_boolean(self.NotifyWhenDeliveryOccurs, input_name='NotifyWhenDeliveryOccurs'), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Name':
            Name_ = child_.text
            Name_ = self.gds_validate_string(Name_, node, 'Name')
            self.Name = Name_
        elif nodeName_ == 'Email':
            Email_ = child_.text
            Email_ = self.gds_validate_string(Email_, node, 'Email')
            self.Email = Email_
        elif nodeName_ == 'NotifyWhenExceptionOccurs':
            sval_ = child_.text
            if sval_ in ('true', '1'):
                ival_ = True
            elif sval_ in ('false', '0'):
                ival_ = False
            else:
                raise_parse_error(child_, 'requires boolean')
            ival_ = self.gds_validate_boolean(ival_, node, 'NotifyWhenExceptionOccurs')
            self.NotifyWhenExceptionOccurs = ival_
        elif nodeName_ == 'NotifyWhenDeliveryOccurs':
            sval_ = child_.text
            if sval_ in ('true', '1'):
                ival_ = True
            elif sval_ in ('false', '0'):
                ival_ = False
            else:
                raise_parse_error(child_, 'requires boolean')
            ival_ = self.gds_validate_boolean(ival_, node, 'NotifyWhenDeliveryOccurs')
            self.NotifyWhenDeliveryOccurs = ival_
# end class Subscription


class RequestContext(GeneratedsSuper):
    """RequestContext"""
    subclass = None
    superclass = None
    def __init__(self, Version=None, Language=None, GroupID=None, RequestReference=None, UserToken=None):
        self.original_tagname_ = None
        self.Version = Version
        self.Language = Language
        self.validate_Language(self.Language)
        self.GroupID = GroupID
        self.RequestReference = RequestReference
        self.UserToken = UserToken
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, RequestContext)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if RequestContext.subclass:
            return RequestContext.subclass(*args_, **kwargs_)
        else:
            return RequestContext(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Version(self): return self.Version
    def set_Version(self, Version): self.Version = Version
    def get_Language(self): return self.Language
    def set_Language(self, Language): self.Language = Language
    def get_GroupID(self): return self.GroupID
    def set_GroupID(self, GroupID): self.GroupID = GroupID
    def get_RequestReference(self): return self.RequestReference
    def set_RequestReference(self, RequestReference): self.RequestReference = RequestReference
    def get_UserToken(self): return self.UserToken
    def set_UserToken(self, UserToken): self.UserToken = UserToken
    def validate_Language(self, value):
        # Validate type Language, a restriction on xs:string.
        if value is not None and Validate_simpletypes_:
            value = str(value)
            enumerations = ['en', 'fr']
            enumeration_respectee = False
            for enum in enumerations:
                if value == enum:
                    enumeration_respectee = True
                    break
            if not enumeration_respectee:
                warnings_.warn('Value "%(value)s" does not match xsd enumeration restriction on Language' % {"value" : value.encode("utf-8")} )
    def hasContent_(self):
        if (
            self.Version is not None or
            self.Language is not None or
            self.GroupID is not None or
            self.RequestReference is not None or
            self.UserToken is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='RequestContext', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('RequestContext')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='RequestContext')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='RequestContext', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='RequestContext'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='RequestContext', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Version is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Version>%s</tns:Version>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Version), input_name='Version')), eol_))
        if self.Language is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Language>%s</tns:Language>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Language), input_name='Language')), eol_))
        if self.GroupID is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:GroupID>%s</tns:GroupID>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.GroupID), input_name='GroupID')), eol_))
        if self.RequestReference is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:RequestReference>%s</tns:RequestReference>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.RequestReference), input_name='RequestReference')), eol_))
        if self.UserToken is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:UserToken>%s</tns:UserToken>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.UserToken), input_name='UserToken')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Version':
            Version_ = child_.text
            Version_ = self.gds_validate_string(Version_, node, 'Version')
            self.Version = Version_
        elif nodeName_ == 'Language':
            Language_ = child_.text
            Language_ = self.gds_validate_string(Language_, node, 'Language')
            self.Language = Language_
            # validate type Language
            self.validate_Language(self.Language)
        elif nodeName_ == 'GroupID':
            GroupID_ = child_.text
            GroupID_ = self.gds_validate_string(GroupID_, node, 'GroupID')
            self.GroupID = GroupID_
        elif nodeName_ == 'RequestReference':
            RequestReference_ = child_.text
            RequestReference_ = self.gds_validate_string(RequestReference_, node, 'RequestReference')
            self.RequestReference = RequestReference_
        elif nodeName_ == 'UserToken':
            UserToken_ = child_.text
            UserToken_ = self.gds_validate_string(UserToken_, node, 'UserToken')
            self.UserToken = UserToken_
# end class RequestContext


class ResponseContainer(GeneratedsSuper):
    subclass = None
    superclass = None
    def __init__(self, ResponseInformation=None):
        self.original_tagname_ = None
        self.ResponseInformation = ResponseInformation
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ResponseContainer)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ResponseContainer.subclass:
            return ResponseContainer.subclass(*args_, **kwargs_)
        else:
            return ResponseContainer(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_ResponseInformation(self): return self.ResponseInformation
    def set_ResponseInformation(self, ResponseInformation): self.ResponseInformation = ResponseInformation
    def hasContent_(self):
        if (
            self.ResponseInformation is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ResponseContainer', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ResponseContainer')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ResponseContainer')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ResponseContainer', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ResponseContainer'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ResponseContainer', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.ResponseInformation is not None:
            self.ResponseInformation.export(outfile, level, namespace_, name_='ResponseInformation', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'ResponseInformation':
            obj_ = ResponseInformation.factory()
            obj_.build(child_)
            self.ResponseInformation = obj_
            obj_.original_tagname_ = 'ResponseInformation'
# end class ResponseContainer


class ResponseInformation(GeneratedsSuper):
    """ResponseInformation"""
    subclass = None
    superclass = None
    def __init__(self, Errors=None, InformationalMessages=None):
        self.original_tagname_ = None
        self.Errors = Errors
        self.InformationalMessages = InformationalMessages
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ResponseInformation)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ResponseInformation.subclass:
            return ResponseInformation.subclass(*args_, **kwargs_)
        else:
            return ResponseInformation(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Errors(self): return self.Errors
    def set_Errors(self, Errors): self.Errors = Errors
    def get_InformationalMessages(self): return self.InformationalMessages
    def set_InformationalMessages(self, InformationalMessages): self.InformationalMessages = InformationalMessages
    def hasContent_(self):
        if (
            self.Errors is not None or
            self.InformationalMessages is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ResponseInformation', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ResponseInformation')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ResponseInformation')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ResponseInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ResponseInformation'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ResponseInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Errors is not None:
            self.Errors.export(outfile, level, namespace_, name_='Errors', pretty_print=pretty_print)
        if self.InformationalMessages is not None:
            self.InformationalMessages.export(outfile, level, namespace_, name_='InformationalMessages', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Errors':
            obj_ = ArrayOfError.factory()
            obj_.build(child_)
            self.Errors = obj_
            obj_.original_tagname_ = 'Errors'
        elif nodeName_ == 'InformationalMessages':
            obj_ = ArrayOfInformationalMessage.factory()
            obj_.build(child_)
            self.InformationalMessages = obj_
            obj_.original_tagname_ = 'InformationalMessages'
# end class ResponseInformation


class ArrayOfError(GeneratedsSuper):
    subclass = None
    superclass = None
    def __init__(self, Error=None):
        self.original_tagname_ = None
        if Error is None:
            self.Error = []
        else:
            self.Error = Error
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ArrayOfError)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ArrayOfError.subclass:
            return ArrayOfError.subclass(*args_, **kwargs_)
        else:
            return ArrayOfError(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Error(self): return self.Error
    def set_Error(self, Error): self.Error = Error
    def add_Error(self, value): self.Error.append(value)
    def insert_Error_at(self, index, value): self.Error.insert(index, value)
    def replace_Error_at(self, index, value): self.Error[index] = value
    def hasContent_(self):
        if (
            self.Error
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ArrayOfError', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ArrayOfError')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ArrayOfError')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ArrayOfError', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ArrayOfError'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ArrayOfError', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        for Error_ in self.Error:
            Error_.export(outfile, level, namespace_, name_='Error', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Error':
            obj_ = Error.factory()
            obj_.build(child_)
            self.Error.append(obj_)
            obj_.original_tagname_ = 'Error'
# end class ArrayOfError


class Error(GeneratedsSuper):
    """Error"""
    subclass = None
    superclass = None
    def __init__(self, Code=None, Description=None, AdditionalInformation=None):
        self.original_tagname_ = None
        self.Code = Code
        self.Description = Description
        self.AdditionalInformation = AdditionalInformation
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, Error)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if Error.subclass:
            return Error.subclass(*args_, **kwargs_)
        else:
            return Error(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Code(self): return self.Code
    def set_Code(self, Code): self.Code = Code
    def get_Description(self): return self.Description
    def set_Description(self, Description): self.Description = Description
    def get_AdditionalInformation(self): return self.AdditionalInformation
    def set_AdditionalInformation(self, AdditionalInformation): self.AdditionalInformation = AdditionalInformation
    def hasContent_(self):
        if (
            self.Code is not None or
            self.Description is not None or
            self.AdditionalInformation is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='Error', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('Error')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='Error')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='Error', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='Error'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='Error', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Code is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Code>%s</tns:Code>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Code), input_name='Code')), eol_))
        if self.Description is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Description>%s</tns:Description>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Description), input_name='Description')), eol_))
        if self.AdditionalInformation is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:AdditionalInformation>%s</tns:AdditionalInformation>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.AdditionalInformation), input_name='AdditionalInformation')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Code':
            Code_ = child_.text
            Code_ = self.gds_validate_string(Code_, node, 'Code')
            self.Code = Code_
        elif nodeName_ == 'Description':
            Description_ = child_.text
            Description_ = self.gds_validate_string(Description_, node, 'Description')
            self.Description = Description_
        elif nodeName_ == 'AdditionalInformation':
            AdditionalInformation_ = child_.text
            AdditionalInformation_ = self.gds_validate_string(AdditionalInformation_, node, 'AdditionalInformation')
            self.AdditionalInformation = AdditionalInformation_
# end class Error


class ArrayOfInformationalMessage(GeneratedsSuper):
    subclass = None
    superclass = None
    def __init__(self, InformationalMessage=None):
        self.original_tagname_ = None
        if InformationalMessage is None:
            self.InformationalMessage = []
        else:
            self.InformationalMessage = InformationalMessage
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ArrayOfInformationalMessage)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ArrayOfInformationalMessage.subclass:
            return ArrayOfInformationalMessage.subclass(*args_, **kwargs_)
        else:
            return ArrayOfInformationalMessage(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_InformationalMessage(self): return self.InformationalMessage
    def set_InformationalMessage(self, InformationalMessage): self.InformationalMessage = InformationalMessage
    def add_InformationalMessage(self, value): self.InformationalMessage.append(value)
    def insert_InformationalMessage_at(self, index, value): self.InformationalMessage.insert(index, value)
    def replace_InformationalMessage_at(self, index, value): self.InformationalMessage[index] = value
    def hasContent_(self):
        if (
            self.InformationalMessage
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ArrayOfInformationalMessage', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ArrayOfInformationalMessage')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ArrayOfInformationalMessage')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ArrayOfInformationalMessage', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ArrayOfInformationalMessage'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ArrayOfInformationalMessage', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        for InformationalMessage_ in self.InformationalMessage:
            InformationalMessage_.export(outfile, level, namespace_, name_='InformationalMessage', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'InformationalMessage':
            obj_ = InformationalMessage.factory()
            obj_.build(child_)
            self.InformationalMessage.append(obj_)
            obj_.original_tagname_ = 'InformationalMessage'
# end class ArrayOfInformationalMessage


class InformationalMessage(GeneratedsSuper):
    """InformationalMessage"""
    subclass = None
    superclass = None
    def __init__(self, Code=None, Message=None):
        self.original_tagname_ = None
        self.Code = Code
        self.Message = Message
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, InformationalMessage)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if InformationalMessage.subclass:
            return InformationalMessage.subclass(*args_, **kwargs_)
        else:
            return InformationalMessage(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Code(self): return self.Code
    def set_Code(self, Code): self.Code = Code
    def get_Message(self): return self.Message
    def set_Message(self, Message): self.Message = Message
    def hasContent_(self):
        if (
            self.Code is not None or
            self.Message is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='InformationalMessage', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('InformationalMessage')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='InformationalMessage')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='InformationalMessage', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='InformationalMessage'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='InformationalMessage', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Code is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Code>%s</tns:Code>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Code), input_name='Code')), eol_))
        if self.Message is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Message>%s</tns:Message>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Message), input_name='Message')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Code':
            Code_ = child_.text
            Code_ = self.gds_validate_string(Code_, node, 'Code')
            self.Code = Code_
        elif nodeName_ == 'Message':
            Message_ = child_.text
            Message_ = self.gds_validate_string(Message_, node, 'Message')
            self.Message = Message_
# end class InformationalMessage


class PIN(GeneratedsSuper):
    """PIN"""
    subclass = None
    superclass = None
    def __init__(self, Value=None):
        self.original_tagname_ = None
        self.Value = Value
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, PIN)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if PIN.subclass:
            return PIN.subclass(*args_, **kwargs_)
        else:
            return PIN(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Value(self): return self.Value
    def set_Value(self, Value): self.Value = Value
    def hasContent_(self):
        if (
            self.Value is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='PIN', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('PIN')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='PIN')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='PIN', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='PIN'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='PIN', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Value is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Value>%s</tns:Value>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.Value), input_name='Value')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Value':
            Value_ = child_.text
            Value_ = self.gds_validate_string(Value_, node, 'Value')
            self.Value = Value_
# end class PIN


class ArrayOfPIN(GeneratedsSuper):
    subclass = None
    superclass = None
    def __init__(self, PIN=None):
        self.original_tagname_ = None
        if PIN is None:
            self.PIN = []
        else:
            self.PIN = PIN
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ArrayOfPIN)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ArrayOfPIN.subclass:
            return ArrayOfPIN.subclass(*args_, **kwargs_)
        else:
            return ArrayOfPIN(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_PIN(self): return self.PIN
    def set_PIN(self, PIN): self.PIN = PIN
    def add_PIN(self, value): self.PIN.append(value)
    def insert_PIN_at(self, index, value): self.PIN.insert(index, value)
    def replace_PIN_at(self, index, value): self.PIN[index] = value
    def hasContent_(self):
        if (
            self.PIN
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ArrayOfPIN', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ArrayOfPIN')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ArrayOfPIN')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ArrayOfPIN', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ArrayOfPIN'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ArrayOfPIN', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        for PIN_ in self.PIN:
            PIN_.export(outfile, level, namespace_, name_='PIN', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'PIN':
            obj_ = PIN.factory()
            obj_.build(child_)
            self.PIN.append(obj_)
            obj_.original_tagname_ = 'PIN'
# end class ArrayOfPIN


class ResponseContext(GeneratedsSuper):
    """ResponseContext"""
    subclass = None
    superclass = None
    def __init__(self, ResponseReference=None):
        self.original_tagname_ = None
        self.ResponseReference = ResponseReference
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ResponseContext)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ResponseContext.subclass:
            return ResponseContext.subclass(*args_, **kwargs_)
        else:
            return ResponseContext(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_ResponseReference(self): return self.ResponseReference
    def set_ResponseReference(self, ResponseReference): self.ResponseReference = ResponseReference
    def hasContent_(self):
        if (
            self.ResponseReference is not None
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ResponseContext', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ResponseContext')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ResponseContext')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ResponseContext', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ResponseContext'):
        pass
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ResponseContext', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.ResponseReference is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:ResponseReference>%s</tns:ResponseReference>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.ResponseReference), input_name='ResponseReference')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        pass
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'ResponseReference':
            ResponseReference_ = child_.text
            ResponseReference_ = self.gds_validate_string(ResponseReference_, node, 'ResponseReference')
            self.ResponseReference = ResponseReference_
# end class ResponseContext


class VoidShipmentRequestContainer(RequestContainer):
    """VoidShipmentRequest"""
    subclass = None
    superclass = RequestContainer
    def __init__(self, PIN=None):
        self.original_tagname_ = None
        super(VoidShipmentRequestContainer, self).__init__()
        self.PIN = PIN
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, VoidShipmentRequestContainer)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if VoidShipmentRequestContainer.subclass:
            return VoidShipmentRequestContainer.subclass(*args_, **kwargs_)
        else:
            return VoidShipmentRequestContainer(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_PIN(self): return self.PIN
    def set_PIN(self, PIN): self.PIN = PIN
    def hasContent_(self):
        if (
            self.PIN is not None or
            super(VoidShipmentRequestContainer, self).hasContent_()
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='VoidShipmentRequestContainer', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('VoidShipmentRequestContainer')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='VoidShipmentRequestContainer')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='VoidShipmentRequestContainer', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='VoidShipmentRequestContainer'):
        super(VoidShipmentRequestContainer, self).exportAttributes(outfile, level, already_processed, namespace_, name_='VoidShipmentRequestContainer')
    def exportChildren(self, outfile, level, namespace_='tns:', name_='VoidShipmentRequestContainer', fromsubclass_=False, pretty_print=True):
        super(VoidShipmentRequestContainer, self).exportChildren(outfile, level, namespace_, name_, True, pretty_print=pretty_print)
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.PIN is not None:
            self.PIN.export(outfile, level, namespace_, name_='PIN', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        super(VoidShipmentRequestContainer, self).buildAttributes(node, attrs, already_processed)
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'PIN':
            obj_ = PIN.factory()
            obj_.build(child_)
            self.PIN = obj_
            obj_.original_tagname_ = 'PIN'
        super(VoidShipmentRequestContainer, self).buildChildren(child_, node, nodeName_, True)
# end class VoidShipmentRequestContainer


class VoidShipmentResponseContainer(ResponseContainer):
    """VoidShipmentResponse"""
    subclass = None
    superclass = ResponseContainer
    def __init__(self, ShipmentVoided=None):
        self.original_tagname_ = None
        super(VoidShipmentResponseContainer, self).__init__()
        self.ShipmentVoided = ShipmentVoided
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, VoidShipmentResponseContainer)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if VoidShipmentResponseContainer.subclass:
            return VoidShipmentResponseContainer.subclass(*args_, **kwargs_)
        else:
            return VoidShipmentResponseContainer(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_ShipmentVoided(self): return self.ShipmentVoided
    def set_ShipmentVoided(self, ShipmentVoided): self.ShipmentVoided = ShipmentVoided
    def hasContent_(self):
        if (
            self.ShipmentVoided is not None or
            super(VoidShipmentResponseContainer, self).hasContent_()
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='VoidShipmentResponseContainer', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('VoidShipmentResponseContainer')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='VoidShipmentResponseContainer')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='VoidShipmentResponseContainer', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='VoidShipmentResponseContainer'):
        super(VoidShipmentResponseContainer, self).exportAttributes(outfile, level, already_processed, namespace_, name_='VoidShipmentResponseContainer')
    def exportChildren(self, outfile, level, namespace_='tns:', name_='VoidShipmentResponseContainer', fromsubclass_=False, pretty_print=True):
        super(VoidShipmentResponseContainer, self).exportChildren(outfile, level, namespace_, name_, True, pretty_print=pretty_print)
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.ShipmentVoided is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:ShipmentVoided>%s</tns:ShipmentVoided>%s' % (self.gds_format_boolean(self.ShipmentVoided, input_name='ShipmentVoided'), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        super(VoidShipmentResponseContainer, self).buildAttributes(node, attrs, already_processed)
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'ShipmentVoided':
            sval_ = child_.text
            if sval_ in ('true', '1'):
                ival_ = True
            elif sval_ in ('false', '0'):
                ival_ = False
            else:
                raise_parse_error(child_, 'requires boolean')
            ival_ = self.gds_validate_boolean(ival_, node, 'ShipmentVoided')
            self.ShipmentVoided = ival_
        super(VoidShipmentResponseContainer, self).buildChildren(child_, node, nodeName_, True)
# end class VoidShipmentResponseContainer


class ValidateShipmentRequestContainer(RequestContainer):
    """ValidateShipmentRequest"""
    subclass = None
    superclass = RequestContainer
    def __init__(self, Shipment=None):
        self.original_tagname_ = None
        super(ValidateShipmentRequestContainer, self).__init__()
        self.Shipment = Shipment
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ValidateShipmentRequestContainer)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ValidateShipmentRequestContainer.subclass:
            return ValidateShipmentRequestContainer.subclass(*args_, **kwargs_)
        else:
            return ValidateShipmentRequestContainer(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Shipment(self): return self.Shipment
    def set_Shipment(self, Shipment): self.Shipment = Shipment
    def hasContent_(self):
        if (
            self.Shipment is not None or
            super(ValidateShipmentRequestContainer, self).hasContent_()
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ValidateShipmentRequestContainer', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ValidateShipmentRequestContainer')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ValidateShipmentRequestContainer')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ValidateShipmentRequestContainer', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ValidateShipmentRequestContainer'):
        super(ValidateShipmentRequestContainer, self).exportAttributes(outfile, level, already_processed, namespace_, name_='ValidateShipmentRequestContainer')
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ValidateShipmentRequestContainer', fromsubclass_=False, pretty_print=True):
        super(ValidateShipmentRequestContainer, self).exportChildren(outfile, level, namespace_, name_, True, pretty_print=pretty_print)
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Shipment is not None:
            self.Shipment.export(outfile, level, namespace_, name_='Shipment', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        super(ValidateShipmentRequestContainer, self).buildAttributes(node, attrs, already_processed)
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Shipment':
            obj_ = Shipment.factory()
            obj_.build(child_)
            self.Shipment = obj_
            obj_.original_tagname_ = 'Shipment'
        super(ValidateShipmentRequestContainer, self).buildChildren(child_, node, nodeName_, True)
# end class ValidateShipmentRequestContainer


class ValidateShipmentResponseContainer(ResponseContainer):
    """ValidateShipmentResponse"""
    subclass = None
    superclass = ResponseContainer
    def __init__(self, ValidShipment=None):
        self.original_tagname_ = None
        super(ValidateShipmentResponseContainer, self).__init__()
        self.ValidShipment = ValidShipment
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ValidateShipmentResponseContainer)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ValidateShipmentResponseContainer.subclass:
            return ValidateShipmentResponseContainer.subclass(*args_, **kwargs_)
        else:
            return ValidateShipmentResponseContainer(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_ValidShipment(self): return self.ValidShipment
    def set_ValidShipment(self, ValidShipment): self.ValidShipment = ValidShipment
    def hasContent_(self):
        if (
            self.ValidShipment is not None or
            super(ValidateShipmentResponseContainer, self).hasContent_()
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ValidateShipmentResponseContainer', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ValidateShipmentResponseContainer')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ValidateShipmentResponseContainer')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ValidateShipmentResponseContainer', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ValidateShipmentResponseContainer'):
        super(ValidateShipmentResponseContainer, self).exportAttributes(outfile, level, already_processed, namespace_, name_='ValidateShipmentResponseContainer')
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ValidateShipmentResponseContainer', fromsubclass_=False, pretty_print=True):
        super(ValidateShipmentResponseContainer, self).exportChildren(outfile, level, namespace_, name_, True, pretty_print=pretty_print)
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.ValidShipment is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:ValidShipment>%s</tns:ValidShipment>%s' % (self.gds_format_boolean(self.ValidShipment, input_name='ValidShipment'), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        super(ValidateShipmentResponseContainer, self).buildAttributes(node, attrs, already_processed)
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'ValidShipment':
            sval_ = child_.text
            if sval_ in ('true', '1'):
                ival_ = True
            elif sval_ in ('false', '0'):
                ival_ = False
            else:
                raise_parse_error(child_, 'requires boolean')
            ival_ = self.gds_validate_boolean(ival_, node, 'ValidShipment')
            self.ValidShipment = ival_
        super(ValidateShipmentResponseContainer, self).buildChildren(child_, node, nodeName_, True)
# end class ValidateShipmentResponseContainer


class ConsolidateRequestContainer(RequestContainer):
    """ValidateShipmentRequest"""
    subclass = None
    superclass = RequestContainer
    def __init__(self):
        self.original_tagname_ = None
        super(ConsolidateRequestContainer, self).__init__()
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ConsolidateRequestContainer)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ConsolidateRequestContainer.subclass:
            return ConsolidateRequestContainer.subclass(*args_, **kwargs_)
        else:
            return ConsolidateRequestContainer(*args_, **kwargs_)
    factory = staticmethod(factory)
    def hasContent_(self):
        if (
            super(ConsolidateRequestContainer, self).hasContent_()
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ConsolidateRequestContainer', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ConsolidateRequestContainer')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ConsolidateRequestContainer')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ConsolidateRequestContainer', pretty_print=pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ConsolidateRequestContainer'):
        super(ConsolidateRequestContainer, self).exportAttributes(outfile, level, already_processed, namespace_, name_='ConsolidateRequestContainer')
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ConsolidateRequestContainer', fromsubclass_=False, pretty_print=True):
        super(ConsolidateRequestContainer, self).exportChildren(outfile, level, namespace_, name_, True, pretty_print=pretty_print)
        pass
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        super(ConsolidateRequestContainer, self).buildAttributes(node, attrs, already_processed)
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        super(ConsolidateRequestContainer, self).buildChildren(child_, node, nodeName_, True)
        pass
# end class ConsolidateRequestContainer


class ConsolidateResponseContainer(ResponseContainer):
    """ValidateShipmentResponse"""
    subclass = None
    superclass = ResponseContainer
    def __init__(self, Consolidate=None):
        self.original_tagname_ = None
        super(ConsolidateResponseContainer, self).__init__()
        self.Consolidate = Consolidate
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, ConsolidateResponseContainer)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if ConsolidateResponseContainer.subclass:
            return ConsolidateResponseContainer.subclass(*args_, **kwargs_)
        else:
            return ConsolidateResponseContainer(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Consolidate(self): return self.Consolidate
    def set_Consolidate(self, Consolidate): self.Consolidate = Consolidate
    def hasContent_(self):
        if (
            self.Consolidate is not None or
            super(ConsolidateResponseContainer, self).hasContent_()
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='ConsolidateResponseContainer', namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('ConsolidateResponseContainer')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='ConsolidateResponseContainer')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='ConsolidateResponseContainer', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='ConsolidateResponseContainer'):
        super(ConsolidateResponseContainer, self).exportAttributes(outfile, level, already_processed, namespace_, name_='ConsolidateResponseContainer')
    def exportChildren(self, outfile, level, namespace_='tns:', name_='ConsolidateResponseContainer', fromsubclass_=False, pretty_print=True):
        super(ConsolidateResponseContainer, self).exportChildren(outfile, level, namespace_, name_, True, pretty_print=pretty_print)
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Consolidate is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:Consolidate>%s</tns:Consolidate>%s' % (self.gds_format_boolean(self.Consolidate, input_name='Consolidate'), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        super(ConsolidateResponseContainer, self).buildAttributes(node, attrs, already_processed)
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Consolidate':
            sval_ = child_.text
            if sval_ in ('true', '1'):
                ival_ = True
            elif sval_ in ('false', '0'):
                ival_ = False
            else:
                raise_parse_error(child_, 'requires boolean')
            ival_ = self.gds_validate_boolean(ival_, node, 'Consolidate')
            self.Consolidate = ival_
        super(ConsolidateResponseContainer, self).buildChildren(child_, node, nodeName_, True)
# end class ConsolidateResponseContainer


class CreateShipmentResponseContainer(ResponseContainer):
    """CreateShipmentResponse"""
    subclass = None
    superclass = ResponseContainer
    def __init__(self, ShipmentPIN=None, PiecePINs=None, ReturnShipmentPINs=None, ExpressChequePIN=None):
        self.original_tagname_ = None
        super(CreateShipmentResponseContainer, self).__init__()
        self.ShipmentPIN = ShipmentPIN
        self.PiecePINs = PiecePINs
        self.ReturnShipmentPINs = ReturnShipmentPINs
        self.ExpressChequePIN = ExpressChequePIN
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, CreateShipmentResponseContainer)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if CreateShipmentResponseContainer.subclass:
            return CreateShipmentResponseContainer.subclass(*args_, **kwargs_)
        else:
            return CreateShipmentResponseContainer(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_ShipmentPIN(self): return self.ShipmentPIN
    def set_ShipmentPIN(self, ShipmentPIN): self.ShipmentPIN = ShipmentPIN
    def get_PiecePINs(self): return self.PiecePINs
    def set_PiecePINs(self, PiecePINs): self.PiecePINs = PiecePINs
    def get_ReturnShipmentPINs(self): return self.ReturnShipmentPINs
    def set_ReturnShipmentPINs(self, ReturnShipmentPINs): self.ReturnShipmentPINs = ReturnShipmentPINs
    def get_ExpressChequePIN(self): return self.ExpressChequePIN
    def set_ExpressChequePIN(self, ExpressChequePIN): self.ExpressChequePIN = ExpressChequePIN
    def hasContent_(self):
        if (
            self.ShipmentPIN is not None or
            self.PiecePINs is not None or
            self.ReturnShipmentPINs is not None or
            self.ExpressChequePIN is not None or
            super(CreateShipmentResponseContainer, self).hasContent_()
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='CreateShipmentResponseContainer', namespacedef_='', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('CreateShipmentResponseContainer')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='CreateShipmentResponseContainer')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='CreateShipmentResponseContainer', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='CreateShipmentResponseContainer'):
        super(CreateShipmentResponseContainer, self).exportAttributes(outfile, level, already_processed, namespace_, name_='CreateShipmentResponseContainer')
    def exportChildren(self, outfile, level, namespace_='tns:', name_='CreateShipmentResponseContainer', fromsubclass_=False, pretty_print=True):
        super(CreateShipmentResponseContainer, self).exportChildren(outfile, level, namespace_, name_, True, pretty_print=pretty_print)
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.ShipmentPIN is not None:
            self.ShipmentPIN.export(outfile, level, namespace_, name_='ShipmentPIN', pretty_print=pretty_print)
        if self.PiecePINs is not None:
            self.PiecePINs.export(outfile, level, namespace_, name_='PiecePINs', pretty_print=pretty_print)
        if self.ReturnShipmentPINs is not None:
            self.ReturnShipmentPINs.export(outfile, level, namespace_, name_='ReturnShipmentPINs', pretty_print=pretty_print)
        if self.ExpressChequePIN is not None:
            self.ExpressChequePIN.export(outfile, level, namespace_, name_='ExpressChequePIN', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        super(CreateShipmentResponseContainer, self).buildAttributes(node, attrs, already_processed)
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'ShipmentPIN':
            obj_ = PIN.factory()
            obj_.build(child_)
            self.ShipmentPIN = obj_
            obj_.original_tagname_ = 'ShipmentPIN'
        elif nodeName_ == 'PiecePINs':
            obj_ = ArrayOfPIN.factory()
            obj_.build(child_)
            self.PiecePINs = obj_
            obj_.original_tagname_ = 'PiecePINs'
        elif nodeName_ == 'ReturnShipmentPINs':
            obj_ = ArrayOfPIN.factory()
            obj_.build(child_)
            self.ReturnShipmentPINs = obj_
            obj_.original_tagname_ = 'ReturnShipmentPINs'
        elif nodeName_ == 'ExpressChequePIN':
            obj_ = PIN.factory()
            obj_.build(child_)
            self.ExpressChequePIN = obj_
            obj_.original_tagname_ = 'ExpressChequePIN'
        super(CreateShipmentResponseContainer, self).buildChildren(child_, node, nodeName_, True)
# end class CreateShipmentResponseContainer


class CreateShipmentRequestContainer(RequestContainer):
    """CreateShipmentRequest"""
    subclass = None
    superclass = RequestContainer
    def __init__(self, Shipment=None, PrinterType=None):
        self.original_tagname_ = None
        super(CreateShipmentRequestContainer, self).__init__()
        self.Shipment = Shipment
        self.PrinterType = PrinterType
        self.validate_PrinterType(self.PrinterType)
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, CreateShipmentRequestContainer)
            if subclass is not None:
                return subclass(*args_, **kwargs_)
        if CreateShipmentRequestContainer.subclass:
            return CreateShipmentRequestContainer.subclass(*args_, **kwargs_)
        else:
            return CreateShipmentRequestContainer(*args_, **kwargs_)
    factory = staticmethod(factory)
    def get_Shipment(self): return self.Shipment
    def set_Shipment(self, Shipment): self.Shipment = Shipment
    def get_PrinterType(self): return self.PrinterType
    def set_PrinterType(self, PrinterType): self.PrinterType = PrinterType
    def validate_PrinterType(self, value):
        # Validate type PrinterType, a restriction on xs:string.
        if value is not None and Validate_simpletypes_:
            value = str(value)
            enumerations = ['Regular', 'Thermal']
            enumeration_respectee = False
            for enum in enumerations:
                if value == enum:
                    enumeration_respectee = True
                    break
            if not enumeration_respectee:
                warnings_.warn('Value "%(value)s" does not match xsd enumeration restriction on PrinterType' % {"value" : value.encode("utf-8")} )
    def hasContent_(self):
        if (
            self.Shipment is not None or
            self.PrinterType is not None or
            super(CreateShipmentRequestContainer, self).hasContent_()
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespace_='tns:', name_='CreateShipmentRequestContainer', namespacedef_='', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('CreateShipmentRequestContainer')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.original_tagname_ is not None:
            name_ = self.original_tagname_
        showIndent(outfile, level, pretty_print)
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='CreateShipmentRequestContainer')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='tns:', name_='CreateShipmentRequestContainer', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='tns:', name_='CreateShipmentRequestContainer'):
        super(CreateShipmentRequestContainer, self).exportAttributes(outfile, level, already_processed, namespace_, name_='CreateShipmentRequestContainer')
    def exportChildren(self, outfile, level, namespace_='tns:', name_='CreateShipmentRequestContainer', fromsubclass_=False, pretty_print=True):
        super(CreateShipmentRequestContainer, self).exportChildren(outfile, level, namespace_, name_, True, pretty_print=pretty_print)
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.Shipment is not None:
            self.Shipment.export(outfile, level, namespace_, name_='Shipment', pretty_print=pretty_print)
        if self.PrinterType is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<tns:PrinterType>%s</tns:PrinterType>%s' % (self.gds_encode(self.gds_format_string(quote_xml(self.PrinterType), input_name='PrinterType')), eol_))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
        return self
    def buildAttributes(self, node, attrs, already_processed):
        super(CreateShipmentRequestContainer, self).buildAttributes(node, attrs, already_processed)
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'Shipment':
            obj_ = Shipment.factory()
            obj_.build(child_)
            self.Shipment = obj_
            obj_.original_tagname_ = 'Shipment'
        elif nodeName_ == 'PrinterType':
            PrinterType_ = child_.text
            PrinterType_ = self.gds_validate_string(PrinterType_, node, 'PrinterType')
            self.PrinterType = PrinterType_
            # validate type PrinterType
            self.validate_PrinterType(self.PrinterType)
        super(CreateShipmentRequestContainer, self).buildChildren(child_, node, nodeName_, True)
# end class CreateShipmentRequestContainer


GDSClassesMapping = {
    'Address': Address,
    'ArrayOfContentDetail': ArrayOfContentDetail,
    'ArrayOfError': ArrayOfError,
    'ArrayOfInformationalMessage': ArrayOfInformationalMessage,
    'ArrayOfOptionIDValuePair': ArrayOfOptionIDValuePair,
    'ArrayOfPIN': ArrayOfPIN,
    'ArrayOfPiece': ArrayOfPiece,
    'ArrayOfSubscription': ArrayOfSubscription,
    'BuyerInformation': BuyerInformation,
    'ConsolidateRequest': ConsolidateRequestContainer,
    'ConsolidateRequestContainer': ConsolidateRequestContainer,
    'ConsolidateResponse': ConsolidateResponseContainer,
    'ConsolidateResponseContainer': ConsolidateResponseContainer,
    'ContentDetail': ContentDetail,
    'CreateShipmentRequest': CreateShipmentRequestContainer,
    'CreateShipmentRequestContainer': CreateShipmentRequestContainer,
    'CreateShipmentResponse': CreateShipmentResponseContainer,
    'CreateShipmentResponseContainer': CreateShipmentResponseContainer,
    'CreditCardInformation': CreditCardInformation,
    'Dimension': Dimension,
    'DutyInformation': DutyInformation,
    'Error': Error,
    'FromOnLabelInformation': FromOnLabelInformation,
    'InformationalMessage': InformationalMessage,
    'InternationalInformation': InternationalInformation,
    'NotificationInformation': NotificationInformation,
    'OptionIDValuePair': OptionIDValuePair,
    'OptionsInformation': OptionsInformation,
    'OtherInformation': OtherInformation,
    'PIN': PIN,
    'PackageInformation': PackageInformation,
    'PaymentInformation': PaymentInformation,
    'PhoneNumber': PhoneNumber,
    'PickupInformation': PickupInformation,
    'Piece': Piece,
    'ProactiveNotification': ProactiveNotification,
    'ReceiverInformation': ReceiverInformation,
    'RequestContainer': RequestContainer,
    'RequestContext': RequestContext,
    'ResponseContainer': ResponseContainer,
    'ResponseContext': ResponseContext,
    'ResponseInformation': ResponseInformation,
    'ReturnShipment': ReturnShipment,
    'ReturnShipmentInformation': ReturnShipmentInformation,
    'SenderInformation': SenderInformation,
    'Shipment': Shipment,
    'Subscription': Subscription,
    'TotalWeight': TotalWeight,
    'TrackingReferenceInformation': TrackingReferenceInformation,
    'ValidateShipmentRequest': ValidateShipmentRequestContainer,
    'ValidateShipmentRequestContainer': ValidateShipmentRequestContainer,
    'ValidateShipmentResponse': ValidateShipmentResponseContainer,
    'ValidateShipmentResponseContainer': ValidateShipmentResponseContainer,
    'VoidShipmentRequest': VoidShipmentRequestContainer,
    'VoidShipmentRequestContainer': VoidShipmentRequestContainer,
    'VoidShipmentResponse': VoidShipmentResponseContainer,
    'VoidShipmentResponseContainer': VoidShipmentResponseContainer,
    'Weight': Weight,
}


USAGE_TEXT = """
Usage: python <Parser>.py [ -s ] <in_xml_file>
"""


def usage():
    print(USAGE_TEXT)
    sys.exit(1)


def get_root_tag(node):
    tag = Tag_pattern_.match(node.tag).groups()[-1]
    rootClass = GDSClassesMapping.get(tag)
    if rootClass is None:
        rootClass = globals().get(tag)
    return tag, rootClass


def parse(inFileName, silence=False):
    parser = None
    doc = parsexml_(inFileName, parser)
    rootNode = doc.getroot()
    rootTag, rootClass = get_root_tag(rootNode)
    if rootClass is None:
        rootTag = 'CreateShipmentRequestContainer'
        rootClass = CreateShipmentRequestContainer
    rootObj = rootClass.factory()
    rootObj.build(rootNode)
    # Enable Python to collect the space used by the DOM.
    doc = None
    if not silence:
        sys.stdout.write('<?xml version="1.0" ?>\n')
        rootObj.export(
            sys.stdout, 0, name_=rootTag,
            namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"',
            pretty_print=True)
    return rootObj


def parseEtree(inFileName, silence=False):
    parser = None
    doc = parsexml_(inFileName, parser)
    rootNode = doc.getroot()
    rootTag, rootClass = get_root_tag(rootNode)
    if rootClass is None:
        rootTag = 'CreateShipmentRequestContainer'
        rootClass = CreateShipmentRequestContainer
    rootObj = rootClass.factory()
    rootObj.build(rootNode)
    # Enable Python to collect the space used by the DOM.
    doc = None
    mapping = {}
    rootElement = rootObj.to_etree(None, name_=rootTag, mapping_=mapping)
    reverse_mapping = rootObj.gds_reverse_node_mapping(mapping)
    if not silence:
        content = etree_.tostring(
            rootElement, pretty_print=True,
            xml_declaration=True, encoding="utf-8")
        sys.stdout.write(content)
        sys.stdout.write('\n')
    return rootObj, rootElement, mapping, reverse_mapping


def parseString(inString, silence=False):
    '''Parse a string, create the object tree, and export it.

    Arguments:
    - inString -- A string.  This XML fragment should not start
      with an XML declaration containing an encoding.
    - silence -- A boolean.  If False, export the object.
    Returns -- The root object in the tree.
    '''
    parser = None
    rootNode= parsexmlstring_(inString, parser)
    rootTag, rootClass = get_root_tag(rootNode)
    if rootClass is None:
        rootTag = 'CreateShipmentRequestContainer'
        rootClass = CreateShipmentRequestContainer
    rootObj = rootClass.factory()
    rootObj.build(rootNode)
    # Enable Python to collect the space used by the DOM.
    if not silence:
        sys.stdout.write('<?xml version="1.0" ?>\n')
        rootObj.export(
            sys.stdout, 0, name_=rootTag,
            namespacedef_='xmlns:tns="http://purolator.com/pws/datatypes/v2"')
    return rootObj


def parseLiteral(inFileName, silence=False):
    parser = None
    doc = parsexml_(inFileName, parser)
    rootNode = doc.getroot()
    rootTag, rootClass = get_root_tag(rootNode)
    if rootClass is None:
        rootTag = 'CreateShipmentRequestContainer'
        rootClass = CreateShipmentRequestContainer
    rootObj = rootClass.factory()
    rootObj.build(rootNode)
    # Enable Python to collect the space used by the DOM.
    doc = None
    if not silence:
        sys.stdout.write('#from ShippingService.xsd import *\n\n')
        sys.stdout.write('import ShippingService.xsd as model_\n\n')
        sys.stdout.write('rootObj = model_.rootClass(\n')
        rootObj.exportLiteral(sys.stdout, 0, name_=rootTag)
        sys.stdout.write(')\n')
    return rootObj


def main():
    args = sys.argv[1:]
    if len(args) == 1:
        parse(args[0])
    else:
        usage()


if __name__ == '__main__':
    #import pdb; pdb.set_trace()
    main()


__all__ = [
    "Address",
    "ArrayOfContentDetail",
    "ArrayOfError",
    "ArrayOfInformationalMessage",
    "ArrayOfOptionIDValuePair",
    "ArrayOfPIN",
    "ArrayOfPiece",
    "ArrayOfSubscription",
    "BuyerInformation",
    "ConsolidateRequestContainer",
    "ConsolidateResponseContainer",
    "ContentDetail",
    "CreateShipmentRequestContainer",
    "CreateShipmentResponseContainer",
    "CreditCardInformation",
    "Dimension",
    "DutyInformation",
    "Error",
    "FromOnLabelInformation",
    "InformationalMessage",
    "InternationalInformation",
    "NotificationInformation",
    "OptionIDValuePair",
    "OptionsInformation",
    "OtherInformation",
    "PIN",
    "PackageInformation",
    "PaymentInformation",
    "PhoneNumber",
    "PickupInformation",
    "Piece",
    "ProactiveNotification",
    "ReceiverInformation",
    "RequestContainer",
    "RequestContext",
    "ResponseContainer",
    "ResponseContext",
    "ResponseInformation",
    "ReturnShipment",
    "ReturnShipmentInformation",
    "SenderInformation",
    "Shipment",
    "Subscription",
    "TotalWeight",
    "TrackingReferenceInformation",
    "ValidateShipmentRequestContainer",
    "ValidateShipmentResponseContainer",
    "VoidShipmentRequestContainer",
    "VoidShipmentResponseContainer",
    "Weight"
]
