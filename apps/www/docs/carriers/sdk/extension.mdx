...
7. Add a database model for karrio server

8. Add the carrier configuration to the dashboard
...
```python title="modules/core/karrio/server/providers/extensions/[carrier_name].py"
...
-   First, you need to install your new extension and add it as a dependency in `./requirements.server.dev.txt`
...
-   Then, you can apply the database changes
...
<details>
<summary>Update the user connections query fragment</summary>

```ts title="packages/types/graphql/queries.ts"
...
```tsx title="packages/ui/modals/connect-provider-modal.tsx"
