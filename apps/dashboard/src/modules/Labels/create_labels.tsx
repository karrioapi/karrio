...
import { CheckBoxField, Dropdown, InputField, SelectField, Spinner } from '@karrio/ui/components';
...
export default function Page(pageProps: any) {
  const Component: React.FC = () => {
    ...
    const shipmentsQuery = useShipments({ id: shipmentIds, isDisabled: shipmentIds.length === 0 });
    ...
    const toggle = (shipment) => (e: React.MouseEvent) => {
      e.preventDefault();
    ...
    return (
      <div className="p-4">
        ...
        {(ready && Object.keys(keys).length > 0) && <>
                  ...
                  {batch.shipments.map((shipment, shipment_index) => (
                    <tr className="items is-size-7" key={keys[shipment_index]}>
                      ...
                      <td className="order is-vcentered panel is-size-7">
                        <label className="panel-block p-2 card" onClick={toggle(shipment)}>
                          <div>
                      ...
                      <td className="total is-vcentered">
                        <label className="panel-block px-2 p-4 card" onClick={toggle(shipment)}>
                          <div>
          ...
          {/* Label editor */}
          <>
            <Dialog as="div" open={isOpen} onClose={() => setIsOpen(false)} className={`modal side-modal ${isOpen ? "is-active" : ""}`}>
              <Dialog.Panel className="modal-card side-modal-body">
                <section className="modal-card-body has-background-white p-2">
                  {batch.shipments
                    .filter(({ id }, index) => (`${id || index}` === selectedRow))
                    .map((shipment, shipment_index) => {
                          ...
                          <ContextProviders>
                            <CommodityEditModalProvider orderIds={shipmentOrderIds}>
                              <header className="form-floating-header p-3 is-flex is-justify-content-space-between is-size-7">
                                ...
                                <div>
                                  <button className="button is-white m-0" style={{ borderRadius: '50%' }} onClick={toggle}>
                                    <span className="icon is-size-6">
                                      <i className="fas fa-lg fa-times"></i>
                                    </span>
                                  </button>
                                                  ...
                                                  <div className="field has-addons">
                                                    <p className="control is-expanded">
                                                      <input
                                                        ...
                                                        onChange={e => {
                                                          mutation.updateItem(shipment_index)(pkg_index, item_index, pkg.id)({ quantity: parseInt(e.target.value) } as CommodityType)
                                                        }}
                              ...
                              <div className="form-floating-footer px-2 pt-2 pb-4" style={{ borderTop: '1px solid #ddd' }}>
                                <header className="py-2 is-flex is-justify-content-space-between">
                                  <span className="is-title is-size-7 has-text-weight-bold is-vcentered">Service</span>
                                </header>
