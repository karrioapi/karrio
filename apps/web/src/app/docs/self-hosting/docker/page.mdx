...
Karrio includes built-in support for OpenTelemetry, providing comprehensive observability with distributed tracing, metrics collection, and log correlation.

### Quick Setup Options

#### Option 1: Simple Tracing with Jaeger

```bash
# Start Karrio with Jaeger tracing
docker compose -f docker-compose.yml -f observability/docker-compose.otel.yml up -d
```
...
```bash
# Start Karrio with Prometheus metrics
docker compose -f docker-compose.yml -f observability/docker-compose.prometheus.yml up -d
```

- **Prometheus**: http://localhost:9090 - Metrics collection and storage
- **What you get**: Custom application metrics and performance monitoring

#### Option 3: Complete Observability Stack

```bash
# Start Karrio with full observability stack
...
For detailed setup instructions, configuration files, and advanced options, see the [observability README](https://github.com/karrioapi/karrio/blob/main/docker/observability/README.md).
