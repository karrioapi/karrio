...
export function FeatureTabs({ tabs }: FeatureTabsProps) {
  ...
  return (
    <div className="feature-tabs w-full mx-auto bg-gradient-to-br from-[#1a103d]/80 to-[#0f0826]/90 backdrop-blur-sm rounded-xl border border-white/10">
          ...
          <TabsList className="w-full flex pt-2 pb-4 justify-between border-b border-white/10 !bg-transparent min-h-12 h-auto overflow-x-auto overflow-y-visible scrollbar-none">
            {tabs.map((tab) => (
              <TabsTrigger
                key={tab.value}
                value={tab.value}
                className="flex items-center shrink-0 justify-center gap-1.5 px-6 py-2.5 flex-1 text-white/70 text-sm font-medium transition-colors hover:text-white data-[state=active]:bg-[#5722cc] data-[state=active]:text-white data-[state=active]:shadow-sm data-[state=active]:rounded-md dark:data-[state=active]:bg-[#5722cc]"
              >
                {tab.icon}
