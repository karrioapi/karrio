...
def _build_token_response(access_token, refresh_token, is_verified: bool = True):
    """Build a 201 token pair response with no-cache headers.

    `is_verified` exposes the JWT's two-factor claim in the response body so
    clients (e.g. the admin dashboard) can decide whether to prompt for OTP
    without having to decode the JWT themselves. Defaults to True so refresh /
    verified endpoints — which always issue verified tokens — keep their
    existing response shape.
    """
    response = Response(
        {"access": access_token, "refresh": refresh_token, "is_verified": is_verified},
...
class TokenObtainPair(jwt_views.TokenObtainPairView):
    ...
    def post(self, *args, **kwargs):
        ...
        # `is_verified` is sourced from the refresh token claim — clients use it
        # to decide whether to prompt for an OTP and call /api/token/verified.
        is_verified = bool(jwt.RefreshToken(data["refresh"])["is_verified"])
        response = _build_token_response(data["access"], data["refresh"], is_verified)
