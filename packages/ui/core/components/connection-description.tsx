import { SystemConnectionType } from "@karrio/hooks/system-connection";
...
export const ConnectionDescription = ({
  connection,
}: ConnectionDescriptionComponent): JSX.Element => {
  const [key] = React.useState<string>(
    `description-${connection.id}-${Date.now()}`,
  );
