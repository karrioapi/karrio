...
type ConnectionType = GetRateSheet_rate_sheet;
type OperationType = {
...
type stateValue =
  | string
  | boolean
  | Partial<ServiceLevelType>
  | ServiceLevelType[]
  | undefined
  | null;

export const RateSheetStateContext =
  React.createContext<RateSheetStateContextType>(
    {} as RateSheetStateContextType,
  );
...
function reducer(
  ...
  switch (name) {
    case "currency":
      const newValue = (state as ServiceLevelType[]).map((service) => ({
        ...service,
        currency: value,
      }));
...
export const RateSheetEditModalProvider = ({
  ...
  const isUnChanged = (change: ServiceLevelType[]): boolean => {
    return isEqual(services, operation?.connection.services || []);
  };
  const editRateSheet = (operation: OperationType) => {
    const services = (operation.connection.services ||
      []) as ServiceLevelType[];
  ...
  const updateServices = (key: string) => (e: React.ChangeEvent<any>) => {
    const newValue = (services || []).map((service) => ({
      ...service,
      [key]: e.target.value,
    }));
  ...
  const updateSercice = (idx: number) => (event: React.ChangeEvent<any>) => {
    ...
    const newValue = (services || []).map((service, key) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    operation?.onSubmit &&
      (await operation?.onSubmit(services as ServiceLevelType[]));
    // close();
  };
  ...
  return (
    <Notifier>
      <RateSheetStateContext.Provider
        value={{ editRateSheet, operation, isActive }}
      >
        {children}
      </RateSheetStateContext.Provider>
                ...
                <div className="column px-0 pb-4">
                  ...
                  <div className="box mt-5">
                    <div className="content" style={{ fontSize: "90%" }}>
                      ...
                      <p>
                        The platform propose a unified way of defining carrier
                        service levels and rates. This is a work in progress and
                        we are looking for your feedback to improve it.
                      </p>
                    </div>
                  </div>
                </div>
