...
export const AcceptInvitationContext =
  React.createContext<AcceptInvitationInterface>(
...
export const AcceptInvitationProvider = ({
  ...
  const accept_invitation = searchParams.get("accept_invitation") as string;
  const [operation, setOperation] = useState<OperationType>(
    {} as OperationType,
  );
  const {
    query: { error, data: { organization_invitation } = {} },
    ...mutation
  } = useOrganizationInvitation(accept_invitation);
  ...
  const handleSubmit = async (evt: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const data = await mutation.acceptInvitation.mutateAsync({
        guid: accept_invitation,
      });
          ...
          {isActive && (
            <form className="modal-card" onSubmit={handleSubmit}>
              <section className="modal-card-body modal-form">
                ...
                {!error &&
                  organization_invitation?.invitee_identifier !==
                    user?.email && (
                    <p className="is-size-6 has-text-centered">
                ...
                {organization_invitation?.invitee_identifier ===
                  user?.email && (
                  <p className="is-size-6 has-text-centered">
                    Click Confirm to accept the invitation to{" "}
                    <strong>
                      {organization_invitation?.organization_name}
                    </strong>
                    .
                  </p>
                )}
