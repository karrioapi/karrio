import { request_email_change_request_email_change_errors } from "@karrio/types";
...
export const EmailManagement = ({
  children,
}: EmailManagementComponent): JSX.Element => {
  ...
  const [errors, setErrors] = useState<
    request_email_change_request_email_change_errors[]
  >([]);
            ...
            >
              <section className="modal-card-body modal-form">
                ...
                {(errors as any[])
                  ...
                  .map(({ message }, key) => (
                    <p key={key} className="has-text-danger is-size-7">
                      {message}
                    </p>
                  ))}

                <p className="is-size-6 mt-3 mb-1 has-text-weight-bold">
                  1. Enter a new email
                </p>
                <p className="is-size-7 mb-5">
                  We’ll send you an email to the new address to verify that you
                  own it.
                </p>
                ...
                <div className="field">
                  ...
                  {(errors as any[])
                    .filter(({ validation }) => validation && validation.email)
                    .map(({ validation }, key) => (
                      <p key={key} className="has-text-danger is-size-7">
                ...
                <div className="field">
                  ...
                  {(errors as any[])
                    .filter(
                      ({ validation }) => validation && validation.password,
                    )
                    .map(({ validation }, key) => (
                      <p key={key} className="has-text-danger is-size-7">
