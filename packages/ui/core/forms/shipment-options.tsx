...
function reducer(
  ...
  switch (name) {
    case "addCOD":
      return cleanDict<any>({
        ...state,
        cash_on_delivery: value === true ? "" : undefined,
      });
    case "addInsurance":
      return cleanDict<any>({
        ...state,
        insurance: value === true ? "" : undefined,
      });
    case "addDeclaredValue":
      return cleanDict<any>({
        ...state,
        declared_value: value === true ? "" : undefined,
      });
    default:
...
export const ShipmentOptions = ({
  ...
  const computeDisable = (
    ...
    return (
      (isEqual(shipment.options, options) ||
        (options === ({} as any) && shipment.options === ({} as any))) &&
      (isEqual(shipment.metadata, metadata) ||
        (metadata === ({} as any) && shipment.metadata === ({} as any))) &&
    ...
    try {
      if (shipment.id !== undefined) {
        ...
        notify({
          ...
          message: "Shipment options successfully updated!",
        });
      } else {
        await onSubmit({ options, metadata, reference });
      }
  ...
  return (
    <form className="px-1 py-2" onSubmit={handleSubmit}>
      ...
      <div className="columns is-multiline mb-0">
        <InputField
          defaultValue={options?.shipping_date}
          onChange={(e) =>
            dispatch({
              name: "shipping_date",
              value: moment(e.target.value).format("YYYY-MM-DDTHH:mm:ssZ"),
            })
          }
          label="shipping date"
          name="shipping_date"
          type="datetime-local"
          className="is-small"
          fieldClass="column mb-0 is-5 px-2 py-2"
        />

        <CheckBoxField
          defaultChecked={options?.signature_confirmation}
      ...
      <div className="columns is-multiline mb-0 pt-2">
        <CheckBoxField
          defaultChecked={!isNone(options?.insurance)}
          onChange={handleChange}
          name="addInsurance"
          fieldClass="column mb-0 is-12 px-2 py-2"
        >
        ...
        <div
          className="column is-multiline mb-0 ml-6 my-1 px-2 py-0 is-12"
          style={{
            borderLeft: "solid 2px #ddd",
            display: `${isNone(options?.insurance) ? "none" : "block"}`,
          }}
        >
          <InputField
      ...
      <div className="columns is-multiline mb-0 px-1 my-2">
        <SelectField
          ...
          fieldClass="column is-4 mb-0 px-1 py-2"
          required={
      ...
      <div className="columns is-multiline mb-0 pt-2">
        <CheckBoxField
          defaultChecked={!isNone(options?.cash_on_delivery)}
          onChange={handleChange}
          name="addCOD"
          fieldClass="column mb-0 is-12 px-2 py-2"
        >
        ...
        <div
          ...
          style={{
            borderLeft: "solid 2px #ddd",
            display: `${isNone(options?.cash_on_delivery) ? "none" : "block"}`,
          }}
        >
          <InputField
            defaultValue={options?.cash_on_delivery}
            onChange={handleChange}
            ...
            required={!isNone(options?.cash_on_delivery)}
          >
            <span className="icon is-small is-left">
              <i className="fas fa-dollar-sign"></i>
            </span>
      ...
      <div className="columns is-multiline mb-0 pt-2">
        <CheckBoxField
          defaultChecked={!isNone(options?.declared_value)}
          onChange={handleChange}
        ...
        <div
          ...
          style={{
            borderLeft: "solid 2px #ddd",
            display: `${isNone(options?.declared_value) ? "none" : "block"}`,
          }}
        >
          <InputField
            defaultValue={options?.declared_value}
            onChange={handleChange}
            label="Package value"
            name="declared_value"
            type="number"
            ...
            controlClass="has-icons-left has-icons-right"
            fieldClass="column mb-0 is-4 px-1 py-2"
            required={!isNone(options?.declared_value)}
          >
            <span className="icon is-small is-left">
              <i className="fas fa-dollar-sign"></i>
            </span>
