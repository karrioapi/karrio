...
export const ParcelForm = ({
  ...
  const [parcel_type, setParcelType] = useState<string>(
    isNone(value?.package_preset) ? "custom" : "preset",
  );
  const [dimension, setDimension] = useState<string | undefined>(
    formatDimension(isNone(value?.package_preset) ? undefined : value),
  );
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement | any>) => {
    ...
    if (name === "parcel_type") {
      ...
      } as Partial<ParcelType>;

      setParcelType(value as string);
      setDimension(
        formatDimension(value === "custom" ? undefined : template || preset),
      );
      value = { ...(template || (preset as any)), id: parcel.id };
      name = isNone(template) ? name : "template";
    } else if (name === "package_preset") {
      const preset =
        findPreset(package_presets as PresetCollection, value as string) ||
        parcel;
  ...
  return (
    <div key={key}>
      ...
      {parcel_type === "preset" && (
        <>
          ...
          >
            ...
            {Object.entries(package_presets || {}).map(([key, value]) => (
      ...
      {shouldShowDimension(parcel_type) && (
        <div className="is-size-7 mt-1 mb-2 has-text-grey">
          {dimension || ""}
        </div>
      )}
