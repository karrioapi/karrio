    ...
    <div className="plex-sidebar admin-sidebar" ref={sidebar}>
      <div className="admin-sidebar-menu card px-0 ml-2">
          ...
          <div className="media">
            <div className="media-left">
              <figure className="image is-32x32">
                <Image
                  src={p`/carriers/${metadata?.APP_NAME || "X"}_icon.svg`}
                  width={32}
                  height={32}
                  alt="app icon"
                />
            ...
            <div className="media-content" style={{ overflow: "hidden" }}>
              <p className="title is-size-6 text-ellipsis">
                {formatRef(metadata?.APP_NAME)?.toLocaleLowerCase()}
              </p>
        ...
        >
          <AppLink
            href="/admin"
            className={"menu-item px-3 " + activeClass("/admin")}
            shallow={false}
            prefetch={false}
          >
            <i
              className={`fa fa-layer-group mr-2 ${isActive("/admin") ? "" : "has-text-grey"}`}
            ></i>
            <span className="has-text-weight-bold">Platform details</span>
          </AppLink>
          <AppLink
            href="/admin/carrier_connections"
            className={
              "menu-item px-3 " + activeClass("/admin/carrier_connections")
            }
            shallow={false}
            prefetch={false}
          >
            <i
              className={`fa fa-truck mr-2 ${isActive("/admin/carrier_connections") ? "" : "has-text-grey"}`}
            ></i>
            <span className="has-text-weight-bold">Carrier connections</span>
          </AppLink>
          <AppLink
            href="/admin/user_accounts"
            className={"menu-item px-3 " + activeClass("/admin/user_accounts")}
            shallow={false}
            prefetch={false}
          >
            <i
              className={`fa fa-user-lock mr-2 ${isActive("/admin/user_accounts") ? "" : "has-text-grey"}`}
            ></i>
          ...
          {metadata?.MULTI_ORGANIZATIONS && (
            <>
              <AppLink
                href="/admin/organization_accounts"
                className={
                  "menu-item px-3 " +
                  activeClass("/admin/organization_accounts")
                }
                shallow={false}
                prefetch={false}
              >
                <i
                  className={`fa fa-users mr-2 ${isActive("/admin/organization_accounts") ? "" : "has-text-grey"}`}
                ></i>
          ...
          <AppLink
            ...
            className={"menu-item px-3 " + activeClass("/admin/surcharges")}
            shallow={false}
            prefetch={false}
          >
            <i
              className={`fa fa-percent mr-2 ${isActive("/admin/surcharges") ? "" : "has-text-grey"}`}
            ></i>
            <span className="has-text-weight-bold">
