import {
  useSystemCarrierConnectionMutation,
  useSystemCarrierConnections,
} from "@karrio/hooks/admin/connections";
...
type ConnectionUpdateType = Partial<UpdateConnectionMutationInput> & {
  id: string;
  carrier_name: string;
};
interface SystemCarrierManagementComponent {}

export const SystemCarrierManagement =
  ({}: SystemCarrierManagementComponent): JSX.Element => {
    ...
    const {
      query: { data: { system_connections = [] } = {}, ...query },
    } = useSystemCarrierConnections();

    const update =
      ({ carrier_name, ...data }: ConnectionUpdateType) =>
      async () => {
        try {
      ...
      <>
        ...
        >
          <header className="px-3 mt-3 is-flex is-justify-content-space-between">
            <span className="is-title is-size-6 has-text-weight-bold is-vcentered my-2">
          ...
          <div className="p-0">
            ...
            {query.isFetched && system_connections.length > 0 && (
              <>
                <div
                  className="table-container px-2"
                        ...
                        <tr key={`${connection.id}-${Date.now()}`}>
                          <td className="carrier is-vcentered pl-1">
                            <CarrierBadge
                              ...
                              carrier_name={
                                connection!.carrier_name == "generic"
                          ...
                          <td className="mode is-vcentered">
                            {connection.test_mode && (
                              <span className="tag is-warning is-centered">
                                Test
                              </span>
                            )}
                          </td>
                              ...
                              <button
                                ...
                                onClick={() =>
                                  editConnection({
                                    connection: connection as any,
                                    update:
                                      mutation.updateSystemCarrierConnection
                                        .mutateAsync,
                                  })
                                }
                              >
            ...
            {query.isFetched && system_connections.length == 0 && (
              <div className="message is-white my-6">
                <div className="card-content has-text-centered">
                  <p>No system carrier connections configured.</p>
                  <p>
                    Use the <strong>Add account</strong> button above to add a
                    new connection
                  </p>
                </div>
              </div>
