...
import { Dialog, DialogContent, DialogTitle, DialogDescription } from "@karrio/ui/components/ui/dialog";
...
import { useRateSheetMutation, useRateSheet } from "@karrio/hooks/rate-sheet";
...
export const RateSheetEditor = ({ rateSheetId, onClose, preloadCarrier, linkConnectionId }: RateSheetEditorProps) => {
  const loader = useLoader();
  const { references, metadata } = useAPIMetadata();
  const isNew = rateSheetId === 'new';

  // Fetch carrier metadata and references for default services
  React.useEffect(() => {
    if (metadata?.HOST) {
      Promise.all([
        fetch(`${metadata.HOST}/v1/carriers`).then(res => res.json()),
        fetch(`${metadata.HOST}/v1/references`).then(res => res.json())
      ])
        .then(([carriers, refs]) => {
          setCarrierMetadata(carriers);
          // Store references in a way we can access them
          (window as any).__karrio_references = refs;
        })
        .catch(console.error);
    }
  }, [metadata?.HOST]);
  ...
  const { createRateSheet, updateRateSheet, deleteRateSheetService } = useRateSheetMutation();
  ...
  const loadCarrierDefaults = React.useCallback((carrierName?: string) => {
    ...
    const refs = (window as any).__karrio_references;
  ...
  return (
    <>
      <Dialog open={true} onOpenChange={onClose}>
        <DialogContent className="max-w-full max-h-full h-screen p-0 gap-0 overflow-hidden">
          <DialogTitle className="sr-only">
            {isNew ? "Create Rate Sheet" : "Edit Rate Sheet"}
          </DialogTitle>
          <DialogDescription className="sr-only">
            {isNew ? "Create a new rate sheet by selecting a carrier, naming the sheet, and configuring services and zones." : "Edit the selected rate sheet including services, zones and rates."}
          </DialogDescription>
          {/* Sticky Header */}
          <header
