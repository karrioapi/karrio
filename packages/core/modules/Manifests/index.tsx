...
export default function Page() {
  const Component = (): JSX.Element => {
    ...
    const updatedSelection = (
      selectedManifests: string[],
      current: typeof manifests,
    ) => {
      const manifest_ids = (current?.edges || []).map(
        ({ node: manifest }) => manifest.id,
      );
      const selection = selectedManifests.filter((id) =>
        manifest_ids.includes(id),
      );
      const selected =
        selection.length > 0 &&
        selection.length === (manifest_ids || []).length;
      setAllChecked(selected);
      if (
        selectedManifests.filter((id) => !manifest_ids.includes(id)).length > 0
      ) {
    ...
    const handleSelection = (e: React.ChangeEvent) => {
      ...
      if (name === "all") {
        setSelection(
          !checked
            ? []
            : (manifests?.edges || []).map(({ node: { id } }) => id),
        );
      } else {
        setSelection(
          checked
    ...
    React.useEffect(() => {
      updatedSelection(selection, manifests);
    }, [selection, manifests]);
              ...
              <table className="manifests-table table is-fullwidth">
                <tbody>
                  <tr>
                    ...
                    {selection.length === 0 && (
                      <>
                        <td className="carrier is-size-7 is-vcentered">
                          CARRIER
                        </td>
                        <td className="address is-size-7 is-vcentered">
                          ADDRESS
                        </td>
                      ...
                      >
                        <div className="icon-text">
                          <CarrierImage
                            ...
                            width={28}
                            text_color={
                              getCarrier(manifest)?.config?.text_color
                            }
                            background={
                              getCarrier(manifest)?.config?.brand_color
                            }
                          />
                          <div
                            className="text-ellipsis"
                            style={{ maxWidth: "250px", lineHeight: "16px" }}
                          >
                        ...
                        >
                          <p
                            ...
                            title={formatAddressShort(
                              manifest.address as AddressType,
                            )}
                          >
                            {formatAddressShort(
                              manifest.address as AddressType,
