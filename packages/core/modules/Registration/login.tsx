      ...
      setTimeout(
        () =>
          window.location.replace(
            p`${new URLSearchParams(location.search).get("next") || "/"}`,
          ),
        500,
      );
    } else {
  ...
  return (
    <>
      <SectionLayout {...pageProps}>
        <Head>
          <title>{`Login - ${references.APP_NAME}`}</title>
        </Head>
        ...
        <div className="card isolated-card">
          <div className="card-content">
            ...
            {showError && (
              <p className="has-text-danger is-size-6 has-text-centered">
            ...
            <form method="post" onSubmit={onSubmit}>
              ...
              <div className="field mt-5">
                <label className="label level" htmlFor="id_password">
                  <span>Password</span>
                  <Link legacyBehavior href="/password/reset/request" passHref>
