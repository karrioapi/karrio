...
export const generateMetadata = dynamicMetadata("Apps");
const ContextProviders = bundleContexts([ModalProvider]);

export default function Page(pageProps: any) {
  const Component: React.FC = () => {
    const { query: { data: { installations = {} } = {}, ...query } = {} } =
      {} as any;
            ...
            <>
              <nav
                ...
                style={{
                  ...
                  boxShadow: "0.5px 0.5px 2.5px #efefef",
                }}
              >
                {(installations?.edges || []).map(({ node: installation }) => (
                  <label
                    key={installation.id}
                    className="columns is-mobile m-0 panel-block"
                  >
                    <AppLink
                      href={`/installations/${installation.id}`}
                      className="column is-11 is-size-7 has-text-grey p-0"
                    >
                      <div className="has-text-weight-bold is-size-6 text-ellipsis">
                        {installation.app.display_name}
                      </div>
                      <div className="has-text-weight-semibold mt-2">
                        <span className="tag is-light">
                          {installation.app.developer_name}
                        </span>
                      </div>
                    </AppLink>
