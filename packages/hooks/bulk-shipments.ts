...
const DEFAULT_STATE = {
  shipments: [] as ShipmentType[],
} as BatchShipmentDataType;

function reducer(
  state: Partial<BatchShipmentDataType>,
  { name, value }: { name: string; value: Partial<BatchShipmentDataType> },
): BatchShipmentDataType {
  switch (name) {
    case "full":
    ...
    default:
      let newState = {
        ...state,
        ...(value as Partial<BatchShipmentDataType>),
      } as BatchShipmentDataType;
...
export function useBatchShipmentForm({ shipmentList }: BatchShipmentFormProps) {
  ...
  const [updateRate, setUpdateRate] = React.useState<[number, boolean]>([
    0,
    false,
  ]);
  const [batch, dispatch] = React.useReducer(
    reducer,
    DEFAULT_STATE,
    () => DEFAULT_STATE,
  );
  ...
  const shouldFetchRates =
    (shipment_index: number) => (changes: ShipmentType) => {
      return (
        (!isNone(changes.shipper) &&
          batch.shipments[shipment_index].shipper.address_line1 !==
          changes.shipper.address_line1) ||
        (!isNone(changes.shipper) &&
          batch.shipments[shipment_index].shipper.country_code !==
          changes.shipper.country_code) ||
        (!isNone(changes.shipper) &&
          batch.shipments[shipment_index].shipper.city !==
          changes.shipper.city) ||
        (!isNone(changes.recipient) &&
          batch.shipments[shipment_index].recipient.address_line1 !==
          changes.recipient.address_line1) ||
        (!isNone(changes.recipient) &&
          batch.shipments[shipment_index].recipient.country_code !==
          changes.recipient.country_code) ||
        (!isNone(changes.recipient) &&
          batch.shipments[shipment_index].recipient.city !==
          changes.recipient.city)
      );
    };
  const parcelHasRateUpdateChanges = (
    parcel: ParcelType,
    changes: Partial<ParcelType>,
  ) => {
    return (
      (!isNone(changes.packaging_type) &&
        changes.packaging_type !== parcel.packaging_type) ||
      (!isNone(changes.package_preset) &&
        changes.package_preset !== parcel.package_preset) ||
      (!isNone(changes.is_document) &&
        changes.is_document !== parcel.is_document) ||
  ...
  const syncOptionsChanges =
    (shipment_index: number) =>
      (changes: Partial<ShipmentType>): Partial<ShipmentType> => {
        let options =
          changes.options || batch.shipments[shipment_index].options || {};
        const parcels =
          changes.parcels || batch.shipments[shipment_index].parcels;
        const customs =
          "customs" in changes
            ? changes.customs
            : batch.shipments[shipment_index].customs;
        const commodities = getShipmentCommodities(
          { parcels } as any,
          customs?.commodities,
        );
        ...
        const declared_value = parseFloat(
          commodities
            .reduce(
              (_, { value_amount, quantity }) =>
                _ + toNumber(quantity || 1) * toNumber(value_amount || 0),
              0,
            )
            .toFixed(2),
        );
        const currency = parcels.reduce((__, p) => {
          const curr = (p.items || []).reduce(
            (_, { value_currency }) => (_ ? _ : (value_currency as any)),
            null,
          );
        ...
        if (
          declared_value > 0 &&
          declared_value !==
          batch.shipments[shipment_index].options.declared_value
        ) {
        ...
        if (
          !isNone(currency) &&
          currency !== batch.shipments[shipment_index].options.currency
        ) {
          options = { ...options, currency };
        }

        // ignore if options hasn't changed
        if (isEqual(options, batch.shipments[shipment_index].options))
          return changes;

        return { ...changes, options };
      };
  const syncCustomsChanges =
    (shipment_index: number) =>
      (changes: Partial<ShipmentType>): Partial<ShipmentType> => {
        const shipper =
          changes.shipper || batch.shipments[shipment_index].shipper;
        const recipient =
          changes.recipient || batch.shipments[shipment_index].recipient;
        const isIntl =
          !isNone(shipper?.country_code) &&
          !isNone(recipient?.country_code) &&
        ...
        if (requireCustoms && !hasCustomsChanges && hasParcelsChanges)
          skip = false;
        ...
        const options =
          changes.options || batch.shipments[shipment_index].options;
        const currency =
          batch.shipments[shipment_index].customs?.duty?.currency ||
          options?.currency;
        const paid_by =
          batch.shipments[shipment_index].customs?.duty?.paid_by ||
          batch.shipments[shipment_index].payment?.paid_by;
        const incoterm =
          batch.shipments[shipment_index].customs?.incoterm ||
          (paid_by == "sender" ? "DDP" : "DDU");
        const declared_value =
          options?.declared_value ||
          batch.shipments[shipment_index].customs?.duty?.declared_value;
        const account_number =
          batch.shipments[shipment_index].customs?.duty?.account_number ||
          batch.shipments[shipment_index].payment?.account_number;
        ...
        const customs: any = {
          ...
          commodities,
          duty: {
            ...(batch.shipments[shipment_index].customs?.duty ||
      ...
      async (
        ...
        if (updateLocalState) {
          const update = {
            shipments: batch.shipments.map((shipment, index) =>
              shipment.id === shipment_id || index === shipment_index
                ? { ...shipment, ...changes }
                : shipment,
            ),
          };
        ...
        if (uptateServerState) {
          try {
            ...
            let { status, rates, messages, service, carrier_ids, ...data } =
              changes;
            if (Object.keys(data).length === 0) return; // abort if no data changes

            await shipmentMutation.updateShipment
              .mutateAsync({
                id: batch.shipments[shipment_index].id,
                ...data,
              } as any)
            ...
            const update = {
              shipments: batch.shipments.map((shipment, index) =>
                shipment.id === shipment_id || index === shipment_index
                  ? { ...shipment, messages: errorToMessages(error) }
                  : shipment,
              ),
            };
            updateBatch(update as any);
  ...
  const addParcel = (shipment_index: number) => async (data: ParcelType) => {
    const update = {
      parcels: [...batch.shipments[shipment_index].parcels, data],
    };
    ...
    (shipment_index: number) =>
      (parcel_index: number, parcel_id?: string) =>
        async (items: CommodityType[]) => {
          const ts = Date.now();
          const parcel =
            batch.shipments[shipment_index].parcels.find(
              ({ id }) => id === parcel_id,
            ) || batch.shipments[shipment_index].parcels[parcel_index];
          const indexes = new Set(
            (batch.shipments[shipment_index].parcels[parcel_index].items || []).map(
              (item, index) =>
  ...
  const removeParcel =
    (shipment_index: number) =>
      (parcel_index: number, parcel_id?: string) =>
        async () => {
          const update = {
            parcels: batch.shipments[shipment_index].parcels.filter(
              (_, index) => index !== parcel_index,
            ),
          };

          if (!isLocalDraft(batch.shipments[shipment_index].id) && !!parcel_id) {
            await shipmentMutation.discardParcel.mutateAsync({
              id: parcel_id as string,
            });
          }
  ...
  const addCommodities =
    (shipment_index: number) =>
      async (items: CommodityType[], customs_id?: string) => {
        ...
        async ({ id, ...data }: CommodityType, change?: ChangeType) => {
          ...
          const update = {
            ...
            commodities: (
              ...
              index !== cdt_index ? item : { ...item, ...data },
            ),
          };
        ...
        async (cdt_id?: string) => {
          ...
          const update = {
            ...
            commodities: (
              ...
              !!id ? id !== cdt_id : index !== cdt_index,
            ),
          };

          if (!isLocalDraft(batch.shipments[shipment_index].id) && !!cdt_id) {
            await shipmentMutation.discardCommodity.mutateAsync({
              id: cdt_id as string,
            });
          }
  ...
  const fetchRates = useMutation(async (shipment_index: number) => {
    const data = JSON.parse(
      jsonify(batch.shipments[shipment_index]),
    ) as ShipmentType;
    try {
      const { rates, messages } =
        await shipmentMutation.fetchRates.mutateAsync(data);
      await updateShipment(shipment_index)({
        rates,
        messages,
      } as Partial<ShipmentType>);
    } catch (error: any) {
      await updateShipment(shipment_index)({
        rates: [],
        messages: errorToMessages(error),
      } as Partial<ShipmentType>);
    }
  });
  const buyLabels = useMutation(async () => {
    ...
    const data = {
      shipments: batch.shipments.map((shipment) => ({
        ...
        service:
          shipment.options.preferred_service || shipment.rates?.[0]?.service,
      })),
    ...
    try {
      ...
      notifier.notify({
        ...
        message: `Batch shipments created.`,
      });
    } catch (error: any) {
  ...
  React.useEffect(() => {
    if (
      ...
      !!batch?.shipments.find((shipment) => isNone(shipment.id))
    ) {
      console.log("> create missing drafts...");
      Promise.all(
        batch.shipments.map(async (shipment) =>
          !!shipment.id && (shipment.rates || []).length > 0
            ? shipment
            : ((await shipmentMutation.createShipment.mutateAsync(
              shipment as any,
            )) as ShipmentType),
        ),
      )
        .then((shipments) => dispatch({ name: "full", value: { shipments } }))
        .finally(() => {
