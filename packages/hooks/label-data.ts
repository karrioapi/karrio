...
function reducer(
  state: any,
  {
    name,
    value,
  }: { name: string; value: Partial<ShipmentType> | ShipmentType },
): ShipmentType {
  switch (name) {
    case "full":
...
export function useLabelData(id: string, initialData?: ShipmentType) {
  ...
  const [shipment, dispatch] = React.useReducer(
    reducer,
    initialData || DEFAULT_SHIPMENT_DATA,
    () => initialData || DEFAULT_SHIPMENT_DATA,
  );
  ...
  React.useEffect(() => {
    if (
      id !== "new" &&
      isEqual(shipment, DEFAULT_SHIPMENT_DATA) &&
      !isNone(query.data?.shipment)
    ) {
...
export function useLabelDataMutation(id: string, initialData?: ShipmentType) {
  ...
  const syncOptionsChanges = (
    ...
    const customs =
      "customs" in changes ? changes.customs : state.shipment.customs;
    ...
    if (
      declared_value > 0 &&
      declared_value !== state.shipment.options.declared_value
    ) {
  ...
  const syncCustomsChanges = (
    ...
    const currency =
      state.shipment.customs?.duty?.currency || options?.currency;
    const paid_by =
      state.shipment.customs?.duty?.paid_by || state.shipment.payment?.paid_by;
    const incoterm =
      state.shipment.customs?.incoterm || (paid_by == "sender" ? "DDP" : "DDU");
    const declared_value =
      options?.declared_value || state.shipment.customs?.duty?.declared_value;
    const account_number =
      state.shipment.customs?.duty?.account_number ||
      state.shipment.payment?.account_number;
  ...
  const updateShipment = async (
    ...
    if (shouldFetchRates(changes as any)) {
      setUpdateRate(true);
    }
    ...
    if (uptateServerState && !mutation.updateShipment.isLoading) {
      try {
        ...
        await mutation.updateShipment
          .mutateAsync({ id: state.shipment.id, ...data } as any)
  ...
  const removeParcel =
    (parcel_index: number, parcel_id?: string) => async () => {
      const update = {
        parcels: state.shipment.parcels.filter(
  ...
  const removeCommodity =
    (cdt_index: number, customs_id?: string) => async (cdt_id?: string) => {
      ...
      const update = {
        ...(state.shipment.customs as CustomsType),
        commodities: (state.shipment.customs?.commodities || []).filter(
          ({ id }, index) => (!!id ? id !== cdt_id : index !== cdt_index),
        ),
      };
  ...
  const fetchRates = useMutation(async () => {
    ...
    try {
      ...
      const { rates, messages } = await mutation.fetchRates.mutateAsync(
        data as ShipmentType,
      );
  ...
  const buyLabel = useMutation(
    async (
      rate: ShipmentType["rates"][0],
      action: { redirect: boolean } = { redirect: true },
    ) => {
      if (mutation.buyLabel.isLoading) return;
      const { messages, rates, ...data } = state.shipment;
      const selection = isLocalDraft(state.shipment.id)
        ? { service: rate.service, carrier_ids: [rate.carrier_id] }
        : { selected_rate_id: rate.id };

      try {
        loader.setLoading(true);
        const { id } = await mutation.buyLabel.mutateAsync({
          ...data,
          ...selection,
        } as any);
        notifier.notify({
  ...
  const saveDraft = useMutation(
    async (
      action: { redirect?: boolean; notify?: boolean } = {
        redirect: true,
        notify: true,
      },
    ) => {
      ...
      try {
        loader.setLoading(true);
        const { id } = isLocalDraft(state.shipment.id)
  ...
  React.useEffect(() => {
    if (
      ...
      updateRate &&
      hasRateRequirements(state.shipment)
    ) {
      setUpdateRate(false);
