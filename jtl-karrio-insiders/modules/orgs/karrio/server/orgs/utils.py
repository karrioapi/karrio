...
def owner_required(instance, context) -> models.Organization:
    organization = (
        instance.organization if hasattr(instance, "organization") else instance
    )
    if not organization.organization_users.filter(
        is_owner=True, user__id=context.request.user.id
    ).exists():
        raise exceptions.PermissionDenied()
...
def send_invitation_emails(
    ...
    roles: typing.List[OrganizationUserRole] = [],
    is_owner: bool = False,
):
    @utils.tenant_aware
    @transaction.atomic
    ...
    # Use synchronous execution for reliable invitation creation
    # The concurrent version was causing issues in tests
    filtered_emails = [email for email in emails if email not in exclusion]
    results = []
    for email in filtered_emails:
        try:
            result = action(email, organization, schema=schema)
...
def send_invitation(
    ...
    text = render_to_string("karrio/invitation_email.txt", context)
