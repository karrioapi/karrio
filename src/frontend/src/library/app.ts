...
import { References, Shipment, Purplship, Address, Parcel, Customs, ShipmentData, ShipmentPurchaseData, ShipmentDataLabelTypeEnum, AddressData, ParcelData, CustomsData } from '@/api';
...
class AppState {
    ...
    private async fetchReferences() {
        const references = await this.purplship.API.data();
        this.references$.next(references);
        return references;
    }

    public async retrieveShipment(id: string) {
        return handleFailure(
            this.purplship.shipments.retrieve({ id })
        );
    }

    public async fetchRates(shipment: Shipment | ShipmentData) {
        return handleFailure((async () => {
            if ((shipment as Shipment).id !== undefined) {
                const response = this.purplship.shipments.rates({ id: (shipment as Shipment).id as string });
                response.then(() => this.shipments$.next(DEFAULT_PAGINATED_RESULT as any));
                return response;
            } else {
                const response = this.purplship.shipments.create({ data: (shipment as ShipmentData) });
                response.then(() => this.shipments$.next(DEFAULT_PAGINATED_RESULT as any));
                return response;
            }
        })());
    ...
    public async voidLabel(shipment: Shipment) {
        const response = handleFailure(
            this.purplship.shipments.cancel({ id: shipment.id as string })
        );
        response.then(() => this.fetchShipments());
        return response;
    }

    public async setOptions(shipment_id: string, data: {}) {
        const response = handleFailure(
            this.purplship.shipments.setOptions({ data, id: shipment_id })
        );
        response.then(() => this.shipments$.next(DEFAULT_PAGINATED_RESULT as any));
        return response;
    }

    public async saveAddress(data: AddressData) {
        const response = handleFailure(
            this.purplship.addresses.create({ data })
        );
        return response;
    }
    ...
    public async updateAddress(address: Address) {
        const response = handleFailure(
            this.purplship.addresses.update({ data: address, id: address.id } as any)
        );
        response.then(() => this.shipments$.next(DEFAULT_PAGINATED_RESULT as any));
        return response;
    }

    public async saveParcel(data: ParcelData) {
        const response = handleFailure(
            this.purplship.parcels.create({ data })
        );
        return response;
    }
    ...
    public async updateParcel(parcel: Parcel) {
        const response = handleFailure(
            this.purplship.parcels.update({ data: parcel, id: parcel.id } as any)
        );
        response.then(() => this.shipments$.next(DEFAULT_PAGINATED_RESULT as any));
        return response;
    }

    public async removeParcel(id: string) {
        const response = handleFailure(
            this.purplship.parcels.discard({ id })
        );
        return response;
    }
    ...
    public async addCustoms(shipment_id: string, customs: Customs) {
        const response = handleFailure(
            this.purplship.shipments.addCustoms({ data: customs, id: shipment_id } as any)
        );
        response.then(() => this.shipments$.next(DEFAULT_PAGINATED_RESULT as any));
        return response;
    }

    public async saveCustoms(data: CustomsData) {
        const response = handleFailure(
            this.purplship.customs.create({ data })
        );
        return response;
    }
    ...
    public async updateCustoms(customs: Customs) {
        const response = handleFailure(
            this.purplship.customs.update({ data: customs, id: customs.id } as any)
        );
        response.then(() => this.shipments$.next(DEFAULT_PAGINATED_RESULT as any));
        return response;
    }

    public async removeCustoms(id: string) {
        const response = handleFailure(
            this.purplship.customs.discard({ id })
        );
        response.then(() => this.shipments$.next(DEFAULT_PAGINATED_RESULT as any));
        return response;
    }
    ...
    public async createTracker(carrier: Connection, tracking_number: string) {
        const response = handleFailure(this.purplship.trackers.retrieve({ 
            carrierName: carrier.carrier_name.toString(),
