...
import { Collection, Connection, NotificationType } from '@/library/types';
...
const ConnectProviderModal: React.FC<ConnectProviderModalComponent> = ({ children, connection, className }) => {
    ...
    const handleSubmit = async (evt: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            setIsDisabled(true);
            const data = {
                carrier_name: payload.carrier_name as CarrierSettingsCarrierNameEnum,
                        ...
                        <SelectField value={payload.carrier_name} onChange={handleOnChange("carrier_name")} disabled={!isNew} key={`select-${key}`} className="is-fullwidth" required>
                            ...
                            <Reference.Consumer>
                                {(ref) => (Object.values(ref || {}).length > 0) && Object.keys(ref.carriers).map(carrier => (
                                    <option key={carrier} value={carrier}>{(ref.carriers as Collection)[carrier]}</option>
                                ))}
                            </Reference.Consumer>
...
function hasProperty(carrier_name: CarrierSettingsCarrierNameEnum, property: string): boolean {
    ...
    return ({
        ...
        [CarrierSettingsCarrierNameEnum.FedexExpress]: ["carrier_id", "test", "user_key", "password", "meter_number", "account_number"],
        ...
        [CarrierSettingsCarrierNameEnum.UpsPackage]: ["carrier_id", "test", "username", "password", "access_license_number", "account_number"],
