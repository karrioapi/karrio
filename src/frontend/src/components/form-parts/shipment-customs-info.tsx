...
const ShipmentCustomsInfo: React.FC<ShipmentCustomsInfoComponent> = ({ shipment, update }) => {
    const [customs, dispatch] = useReducer(reducer, shipment?.customs, () => shipment?.customs);
    ...
    const applyOptOut = async (e: ChangeEvent<any>) => {
        e.preventDefault();
        try {
            if (!isNone(shipment.id) && !isNone(customs.id)) {
    ...
    return (
        <>
            ...
            {isNone(customs) && <div>
                <ButtonField className="is-primary" fieldClass="has-text-centered mt-3" onClick={applyOptOut} disabled={isNone(customs)}>
                    <span>Save</span>
            ...
            {!isNone(customs) && <form className="px-1 py-2" onSubmit={handleSubmit} ref={form}>
                ...
                <div className="columns is-multiline mb-0 pt-2">
                    ...
                    <User.Consumer>
                        {(user) => (
                            <InputField label="Signed By" defaultValue={(customs?.signer || user.full_name) as string} onChange={handleChange} name="signer" fieldClass="column mb-0 is-12 px-2 py-2" required />
                        )}
                    </User.Consumer>
                ...
                <ButtonField type="submit" className="is-primary" fieldClass="has-text-centered mt-3" disabled={deepEqual(shipment?.customs || {}, customs)}>
