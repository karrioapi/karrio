...
const ParcelForm: React.FC<ParcelFormComponent> = ({ value, shipment, update, children }) => {
    ...
    const [presets, setPresets] = useState<PresetCollection>(Ref?.package_presets as PresetCollection);
    ...
    useEffect(() => {
        if (!isNone(Ref?.package_presets)) {
            setPresets(Ref.package_presets as PresetCollection);
            const preset = findPreset(Ref.package_presets as PresetCollection, parcel.package_preset) as Partial<Parcel>;
            if (!isNone(preset)) {
                setDimension(formatDimension(preset));
    ...
    useEffect(() => {
        ...
        if (!isNone(Ref?.package_presets) && shipment !== undefined && isNone(shipment.id) && !isNone(Defaults?.parcel) && !deepEqual(Defaults.parcel, parcel)) {
            const preset = findPreset(Ref.package_presets as PresetCollection, Defaults.parcel?.package_preset as string) as Partial<Parcel>;
            if (!isNone(preset)) {
                setDimension(formatDimension(preset));
