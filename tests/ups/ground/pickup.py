...
class TestUPSPickup(unittest.TestCase):
    ...
    def test_create_pickup_request(self):
        request = freight_gateway.mapper.create_pickup_request(self.PickupRequest)
    ...
    def test_update_pickup_request(self):
        request = freight_gateway.mapper.create_pickup_update_request(self.PickupUpdateRequest)
        pipeline = request.serialize()
        rate_request = pipeline["rate"]()
        create_request = pipeline["create"](PickupRateResponseXML)
    ...
    def test_create_pickup(self):
        with patch("purplship.mappers.ups_ground.proxy.http") as mocks:
            mocks.side_effect = [PickupRateResponseXML, PickupResponseXML]
            purplship.Pickup.schedule(self.PickupRequest).from_(freight_gateway)

            rate_call, create_call = mocks.call_args_list
            self.assertEqual(
                rate_call[1]["url"],
                f"{freight_gateway.settings.server_url}/PickupFreight",
            )
            self.assertEqual(
                create_call[1]["url"],
                f"{freight_gateway.settings.server_url}/PickupFreight",
    ...
    def test_update_pickup(self):
        with patch("purplship.mappers.ups_ground.proxy.http") as mocks:
            ...
            purplship.Pickup.update(self.PickupUpdateRequest).from_(freight_gateway)
    ...
    def test_parse_pickup_reply(self):
        with patch("purplship.mappers.ups_ground.proxy.http") as mocks:
            mocks.side_effect = [PickupRateResponseXML, PickupResponseXML]
            parsed_response = (
                purplship.Pickup.schedule(self.PickupRequest).from_(freight_gateway).parse()
            )
    ...
    def test_parse_pickup_cancel_reply(self):
        with patch("purplship.mappers.ups_ground.proxy.http") as mock:
            ...
            parsed_response = (
                purplship.Pickup.cancel(self.PickupCancelRequest).from_(freight_gateway).parse()
            )
