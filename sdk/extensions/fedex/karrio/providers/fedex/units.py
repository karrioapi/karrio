...
class ShippingOption(utils.Enum):
    fedex_blind_shipment = utils.OptionEnum("BLIND_SHIPMENT")
    fedex_broker_select_option = utils.OptionEnum("BROKER_SELECT_OPTION")
    fedex_call_before_delivery = utils.OptionEnum("CALL_BEFORE_DELIVERY")
    fedex_cod = utils.OptionEnum("COD", float)
    fedex_cod_remittance = utils.OptionEnum("COD_REMITTANCE")
    fedex_custom_delivery_window = utils.OptionEnum("CUSTOM_DELIVERY_WINDOW")
    fedex_cut_flowers = utils.OptionEnum("CUT_FLOWERS")
    fedex_dangerous_goods = utils.OptionEnum("DANGEROUS_GOODS")
    fedex_delivery_on_invoice_acceptance = utils.OptionEnum(
        "DELIVERY_ON_INVOICE_ACCEPTANCE"
    )
    fedex_detention = utils.OptionEnum("DETENTION")
    fedex_do_not_break_down_pallets = utils.OptionEnum("DO_NOT_BREAK_DOWN_PALLETS")
    fedex_do_not_stack_pallets = utils.OptionEnum("DO_NOT_STACK_PALLETS")
    fedex_dry_ice = utils.OptionEnum("DRY_ICE")
    fedex_east_coast_special = utils.OptionEnum("EAST_COAST_SPECIAL")
    fedex_electronic_trade_documents = utils.OptionEnum("ELECTRONIC_TRADE_DOCUMENTS")
    fedex_event_notification = utils.OptionEnum("EVENT_NOTIFICATION")
    fedex_exclude_from_consolidation = utils.OptionEnum("EXCLUDE_FROM_CONSOLIDATION")
    fedex_exclusive_use = utils.OptionEnum("EXCLUSIVE_USE")
    fedex_exhibition_delivery = utils.OptionEnum("EXHIBITION_DELIVERY")
    fedex_exhibition_pickup = utils.OptionEnum("EXHIBITION_PICKUP")
    fedex_expedited_alternate_delivery_route = utils.OptionEnum(
        "EXPEDITED_ALTERNATE_DELIVERY_ROUTE"
    )
    fedex_expedited_one_day_earlier = utils.OptionEnum("EXPEDITED_ONE_DAY_EARLIER")
    ...
    fedex_freight_to_collect = utils.OptionEnum("FREIGHT_TO_COLLECT")
    fedex_future_day_shipment = utils.OptionEnum("FUTURE_DAY_SHIPMENT")
    fedex_hold_at_location = utils.OptionEnum("HOLD_AT_LOCATION")
    fedex_holiday_delivery = utils.OptionEnum("HOLIDAY_DELIVERY")
    fedex_holiday_guarantee = utils.OptionEnum("HOLIDAY_GUARANTEE")
    fedex_home_delivery_premium = utils.OptionEnum("HOME_DELIVERY_PREMIUM")
    fedex_inside_delivery = utils.OptionEnum("INSIDE_DELIVERY")
    fedex_inside_pickup = utils.OptionEnum("INSIDE_PICKUP")
    fedex_international_controlled_export_service = utils.OptionEnum(
        "INTERNATIONAL_CONTROLLED_EXPORT_SERVICE"
    )
    fedex_international_mail_service = utils.OptionEnum("INTERNATIONAL_MAIL_SERVICE")
    fedex_international_traffic_in_arms_regulations = utils.OptionEnum(
        "INTERNATIONAL_TRAFFIC_IN_ARMS_REGULATIONS"
    )
    fedex_liftgate_delivery = utils.OptionEnum("LIFTGATE_DELIVERY")
    fedex_liftgate_pickup = utils.OptionEnum("LIFTGATE_PICKUP")
    fedex_limited_access_delivery = utils.OptionEnum("LIMITED_ACCESS_DELIVERY")
    fedex_limited_access_pickup = utils.OptionEnum("LIMITED_ACCESS_PICKUP")
    fedex_marking_or_tagging = utils.OptionEnum("MARKING_OR_TAGGING")
    fedex_non_business_time = utils.OptionEnum("NON_BUSINESS_TIME")
    fedex_pallet_shrinkwrap = utils.OptionEnum("PALLET_SHRINKWRAP")
    fedex_pallet_weight_allowance = utils.OptionEnum("PALLET_WEIGHT_ALLOWANCE")
    fedex_pallets_provided = utils.OptionEnum("PALLETS_PROVIDED")
    fedex_pending_complete = utils.OptionEnum("PENDING_COMPLETE")
    fedex_pending_shipment = utils.OptionEnum("PENDING_SHIPMENT")
    fedex_permit = utils.OptionEnum("PERMIT")
    fedex_pharmacy_delivery = utils.OptionEnum("PHARMACY_DELIVERY")
    fedex_poison = utils.OptionEnum("POISON")
    fedex_port_delivery = utils.OptionEnum("PORT_DELIVERY")
    fedex_port_pickup = utils.OptionEnum("PORT_PICKUP")
    fedex_pre_delivery_notification = utils.OptionEnum("PRE_DELIVERY_NOTIFICATION")
    fedex_pre_eig_processing = utils.OptionEnum("PRE_EIG_PROCESSING")
    fedex_pre_multiplier_processing = utils.OptionEnum("PRE_MULTIPLIER_PROCESSING")
    fedex_protection_from_freezing = utils.OptionEnum("PROTECTION_FROM_FREEZING")
    fedex_regional_mall_delivery = utils.OptionEnum("REGIONAL_MALL_DELIVERY")
    fedex_regional_mall_pickup = utils.OptionEnum("REGIONAL_MALL_PICKUP")
    fedex_return_shipment = utils.OptionEnum("RETURN_SHIPMENT")
    fedex_returns_clearance = utils.OptionEnum("RETURNS_CLEARANCE")
    fedex_returns_clearance_special_routing_required = utils.OptionEnum(
        "RETURNS_CLEARANCE_SPECIAL_ROUTING_REQUIRED"
    )
    fedex_saturday_delivery = utils.OptionEnum("SATURDAY_DELIVERY")
    fedex_saturday_pickup = utils.OptionEnum("SATURDAY_PICKUP")
    fedex_shipment_assembly = utils.OptionEnum("SHIPMENT_ASSEMBLY")
    fedex_sort_and_segregate = utils.OptionEnum("SORT_AND_SEGREGATE")
    fedex_special_delivery = utils.OptionEnum("SPECIAL_DELIVERY")
    fedex_special_equipment = utils.OptionEnum("SPECIAL_EQUIPMENT")
    fedex_storage = utils.OptionEnum("STORAGE")
    fedex_sunday_delivery = utils.OptionEnum("SUNDAY_DELIVERY")
    fedex_third_party_consignee = utils.OptionEnum("THIRD_PARTY_CONSIGNEE")
    fedex_top_load = utils.OptionEnum("TOP_LOAD")
    fedex_usps_delivery = utils.OptionEnum("USPS_DELIVERY")
    fedex_usps_pickup = utils.OptionEnum("USPS_PICKUP")
    fedex_weighing = utils.OptionEnum("WEIGHING")
