...
#   ('--no-namespace-defs', '')
#   ('-o', './usps_lib/carrier_pickup_inquiry_response.py')
#
# Command line arguments:
#   ./schemas/CarrierPickupInquiryResponse.xsd
#
# Command line:
#   /Users/danielkobina/Workspace/project/purplship-carriers/.venv/purplship-carriers/bin/generateDS --no-namespace-defs -o "./usps_lib/carrier_pickup_inquiry_response.py" ./schemas/CarrierPickupInquiryResponse.xsd
#
...
class CarrierPickupInquiryResponse(GeneratedsSuper):
    ...
    def export(self, outfile, level, namespaceprefix_='', namespacedef_='', name_='CarrierPickupInquiryResponse', pretty_print=True):
        ...
        self.exportAttributes(outfile, level, already_processed, namespaceprefix_, name_='CarrierPickupInquiryResponse')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespaceprefix_, namespacedef_, name_='CarrierPickupInquiryResponse', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespaceprefix_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespaceprefix_='', name_='CarrierPickupInquiryResponse'):
        pass
    def exportChildren(self, outfile, level, namespaceprefix_='', namespacedef_='', name_='CarrierPickupInquiryResponse', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
