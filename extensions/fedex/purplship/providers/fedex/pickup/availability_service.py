...
from purplship.core.utils import Serializable, export, create_envelope, apply_namespaceprefix, Envelope, concat_str
...
def pickup_availability_request(payload: PickupRequest, settings: Settings) -> Serializable[PickupAvailabilityRequest]:
    request_type = next(
        (PickupRequestType[s] for s in PickupRequestType.__members__ if s.name == payload.service),
    ...
    request = PickupAvailabilityRequest(
        ...
        PickupRequestType=request_type,
