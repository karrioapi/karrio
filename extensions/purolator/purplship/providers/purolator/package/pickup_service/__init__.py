...
from purplship.providers.purolator.package.pickup_service.validate_pickup import validate_pickup_request
from purplship.providers.purolator.package.pickup_service.void_pickup import void_pickup_request, parse_void_pickup_reply
from purplship.providers.purolator.package.pickup_service.modify_pickup import modify_pickup_request, parse_modify_pickup_reply
from purplship.providers.purolator.package.pickup_service.schedule_pickup import schedule_pickup_request, parse_schedule_pickup_reply


def schedule_pickup_pipeline(payload: PickupRequest, settings: Settings) -> Serializable[Pipeline]:
    """
...
def modify_pickup_pipeline(payload: PickupUpdateRequest, settings: Settings) -> Serializable[Pipeline]:
    """
...
def _validate_pickup(payload: Union[PickupRequest, PickupUpdateRequest], settings: Settings):
...
def _schedule_pickup(validation_response: str, payload: PickupRequest, settings: Settings):
    ...
    data = (
        schedule_pickup_request(payload, settings) if len(errors) == 0 else None
    )
...
def _modify_pickup(validation_response: str, payload: PickupUpdateRequest, settings: Settings):
    errors = parse_error_response(to_xml(validation_response), settings)
    data = (
        modify_pickup_request(payload, settings) if len(errors) == 0 else None
    )
