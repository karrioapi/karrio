...
def shipment_request(
    ...
    service = FreightServiceCode.map(payload.service).value_or_key
    ship_from = CompleteAddress.map(payload.shipper)
    ship_to = CompleteAddress.map(payload.shipper)

    payment_account_number = getattr(payload.payment, 'account_number', settings.account_number)
    payment_address = getattr(payload.payment, 'address', None)
    mps_packaging = (
        FreightPackagingType.your_packaging.value if len(packages) > 1 else None
    )
    payer = dict(
    ...
    request = create_envelope(
        ...
        body_content=FreightShipRequest(
            ...
            Shipment=ShipmentType(
                ...
                PaymentInformation=PaymentInformationType(
                    Payer=PayerType(
                        ...
                        Address=AddressType(
                            ...
                            ResidentialAddressIndicator=("" if payer.residential else None)
                        ),
                ...
                Commodity=[
                    CommodityType(
                        ...
                        PackagingType=ShipCodeDescriptionType(
                            Code=(mps_packaging or FreightPackagingType[commodity.packaging_type or "your_packaging"].value),
                        ),
                        DangerousGoodsIndicator=("" if options.dangerous_good else None),
                        CommodityValue=None,
                        FreightClass=FreightClass.map(options.freight_class or 50).value,
                        NMFCCommodity=None,
                ...
                Reference=(
                    [ReferenceType(
                        Number=ReferenceNumberType(Code="OTHER", Value=payload.reference)
                    )]
                ...
                ShipmentServiceOptions=(
                    ShipmentServiceOptionsType(
                        EMailInformation=(
                            ...
                            if any([options.email_notification_to, payload.recipient.email])
                            else None
                        ),
                        COD=(
                            CODType(
                                ...
                                RemitTo=RemitToType(
                                    ...
                                    Phone=(
                                        ...
                                        if payer.phone_number else None
                                    ),
                                    FaxNumber=None,
                                    EMailAddress=payer.email
                                )
                    ...
                    if any([
                        options.cash_on_delivery, options.email_notification,
                        options.dangerous_good, options.declared_value,
                    ]) else None
                ),
                PickupRequest=None,
                Documents=DocumentsType(
                    Image=ImageType(
                        ...
                        PrintFormat=ShipCodeDescriptionType(Code="02" if "ZPL" in label_format else "01"),
