...
def parse_rate_response(
    ...
    rates: List[RateDetails] = [
        _extract_package_rate(rate, settings)
        for rate in rate_replys
    ]
...
def rate_request(
    ...
    service = Services(payload.services, FreightServiceCode).first
    ship_from = CompleteAddress.map(payload.shipper)
    ship_to = CompleteAddress.map(payload.shipper)

    mps_packaging = FreightPackagingType.ups_freight_other.value if len(packages) > 1 else None
            ...
            Commodity=[
                CommodityType(
                    ...
                    PackagingType=RateCodeDescriptionType(
                        Code=(
                            mps_packaging or FreightPackagingType[
