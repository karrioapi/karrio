...
function reducer(
  state: Partial<PartialWorkflowActionMutationInput>,
  {
    name,
    value,
  }: {
    ...
    value:
      | string
      | boolean
      | Partial<PartialWorkflowActionMutationInput>
      | string[];
  },
): PartialWorkflowActionMutationInput {
  switch (name) {
...
export const ActionModalEditor = ({
  trigger,
  ...args
}: ModalFormProps<ActionModalEditorProps>): JSX.Element => {
  const modal = useModal();

  const Component = (props: ActionModalEditorProps): JSX.Element => {
    const { action: defaultValue = {}, header, onSubmit } = props;
    const loader = useLoader();
    const { close } = useModal();
    const notifier = useNotifier();
    const [action, dispatch] = React.useReducer(
      reducer,
      defaultValue,
      () => defaultValue,
    );
          ...
          <>
              ...
              {Array.from(new Set(Object.values(AutomationActionType))).map(
                (unit) => (
                  <option
                    key={unit}
                    value={unit}
                    disabled={unit == AutomationActionType.function_call}
                  >
                    {formatRef(unit)}
                  </option>
                ),
            ...
            {action.action_type == AutomationActionType.http_request && (
              <div className="column mb-0 p-0">
                ...
                >
                  {Array.from(
                    new Set(Object.values(AutomationHTTPContentType)),
                  ).map((unit) => (
                    <option key={unit} value={unit}>
                      {formatRef(unit)}
                    </option>
                ...
                <div className="column mb-0 p-0 control">
                  ...
                  <div className="card is-radiusless">
                    ...
                    <CodeMirror
                      ...
                      onChange={(value) =>
                        dispatch({ name: "header_template", value })
                      }
                    />
                  </div>
                </div>
                ...
                >
                  {Array.from(
                    new Set(Object.values(AutomationParametersType)),
                  ).map((unit) => (
                    <option key={unit} value={unit}>
                      {formatRef(unit)}
                    </option>
            ...
            {/* conditional options */}
            {action.action_type == AutomationActionType.conditional && (
              <div className="column mb-0 p-0">
                {/* parameters_template */}
                <div className="column mb-0 p-0 control">
