...
class RateSheetPricingConfig:
    ...
    `use_static_rates`: when True, rate quotes for carriers attached to
    this sheet skip the live carrier API and use the rate-sheet resolver
    (`Rates.resolve`) instead. Drops one carrier round-trip per quote
    and pins the merchant-facing price to the sheet for billing
    consistency. Default `True` — sheet-attached carriers prefer the
    sheet by default; opt back to live with `use_static_rates: false`
    when carrier-side validation at quote time is needed.
    """

    excluded_markup_ids: list = attr.Factory(list)
    sort_order: int | None = None
    use_static_rates: bool = True
    ...
    def from_dict(cls, data) -> RateSheetPricingConfig:
        ...
        # Only override the class default when the key is explicitly
        # present — preserves the True default for sheets that pre-date
        # the field entirely.
        if "use_static_rates" in data:
            kwargs["use_static_rates"] = bool(data["use_static_rates"])
        return cls(**kwargs)
