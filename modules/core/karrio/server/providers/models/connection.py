...
class CarrierConnectionManager(Manager):
    def get_queryset(self):
        return super().get_queryset().filter(is_system=False)


class SystemCarrierConnectionManager(models.Manager):
    def get_queryset(self):
        return (
            super()
            .get_queryset()
...
class CarrierConnection(core.OwnedEntity):
    ...
    id = models.CharField(
        max_length=50,
        primary_key=True,
        default=functools.partial(core.uuid, prefix="conn_"),
        editable=False,
    )
    ...
    objects = Manager()
    user_connections = CarrierConnectionManager()
    system_connections = SystemCarrierConnectionManager()
    ...
    def object_type(self):
        return "carrier-connetion"

    @property
    def carrier_name(self):
        # resolve carrier_name
        pass
    ...
    def gateway(self) -> gateway.Gateway:
        ...
        _carrier_name = self.carrier_name

        return karrio.gateway[_carrier_name].create(
            {**self.credentials, **self.data},
            _tracer,
            _cache,
        )
    ...
    def carrier_display_name(self):
        ...
        return references.collect_references()["carriers"].get(self.carrier_name)
