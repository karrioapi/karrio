...
class ServiceLevel(core.OwnedEntity):
    ...
    # ─────────────────────────────────────────────────────────────────
    # CARRIER OPTIONS (SDK-keyed, e.g. dhl_parcel_de_gogreen_plus)
    # ─────────────────────────────────────────────────────────────────
    carrier_options = models.JSONField(
        blank=True,
        null=True,
        default=core.field_default({}),
    ...
    # ─────────────────────────────────────────────────────────────────
    # CURATED TAGS (registry-driven; see providers/tags.py)
    # ─────────────────────────────────────────────────────────────────
    tags = models.JSONField(
        ...
        help_text=(
            ...
            "{recommended, recommendation_category, recommendation_type, display_priority, surface_visibility}"
        ),
    )
    ...
    def clean(self):
        # Validate ``tags`` against the registry — drops unknown keys / bad values
        # rather than persisting them. GraphQL/admin write paths also call
        # ``providers.tags.validate_tags`` to surface warnings; this is a final
        # safety net for Django-admin saves that bypass the GraphQL surface.
        from karrio.server.providers.tags import validate_tags

        cleaned, _warnings = validate_tags(self.tags)
        self.tags = cleaned
        super().clean()
