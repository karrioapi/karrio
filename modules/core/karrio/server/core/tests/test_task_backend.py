...
class TestTaskExecutionScope(unittest.TestCase):
    """Tests for TaskBackend.task_execution_scope()."""

    def _record_signals(self):
        """Connect recorders to request_started/request_finished for the duration of a test."""
        from django.core.signals import request_finished, request_started

        started: list = []
        finished: list = []

        def on_started(sender, **_):
            started.append(sender)

        def on_finished(sender, **_):
            finished.append(sender)

        request_started.connect(on_started)
        request_finished.connect(on_finished)
        self.addCleanup(request_started.disconnect, on_started)
        self.addCleanup(request_finished.disconnect, on_finished)
        return started, finished

    def test_scope_fires_request_signals(self):
        """The default scope emits request_started on entry and request_finished on exit."""
        backend = ImmediateBackend()
        started, finished = self._record_signals()

        with backend.task_execution_scope():
    ...
    def test_scope_fires_request_finished_even_when_body_raises(self):
        """request_finished must run in a finally block — does not depend on success."""
        backend = ImmediateBackend()
        started, finished = self._record_signals()

        with self.assertRaises(RuntimeError), backend.task_execution_scope():
            raise RuntimeError("boom")

        self.assertEqual(len(started), 1)
        self.assertEqual(len(finished), 1)

    def test_scope_propagates_handler_exception(self):
        """The scope must not swallow exceptions raised inside the body."""
        backend = ImmediateBackend()

        class _SentinelError(Exception):
            pass

        with self.assertRaises(_SentinelError), backend.task_execution_scope():
            raise _SentinelError("must propagate")

    def test_scope_each_invocation_is_independent(self):
        """Two consecutive scopes each fire their own start+finish signals."""
        backend = ImmediateBackend()
        started, finished = self._record_signals()
    ...
    def test_scope_default_works_for_all_backends(self):
        """Concrete TaskBackend subclasses inherit a usable default scope."""
        from karrio.server.core.task_backend import TaskBackend

        backend = ImmediateBackend()
        self.assertIsInstance(backend, TaskBackend)

        cm = backend.task_execution_scope()
        self.assertTrue(hasattr(cm, "__enter__") and hasattr(cm, "__exit__"))


class TestTaskExecutionScopeHealsDeadConnection(TransactionTestCase):
    """Behavioral canary against the production bug this scope exists to fix.

    The mock-based class above proves the request signals fire. This proves
    the end-to-end path works against a real DB: a cached connection that
    was dropped between tasks (simulating Azure's load balancer killing the
    idle socket on a long-lived consumer thread) is healed before the next
    ORM call runs inside the scope. If the scope ever silently regresses to
    a no-op, this test fails — the mock-based ones would still pass green.
    """

    def test_scope_allows_orm_call_after_cached_connection_was_closed(self):
        from django.contrib.auth.models import Permission
        from django.db import connection

        # Touch DB to populate the thread-local cached connection, then drop
        # it — same shape as the production failure mode.
        Permission.objects.count()
        connection.close()

        backend = ImmediateBackend()
        with backend.task_execution_scope():
            # Would raise InterfaceError if the scope no-op'd and left the
            # closed connection cached on this thread.
            Permission.objects.count()
