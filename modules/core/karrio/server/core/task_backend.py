...
class TaskBackend(abc.ABC):
    ...
    def enqueue(
        ...
        *,
        ...
        Args:
            ...
            confirmed: Block until the broker confirms the send (Service Bus
                only — used for confirm-before-ack producers). Other backends
                are already synchronous/local, so this is a no-op for them.
            trace: Propagated trace headers (from
                ``telemetry.get_trace_headers()``) so the consumed task
                continues the enqueuer's trace. Backends that run inline
                (immediate) ignore it — the trace is already current.
    ...
    def task_execution_scope(self) -> typing.Iterator[None]:
        """Context manager that brackets the execution of a single task.

        Backends running tasks on long-lived worker threads should wrap each
        handler invocation in this scope. The default emits Django's
        ``request_started`` / ``request_finished`` signals so each task gets
        a request-equivalent scope: stale DB connections are closed (Django's
        own cleanup hangs off ``request_finished``) and any framework code
        that normally runs on request boundaries — e.g. Constance -> Django
        settings sync wired to ``request_started`` in
        ``karrio.server.core.signals`` — fires once per task. Receivers of
        these signals should therefore be cheap and non-request-specific.
        """
...
class BackgroundTask:
    ...
    def __call__(self, *args, **kwargs) -> str:
        ...
        from karrio.server.core.telemetry import get_trace_headers
    ...
    def enqueue_confirmed(self, *args, **kwargs) -> str:
        """Enqueue and block until the broker confirms the send (Service Bus).

        For confirm-before-ack producers: the caller records success only after
        this returns. Other backends are already synchronous/local, so this
        behaves like a normal enqueue. Run it off the request thread so the
        broker round-trip costs no latency.
        """
        from karrio.server.core.telemetry import get_trace_headers
