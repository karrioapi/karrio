...
# =============================================================================
# Distributed trace propagation across the task queue
# =============================================================================
#
# These helpers let an enqueued task continue the trace of the code that
# enqueued it, so async fan-outs (e.g. a worker consuming an event and
# enqueuing downstream publishes) show up as a single chained trace instead
# of disconnected ones. The producer captures the active trace with
# ``get_trace_headers()`` and stores it in the message; the consumer wraps the
# handler in ``continue_task_trace(...)`` before running it.
#
# Sentry-specific (matches the rest of this module's default provider) and
# fully failsafe — instrumentation must never break task dispatch.


def get_trace_headers() -> dict:
    """Capture the active Sentry trace as propagatable headers.

    Returns ``{"sentry-trace": ..., "baggage": ...}`` for the currently active
    span so an enqueued task can continue the same trace, or ``{}`` when tracing
    is unavailable / no span is active. Never raises.
    """
    ...
    return failsafe(_headers) or {}


@contextlib.contextmanager
def continue_task_trace(headers: dict, name: str, op: str = "queue.task"):
    """Continue a propagated trace for a consumed task.

    Starts a transaction linked to the upstream trace described by ``headers``
    (as produced by :func:`get_trace_headers`) so the task appears chained under
    its enqueuer. A no-op context when no headers were propagated or tracing is
    ...
    try:
        ...
        transaction_cm = sentry_sdk.start_transaction(transaction)
    except Exception:
        transaction_cm = None
