...
def update_settings(current):
    ...
    if values:
        ...
        # Mirror EMAIL_FROM_ADDRESS to OTP_EMAIL_SENDER so the django-otp email
        # plugin uses the same From address as the rest of the application.
        # Without this, OTP challenges fall back to DEFAULT_FROM_EMAIL which is
        # typically not verified on strict SMTP relays (e.g. SendGrid 550).
        with suppress(Exception):
            settings.OTP_EMAIL_SENDER = current.EMAIL_FROM_ADDRESS
        return
