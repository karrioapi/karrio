...
class AuthenticationMiddleware(BaseAuthenticationMiddleware):
    def process_response(self, request, response):
        # `request.org` is a SimpleLazyObject set by the auth class; accessing
        # `.id` triggers the deferred `get_request_org` resolver, which can raise
        # AuthenticationFailed (e.g. a non-member session sending x-tenant-id —
        # the expected state for an ORY user who has not yet called the
        # `onboard_tenant` mutation). The view has already returned by the time
        # we get here, so a raised exception cannot be mapped to a 401 — Django
        # turns it into a 500. Swallow the failure and skip the cookie; any
        # caller that actually needs org context already hit the same check
        # during dispatch via `access_by`, where DRF converts it to a proper 401.
        try:
            if getattr(request, "org", None) is not None:
                org_id = getattr(request.org, "id", None)
                if org_id is not None:
                    response.set_cookie("org_id", org_id)
                    response["X-org-id"] = org_id
