...
class ShipmentFilters(filters.FilterSet):
    ...
    connection_type = filters.CharInFilter(
        method="connection_type_filter",
        help_text="filter by connection type: account, brokered",
    )
    ...
    def connection_type_filter(self, queryset, name, value):
        from django.db.models import Q

        q = Q()
        for ct in value:
            if ct == "brokered":
                q |= Q(carrier__connection_type__in=["brokered", "system"])
            elif ct == "account":
                q |= Q(carrier__connection_type="account")
