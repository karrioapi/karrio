...
class ControlledAccessModel:
    ...
    def access_by(cls: models.Model, context, manager: str = "objects"):
        test_mode = context.get("test_mode") if isinstance(context, dict) else getattr(context, "test_mode", None)
        ...
        if hasattr(cls, "test_mode") and test_mode is not None:
            query = query & models.Q(models.Q(test_mode=test_mode) | models.Q(test_mode__isnull=True))
...
def register_model[T](model: T) -> T:
    def transform(model, transformer):
        return model if transformer is None else pydoc.locate(transformer)(model)
