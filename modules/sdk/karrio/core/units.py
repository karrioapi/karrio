...
class Package:
    ...
    @property
    def weight(self) -> Weight:
        """Return the declared package weight.

        The declared weight (from the parcel or preset) is authoritative.
        Item weights are intentionally ignored — they are informational
        for customs declarations and must not inflate the package weight
        used for rate sheet matching or carrier billing.
        """
        declared = self.parcel.weight or self.preset.weight

        return self._compute_weight(declared)
...
class ShippingOption(utils.Enum):
    ...
    # Generic, carrier-agnostic customer reference printed on the label / carried
    # in the shipping data (e.g. a merchant order number). `universal` surfaces it
    # on every carrier; a connector can alias it onto a native option (see
    # `meta.aliases`). When set it supersedes `ShipmentRequest.reference`. Issue #788.
    shipment_reference = utils.OptionEnum(
        "shipment_reference",
        str,
        help="Customer reference (e.g. order number) carried on the label / shipping data. Supersedes the request reference.",
        meta=dict(category="SHIPMENT", configurable=True, universal=True),
    )
    ...
    # Data-privacy (GDPR) flags — when set, the recipient's contact details are
    # stripped from the request before it reaches the carrier mapper. Enforced
    # universally in `karrio.api.interface.Shipment.create`, so every connector
    # honours them without per-connector handling. Note: with these on, the
    # carrier cannot send package-notification emails/SMS to the recipient.
    suppress_recipient_email = utils.OptionEnum(
        "suppress_recipient_email",
        bool,
        help="Do not transmit the recipient's email address to the carrier.",
        meta=dict(category="DATA_PRIVACY"),
    )
    suppress_recipient_phone = utils.OptionEnum(
        "suppress_recipient_phone",
        bool,
        help="Do not transmit the recipient's phone number to the carrier.",
        meta=dict(category="DATA_PRIVACY"),
    )
...
class ShippingOptions(Options):
    ...
    @property
    def shipment_reference(self) -> utils.OptionEnum:
        return self[ShippingOption.shipment_reference.name]
    ...
    @property
    def suppress_recipient_email(self) -> utils.OptionEnum:
        return self[ShippingOption.suppress_recipient_email.name]

    @property
    def suppress_recipient_phone(self) -> utils.OptionEnum:
        return self[ShippingOption.suppress_recipient_phone.name]
...
class CustomsOption(utils.Enum):
    ...
    ein = utils.OptionEnum("EIN")
    aes = utils.OptionEnum("AES")
    ioss = utils.OptionEnum("IOSS")
    au_gst = utils.OptionEnum("AU_GST")
    ca_gst = utils.OptionEnum("CA_GST")
    nz_gst = utils.OptionEnum("NZ_GST")
    eel_pfc = utils.OptionEnum("EEL_PFC")
    voec_number = utils.OptionEnum("VOEC")
    eori_number = utils.OptionEnum("EORI")
    k_number = utils.OptionEnum("K_NUMBER")
    swiss_vat = utils.OptionEnum("SWISS_VAT")
    nip_number = utils.OptionEnum("NIP_NUMBER")
    vat_registration_number = utils.OptionEnum("VAT")
...
class CustomsOptions(Options):
    ...
    @property
    def ioss(self) -> utils.OptionEnum:
        return self[CustomsOption.ioss.name]

    @property
    def ein(self) -> utils.OptionEnum:
        return self[CustomsOption.ein.name]

    @property
    def voec_number(self) -> utils.OptionEnum:
        return self[CustomsOption.voec_number.name]

    @property
    def swiss_vat(self) -> utils.OptionEnum:
        return self[CustomsOption.swiss_vat.name]

    @property
    def k_number(self) -> utils.OptionEnum:
        return self[CustomsOption.k_number.name]

    @property
    def ca_gst(self) -> utils.OptionEnum:
        return self[CustomsOption.ca_gst.name]

    @property
    def au_gst(self) -> utils.OptionEnum:
        return self[CustomsOption.au_gst.name]

    @property
    def nz_gst(self) -> utils.OptionEnum:
        return self[CustomsOption.nz_gst.name]
