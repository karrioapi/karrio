...
def rate_variant_key(rate: models.RateDetails) -> tuple[str, str]:
    """Identity tuple for a rate's service variant.

    A carrier's API defines one `service_code` per service family (UPS Standard
    is `"11"` whether the package goes Door, Saturday-Door, or Return-Door — the
    variant differs by request options, not by code). Two rates represent the
    same variant iff their `(service_code, service_name)` tuples are equal.

    Callers that group/dedup/match rates across packages or queries MUST key on
    this tuple, not on `rate.service` alone — otherwise multi-piece aggregation
    collapses sibling variants and glues one variant's charges onto another
    variant's meta.

    `service_name` is read from `rate.meta["service_name"]`. Carriers that
    don't emit a service_name fall back to a single-variant identity (empty
    string), which preserves legacy behavior for carriers without variant
    collisions.
    """
    meta = rate.meta if isinstance(rate.meta, dict) else {}
    return (rate.service or "", (meta.get("service_name") or "").strip())
...
def to_multi_piece_rates(package_rates: list[tuple[str, list[models.RateDetails]]]) -> list[models.RateDetails]:
    ...
    for main in main_piece_rates:
        main_key = rate_variant_key(main)
        # Match by `(service_code, service_name)` not bare service_code:
        # multiple ServiceLevels can share a carrier service_code (e.g. UPS
        # Standard / Saturday / Return all carry `ups_standard` because UPS
        # defines one code 11 — saturday is a request option, return is a
        # return-service option). Keying on `rate.service` alone picks the
        # first sibling for every variant and glues its charges onto the
        # wrong meta.
        similar_rates: list[models.RateDetails | None] = [
            next((rate for rate in rates if rate_variant_key(rate) == main_key), None) for _, rates in package_rates
        ]
