...
def suppress_recipient_contact(payload):
    """Strip the recipient's contact details (GDPR / data-privacy toggles).

    Applied to any carrier-bound request that carries a `recipient` Address and
    an `options` dict — both shipment AND rate requests — so the recipient's
    email/phone are never transmitted to a carrier, on any leg, when the
    `suppress_recipient_email` / `suppress_recipient_phone` options are set.
    Mutates the (already-copied) request payload in place; the caller's stored
    data is untouched.
    """
    ...
    options = units.ShippingOptions(getattr(payload, "options", None) or {})
    if options.suppress_recipient_email.state:
        recipient.email = None
    if options.suppress_recipient_phone.state:
...
class Location:
    """The unified Location API fluent interface"""

    @staticmethod
    def search(args: models.LocationRequest | dict) -> IRequestFrom:
        """Search a carrier for locations (depots, parcel shops, lockers, pickup points)

        Args:
            args (Union[LocationRequest, dict]): the location search request payload

        Returns:
            IRequestFrom: a lazy request dataclass instance
        """
        logger.debug("Searching locations", payload=lib.to_dict(args))
        payload = lib.to_object(models.LocationRequest, lib.to_dict(args))

        def action(gateway: gateway.Gateway) -> IDeserialize:
            is_valid, abortion = check_operation(gateway, "get_locations")
            if not is_valid:
                return abortion

            request: lib.Serializable = gateway.mapper.create_location_request(payload)
            response: lib.Deserializable = gateway.proxy.get_locations(request)

            @fail_safe(gateway)
            def deserialize():
                return gateway.mapper.parse_location_response(response)
...
class Rating:
    ...
    def fetch(args: models.RateRequest | dict) -> IRequestFromMany:
        ...
        # Data-privacy (GDPR) suppression — the live-rate leg of a label purchase
        # also hits carrier APIs, and several rate mappers transmit recipient
        # email/phone. Strip them here too so the toggles hold on every carrier leg.
        suppress_recipient_contact(payload)
...
class Shipment:
    ...
    def create(args: models.ShipmentRequest | dict) -> IRequestFrom:
        ...
        # Data-privacy (GDPR) suppression — strip the recipient's contact details
        # before any carrier mapper runs, so every connector honours the toggles
        # uniformly. Applied to `payload.recipient` (the end customer); the
        # `is_return` branch below derives its swapped party from this same object,
        # keeping the customer's PII suppressed regardless of shipment direction.
        suppress_recipient_contact(payload)
