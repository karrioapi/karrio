...
class Proxy(abc.ABC):
    ...
    def get_locations(self, request: lib.Serializable) -> lib.Deserializable:
        """Send a request to search carrier locations (depots, parcel shops, lockers, pickup points)

        Args:
            request (Serializable): a carrier specific serializable request data type

        Returns:
            Deserializable: a Deserializable location response (xml, json, text...)

        Raises:
            MethodNotSupportedError: Is raised when the carrier integration does not implement this method
        """
        raise errors.MethodNotSupportedError(self.__class__.get_locations.__name__, self.settings.carrier_name)
