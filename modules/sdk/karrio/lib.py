...
NoOpTelemetry = utils.NoOpTelemetry
SpanContext = utils.SpanContext
NoOpSpanContext = utils.NoOpSpanContext
get_default_telemetry = utils.get_default_telemetry
Cache = utils.Cache
...
def rate_variant_key(rate: models.RateDetails) -> tuple[str, str]:
    """Identity tuple `(service_code, service_name)` for a rate variant.

    The canonical way to compare two emitted rates for "same service" identity
    when carrier codes can be shared across variants (UPS Standard / Saturday /
    Return all carry `ups_standard` because UPS defines one code 11 — saturday
    is a request option, return is a return-service option). Use this anywhere
    rates are grouped, deduped, or matched across packages/queries.
    """
    return utils.rate_variant_key(rate)
...
def authenticated_request(
    token_manager: typing.Any,
    *,
    auth_header: typing.Callable[[str], dict] | None = None,
    retry_on_statuses: tuple[int, ...] = (401, 403),
    **kwargs,
) -> str:
    """HTTP request with bearer-token auth and one retry on auth rejection.

    Wraps :func:`request` to inject an ``Authorization`` header sourced from
    ``token_manager.get_state()``. If the response status matches
    ``retry_on_statuses`` (default ``(401, 403)``) on the first attempt, the
    cached token is dropped via ``token_manager.invalidate()`` and the request
    is retried once with a fresh credential. The second response (whether
    success or error body) is returned unchanged so existing
    ``parse_*_response`` parsers handle the carrier's error envelope.

    Use this from carrier proxies whose upstream may revoke or rotate tokens
    out-of-band — the cache-version / expiry-buffer mechanism alone does not
    cover that case. For carriers using HTTP Basic or other non-bearer
    schemes, pass ``auth_header=lambda tok: {...}``.

    Example — bearer scheme (default)::
    ...
    Example — non-bearer scheme (HTTP Basic, custom headers, etc.)::
    ...
    Args:
        token_manager: Yields the current token via ``get_state()`` and drops
            the cached value via ``invalidate()`` on auth rejection.
        auth_header: Optional callable mapping ``token -> headers`` dict for
            non-bearer schemes. Defaults to
            ``{"Authorization": f"Bearer {token}"}``.
        retry_on_statuses: HTTP status codes treated as token rejection.
            Default ``(401, 403)``.
        **kwargs: Forwarded to :func:`request` (``url``, ``method``, ``data``,
            ``headers``, ``trace``, ``decoder``, ``on_error``, ...).

    Returns:
        Same as :func:`request` — the decoded response body. If the caller
        passed an ``on_error`` it is invoked with the ``HTTPError`` for both
        attempts; otherwise the default decode-bytes fallback applies.
    """
    base_headers = kwargs.pop("headers", None) or {}
    caller_on_error = kwargs.pop("on_error", None)
    build_auth = auth_header or (lambda tok: {"Authorization": f"Bearer {tok}"})

    response: str = ""
    for attempt in range(2):
        token = token_manager.get_state()
        captured: dict = {}

        def _capture(error, _captured=captured, _delegate=caller_on_error):
            _captured["code"] = getattr(error, "code", None)
            if _delegate is not None:
                return _delegate(error)
            # Mirror request()'s default error-decoding so changes there
            # propagate without drift. Guard against streamed-body / dropped-
            # socket errors so the helper never raises in place of returning
            # an error body the carrier parser can handle.
            try:
                return utils.decode_bytes(error.read())
            except Exception:
                return str(error)

        # NOTE: call the module-level ``request`` so a patch on
        # ``<module>.lib.request`` (Python's standard mocking pattern for
        # carrier proxies) still intercepts the call.
        response = request(
            headers={**base_headers, **build_auth(token)},
            on_error=_capture,
            **kwargs,
        )

        if captured.get("code") in retry_on_statuses and attempt == 0:
            utils.logger.info(
                "Authenticated request rejected by upstream; invalidating token and retrying once",
