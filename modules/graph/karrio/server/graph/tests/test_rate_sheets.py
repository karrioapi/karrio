...
class TestServiceLevelTagsGraphQL(GraphTestCase):
    """Tenant-scoped graph coverage for service-level tags (PR #662).

    Complements the admin tests by hitting the base `/graphql` surface:
      - tag_definitions registry query
      - tags + derived_badges fields on ServiceLevelType inside rate_sheet
    """

    def setUp(self):
        super().setUp()
        self.rate_sheet = providers.RateSheet.objects.create(
            name="Tenant Tags Sheet",
            carrier_name="dhl_parcel_de",
            slug="tenant_tags_sheet",
            created_by=self.user,
        )
        self.service = providers.ServiceLevel.objects.create(
            service_name="DHL Paket Tags",
    ...
    def test_tag_definitions_query_exposes_registry(self):
        """The tag_definitions query is available on the base graph for tenant UIs."""
        from karrio.server.providers.tags import TAG_REGISTRY

        response = self.query(
            """
            query GetTagDefinitions {
              tag_definitions { key kind sortable filterable label_key }
            }
            """,
        ...
        self.assertEqual({d["key"] for d in defs}, set(TAG_REGISTRY.keys()))

    def test_rate_sheet_service_returns_tags_and_derived_badges(self):
        """tags returns the stored dict; derived_badges picks the highest-priority match.

        With recommended=true (badge priority 50) and recommendation_type=fastest
        (priority 55), fastest wins.
        """
        response = self.query(
            """
            query get_rate_sheet($id: String!) {
              rate_sheet(id: $id) {
                services {
                  id
                  tags
                  derived_badges { label_key style priority }
                }
              }
            }
            """,
        ...
        self.assertEqual(svc["derived_badges"][0]["label_key"], "tags.badge.fastest")
        self.assertEqual(svc["derived_badges"][0]["style"], "purple")
        self.assertEqual(svc["derived_badges"][0]["priority"], 55)

    def test_derived_badges_empty_when_no_recognised_tag_values(self):
        """Tags without any badge-bearing values yield an empty derived_badges list."""
        self.service.tags = {"display_priority": 10, "surface_visibility": "default"}
        ...
        response = self.query(
            ...
            query get_rate_sheet($id: String!) {
              rate_sheet(id: $id) {
                services { id derived_badges { label_key } }
              }
            }
            """,
            operation_name="get_rate_sheet",
        ...
        self.assertResponseNoErrors(response)
        svc = next(s for s in response.data["data"]["rate_sheet"]["services"] if s["id"] == self.service.id)
        self.assertEqual(svc["derived_badges"], [])
