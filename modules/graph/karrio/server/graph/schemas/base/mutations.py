...
class MetadataMutation(utils.BaseMutation):
    ...
    def mutate(
        ...
        added_values: dict | None = None,
        discarded_keys: list | None = None,
    ) -> "MetadataMutation":
        object_model = utils.MetadataObjectType[object_type].value
        instance = object_model.access_by(info.context.request).get(id=id)
        instance.metadata = {
            key: value for key, value in (instance.metadata or {}).items() if key not in (discarded_keys or [])
        }
        instance.metadata.update(added_values or {})
...
class SystemCarrierMutation(utils.BaseMutation):
    ...
    def mutate(
        ...
        pk = public_ids.decrypt_id_or_passthrough(
            input.get("id"), public_ids.Namespaces.SYSTEM_CONNECTION
        )
