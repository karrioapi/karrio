...
def _add_extension(
    ...
    with Progress(
        ...
        if "location" in features:
            if is_xml_api:
                templates.XML_SCHEMA_LOCATION_REQUEST_TEMPLATE.stream(**context).dump(
                    f"{schemas_dir}/location_request.xsd"
                )
                templates.XML_SCHEMA_LOCATION_RESPONSE_TEMPLATE.stream(**context).dump(
                    f"{schemas_dir}/location_response.xsd"
                )
            else:
                templates.JSON_SCHEMA_LOCATION_REQUEST_TEMPLATE.stream(**context).dump(
                    f"{schemas_dir}/location_request.json"
                )
                templates.JSON_SCHEMA_LOCATION_RESPONSE_TEMPLATE.stream(**context).dump(
                    f"{schemas_dir}/location_response.json"
                )
        ...
        if "location" in features:
            templates.TEST_LOCATION_TEMPLATE.stream(**context).dump(f"{tests_dir}/test_location.py")

            templates.PROVIDER_LOCATION_TEMPLATE.stream(**context).dump(f"{providers_dir}/location.py")
...
def _add_features(
    ...
    with Progress(
        ...
        if "location" in features:
            if is_xml_api:
                utils.karrio_log("Adding location schema files...")
                templates.XML_SCHEMA_LOCATION_REQUEST_TEMPLATE.stream(**context).dump(
                    os.path.join(schemas_dir, "location_request.xsd")
                )
                templates.XML_SCHEMA_LOCATION_RESPONSE_TEMPLATE.stream(**context).dump(
                    os.path.join(schemas_dir, "location_response.xsd")
                )
            else:
                utils.karrio_log("Adding location schema files...")
                templates.JSON_SCHEMA_LOCATION_REQUEST_TEMPLATE.stream(**context).dump(
                    os.path.join(schemas_dir, "location_request.json")
                )
                templates.JSON_SCHEMA_LOCATION_RESPONSE_TEMPLATE.stream(**context).dump(
                    os.path.join(schemas_dir, "location_response.json")
                )

            utils.karrio_log("Adding location test file...")
            templates.TEST_LOCATION_TEMPLATE.stream(**context).dump(os.path.join(tests_dir, "test_location.py"))

            utils.karrio_log("Adding location provider files...")
            templates.PROVIDER_LOCATION_TEMPLATE.stream(**context).dump(os.path.join(providers_dir, "location.py"))
