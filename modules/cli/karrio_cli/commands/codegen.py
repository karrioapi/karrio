...
def transform(
    input_file: str | None = typer.Argument(None, help="Input file path. If not provided, reads from stdin."),
    output_file: str | None = typer.Argument(None, help="Output file path. If not provided, writes to stdout."),
    append_type_suffix: bool = typer.Option(True, help="Append 'Type' to class names", is_flag=True),
):
    """
...
def transform_content(content: str, append_type_suffix: bool = True) -> str:
    ...
    if append_type_suffix:
        ...
        for original_name, new_name in class_name_mapping.items():
            ...
            content = re.sub(rf"class\s+{original_name}\s*:", f"class {new_name}:", content)
    ...
    for i in range(len(lines)):
        ...
        line = re.sub(
            r"(\[|,\s*)Any(\]|,|\s*\]=)", r"\1typing.Any\2", line
        )  # Match Any inside brackets or between commas
...
def generate(
    ...
    output_file: str | None = typer.Argument(None, help="Output Python file path. If not provided, writes to stdout."),
