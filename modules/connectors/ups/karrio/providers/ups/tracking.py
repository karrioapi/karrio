...
def parse_tracking_response(
    _responses: lib.Deserializable[list[tuple[str, dict]]],
    ...
    packages = [
        ...
        if "trackResponse" in result and result["trackResponse"]["shipment"][0].get("package") is not None
    ]
...
def _extract_details(
    ...
    origin = next((p for p in package.packageAddress or [] if p.type == "ORIGIN"), None)
    destination = next((p for p in package.packageAddress or [] if p.type == "DESTINATION"), None)
    delivered = any(a.status.type == "D" for a in package.activity)
    estimated_delivery = next(iter([d.date for d in package.deliveryDate if d.type == "DEL"]), None)
    signature_image = lib.failsafe(lambda: getattr(package.deliveryInformation.signature, "image", None))
    delivery_image = lib.failsafe(lambda: getattr(package.deliveryInformation.deliveryPhoto, "photo", None))
    last_event = package.activity[0]
    status = (
        provider_units.TrackingStatus.find(getattr(last_event.status, "code", None)).name
        or provider_units.TrackingStatus.find(getattr(last_event.status, "type", None)).name
    ...
    return models.TrackingDetails(
        ...
        events=[
            models.TrackingEvent(
                ...
                status=(
                    provider_units.TrackingStatus.find(getattr(a.status, "code", None)).name
                    or provider_units.TrackingStatus.find(getattr(a.status, "type", None)).name
                ),
        ...
        info=models.TrackingInfo(
            ...
            shipment_pickup_date=(
                lib.fdate(shipment.pickupDate, "%Y%m%d") if getattr(shipment, "pickupDate", None) else None
            ),
            package_weight=getattr(package.weight, "weight", None),
            package_weight_unit=getattr(package.weight, "unitOfMeasurement", None),
            shipment_origin_country=(getattr(origin.address, "country", None) if origin else None),
            shipment_origin_postal_code=(getattr(origin.address, "postalCode", None) if origin else None),
            shipment_destination_country=(getattr(destination.address, "country", None) if destination else None),
            shipment_destination_postal_code=(
                getattr(destination.address, "postalCode", None) if destination else None
            ),
            customer_name=(destination.attentionName or destination.name if destination else None),
        ),
