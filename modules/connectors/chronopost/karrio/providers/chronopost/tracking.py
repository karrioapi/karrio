...
def parse_tracking_response(
    ...
    tracking_nodes: list[chronopost.listEventInfoComps] = lib.find_element(
...
def _extract_details(
    ...
    delivery: chronopost.eventInfoComp = next((event for event in detail.events if event.code == "D"), None)
    ...
    return models.TrackingDetails(
        ...
        info=models.TrackingInfo(
            ...
            customer_name=(getattr(delivery.infoCompList, "name", None) if delivery else None),
...
def tracking_request(
    ...
    request = [
        lib.Envelope(
            Body=lib.Body(chronopost.trackSkybillV2(language=settings.language, skybillNumber=tracking_number)),
        )
