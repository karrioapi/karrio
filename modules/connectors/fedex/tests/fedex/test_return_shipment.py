...
class TestFedExReturnShipment(unittest.TestCase):
    ...
    def test_create_return_shipment_request(self):
        ...
        return_detail = (
            serialized.get("requestedShipment", {}).get("shipmentSpecialServices", {}).get("returnShipmentDetail")
        )
        print(return_detail)
    ...
    def test_create_return_shipment(self):
        with patch("karrio.mappers.fedex.proxy.lib.request") as mock:
            ...
            self.assertEqual(url, f"{gateway.settings.server_url}/ship/v1/shipments")
