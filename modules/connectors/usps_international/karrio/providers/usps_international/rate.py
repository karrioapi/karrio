...
def rate_request(
    ...
    rate_indicator = lib.identity(
        options.usps_rate_indicator.state or provider_units.PackagingType.map(packages.package_type).value or None
    )
    ...
    def package_options(package):
        return lib.identity(package.options if package_mail_class(package) not in [] else {})
