...
def shipment_request(
    ...
    shipper_country_id = next(
        (
            int(country.value)
            for country in list(provider_units.ShippingCountries)
            if shipper.country_code == country.name
        ),
        int(provider_units.ShippingCurrency.AMD.value),
    )

    recipient_country_id = next(
        (
            int(country.value)
            for country in list(provider_units.ShippingCountries)
            if recipient.country_code == country.name
        ),
        int(provider_units.ShippingCurrency.AMD.value),
    )
