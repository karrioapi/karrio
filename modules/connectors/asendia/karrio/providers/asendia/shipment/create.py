...
def shipment_request(
    ...
    # AsendiaService.service is a mandatory API field (CUP/CPPR for outbound, RETPP/RETPAP for returns).
    ...
    service_options = [
        getattr(options, key).code for key in provider_units.SHIPMENT_FLAG_OPTIONS if getattr(options, key).state
    ]
    shipping_cost = options.declared_value.state
    smartgate = options.asendia_smartgate_flex.state or options.asendia_smartgate_direct.state
    ...
    customs_currency = lib.failsafe(lambda: customs.duty.currency) or "EUR"
    ...
    request = [
        asendia_req.ShipmentRequestType(
            ...
            shippingCost=shipping_cost if shipping_cost else None,
            ...
            asendiaService=asendia_req.AsendiaServiceType(
                format=provider_units.resolve_format(product_code, package.packaging_type, smartgate),
                ...
                asendia_req.CustomsInfoType(
                    ...
                    items=[
                        asendia_req.ItemType(
                            ...
                            currency=item.value_currency or customs_currency,
                    ...
                    + (
                        [
                            asendia_req.ItemType(
                                ...
                                unitValue=shipping_cost or 0.0,
                                currency=customs_currency,
                        ...
                        if options.asendia_smartgate_flex.state
                        else []
                    ),
                )
                if any(package.items) or customs.commodities or options.asendia_smartgate_flex.state
                else None
            ),
        )
