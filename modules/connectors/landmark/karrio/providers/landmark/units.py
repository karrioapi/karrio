...
class ShippingOption(lib.Enum):
    ...
    # fmt: off
    landmark_shipment_insurance_freight = lib.OptionEnum("ShipmentInsuranceFreight", float, meta=dict(category="INSURANCE", configurable=True, service_level=False))
    landmark_order_insurance_freight_total = lib.OptionEnum("OrderInsuranceFreightTotal", float, meta=dict(category="INSURANCE", configurable=True, service_level=False))

    """ Unified Option type mapping """
    landmark_produce_label = lib.OptionEnum("ProduceLabel", bool, meta=dict(configurable=True, service_level=False))
    landmark_import_request = lib.OptionEnum("InportRequest", bool, meta=dict(configurable=True, service_level=False))
    fulfilled_by_landmark = lib.OptionEnum("FulfilledByLandmark", bool, meta=dict(configurable=True, service_level=False))
    landmark_freight_pro_number = lib.OptionEnum("FreightProNumber", str, meta=dict(configurable=True, service_level=False))
    landmark_freight_piece_unit = lib.OptionEnum("FreightPieceUnit", str, meta=dict(configurable=True, service_level=False))
    landmark_return_address_code = lib.OptionEnum("ReturnAddressCode", str, meta=dict(category="RETURN", configurable=True, service_level=False))
