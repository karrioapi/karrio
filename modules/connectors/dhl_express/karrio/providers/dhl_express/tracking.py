...
def parse_tracking_response(
    ...
    tracking_details = [
        _extract_tracking(node, settings) for node in nodes if len(lib.find_element("ShipmentInfo", node)) > 0
    ]
    return (
        tracking_details,
...
def _extract_tracking(
    ...
    estimated_delivery = lib.fdate(info.findtext("EstDlvyDate"), "%Y-%m-%d %H:%M:%S %Z%z")
    events: list[dhl.ShipmentEvent] = lib.find_element("ShipmentEvent", info, dhl.ShipmentEvent)
    ...
    return models.TrackingDetails(
        ...
        events=[
            models.TrackingEvent(
                ...
                status=next(
                    (s.name for s in list(provider_units.TrackingStatus) if e.ServiceEvent.EventCode in s.value),
                    None,
                ),
                reason=next(
                    (
        ...
        info=models.TrackingInfo(
            ...
            package_weight_unit=provider_units.WeightUnit.map(info.findtext("WeightUnit")).name_or_key,
        ),
    )
