...
class TestHermesShipment(unittest.TestCase):
    ...
    def test_create_shipment_with_parcel_class(self):
        """parcelClass option (SS01) is sent on the parcel block."""
        request = models.ShipmentRequest(**{**ShipmentPayload, "options": {"hermes_parcel_class": "L"}})
        serialized = gateway.mapper.create_shipment_request(request).serialize()
        self.assertEqual(serialized[0]["parcel"]["parcelClass"], "L")

    def test_create_parcel_shop_by_id(self):
        """psID option routes via SELECT_BY_ID (KA07 / SS07)."""
        request = models.ShipmentRequest(**{**ShipmentPayload, "options": {"hermes_parcel_shop_id": "656499"}})
        serialized = gateway.mapper.create_shipment_request(request).serialize()
        self.assertDictEqual(
            serialized[0]["service"]["parcelShopDeliveryService"],
            {
                "psCustomerFirstName": "Max",
                "psCustomerLastName": "Mustermann",
                "psID": "656499",
                "psSelectionRule": "SELECT_BY_ID",
            },
        )

    def test_create_parcel_shop_by_receiver_address(self):
        """Selection rule alone routes via SELECT_BY_RECEIVER_ADDRESS (KA08 / SS08)."""
        request = models.ShipmentRequest(
            **{
                **ShipmentPayload,
                "options": {"hermes_parcel_shop_selection_rule": "SELECT_BY_RECEIVER_ADDRESS"},
            }
        )
        serialized = gateway.mapper.create_shipment_request(request).serialize()
        ps = serialized[0]["service"]["parcelShopDeliveryService"]
        # psID must be omitted for the by-address variant (sending it triggers e091)
        self.assertNotIn("psID", ps)
        self.assertEqual(ps["psSelectionRule"], "SELECT_BY_RECEIVER_ADDRESS")

    def test_create_shipment_with_customer_references(self):
        """Customer reference options map to clientReference / clientReference2."""
        request = models.ShipmentRequest(
            **{
                **ShipmentPayload,
                "options": {
                    "hermes_customer_reference_1": "ORD-QA-001",
                    "hermes_customer_reference_2": "WAWI-9988",
                },
            }
        )
        serialized = gateway.mapper.create_shipment_request(request).serialize()
        self.assertEqual(serialized[0]["clientReference"], "ORD-QA-001")
        self.assertEqual(serialized[0]["clientReference2"], "WAWI-9988")
