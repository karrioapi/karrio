...
def _validate_tracking_numbers(shipments, tracking_numbers):
    """Validate that all tracking_numbers resolved to tenant-owned shipments
    and that none of them are already linked to an active pickup.

    Raises serializers.ValidationError with a field-keyed error shape so the
    public error contract matches the previous field-level validator.
    """
    if not tracking_numbers:
        return

    found_numbers = {s.tracking_number for s in shipments}
    missing = [t for t in tracking_numbers if t not in found_numbers]
    if missing:
        raise serializers.ValidationError(
            {"tracking_numbers": f"Shipment with the tracking numbers: {missing} not found"},
            code="invalid",
        )

    scheduled = [
        s.tracking_number for s in shipments if s.shipment_pickup.exclude(status__in=["cancelled", "closed"]).exists()
    ]
    if scheduled:
        raise serializers.ValidationError(
            {"tracking_numbers": f"The following shipments {scheduled} are already scheduled for pickups"},
            code="invalid",
        )
...
class PickupSerializer(PickupRequest):
    ...
    def __init__(self, instance: models.Pickup = None, **kwargs):
        ...
        if "data" in kwargs:
            ...
            if tracking_numbers:
                self._shipments = list(models.Shipment.access_by(context).filter(tracking_number__in=tracking_numbers))
            ...
            if data.get("address") is None and instance is None:
                ...
                address = models.Address.access_by(context).get(pk=data.get("address"))
            else:
    ...
    def validate(self, data):
        ...
        _validate_tracking_numbers(self._shipments, validated_data.get("tracking_numbers", []))
...
class PickupData(PickupSerializer):
    def create(self, validated_data: dict, context: Context, **kwargs) -> models.Pickup:
        ...
        # Mirror shipment "bind" save so post_save fires with created=False and
        # update_fields=["status","created_at"], triggering the billing-event
        # signal (`on_pickup_updated`). Without this, brokered pickups never
        # emit a SHIPMENT_CREATED event to billing-service.
        pickup.save(update_fields=["status", "created_at"])
