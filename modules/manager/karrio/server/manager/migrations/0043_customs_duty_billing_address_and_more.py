...
def forwards_func(apps, schema_editor):
    ...
    for customs in Customs.objects.using(db_alias).filter(duty__bill_to__isnull=False).iterator():
        try:
            customs.duty_billing_address = Address.objects.create(**customs.duty["bill_to"])
    ...
    if any(_customs_infos):
        Customs.objects.using(db_alias).bulk_update(_customs_infos, ["duty", "duty_billing_address"])
