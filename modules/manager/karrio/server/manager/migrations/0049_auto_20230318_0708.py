...
def forwards_func(apps, schema_editor):
    ...
    for _record in records:
        _documents = sum(
            (doc if isinstance(doc, list) else [doc] for doc in _record.documents or []),
            start=[],
        )
        _record.documents = [dict(doc_id=doc["document_id"], file_name=doc["file_name"]) for doc in _documents]
