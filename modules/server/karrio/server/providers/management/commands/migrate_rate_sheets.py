    ...
    def handle(self, *args, **options):
        ...
        for rate_sheet in rate_sheets:
            try:
                ...
                has_zones = any(service.zones for service in rate_sheet.services.all())

                if not has_zones:
                    self.stdout.write(self.style.WARNING(f"Skipping {rate_sheet.name} - no zones to migrate"))
                    skipped_count += 1
                    continue

                if dry_run:
                    self.stdout.write(self.style.SUCCESS(f"Would migrate: {rate_sheet.name}"))
                    migrated_count += 1
                else:
                    # Perform migration
                    old_zones_count = sum(len(service.zones or []) for service in rate_sheet.services.all())
                ...
                self.stdout.write(self.style.ERROR(f"Error migrating {rate_sheet.name}: {str(e)}"))
                error_count += 1
