...
def forwards_func(apps, schema_editor):
    ...
    logs = APILog.objects.using(db_alias).filter(models.Q(response__icontains="test_mode")).iterator()

    for log in logs:
        response = utils.failsafe(lambda log=log: lib.to_dict(lib.to_dict(log.response)))
        entity_id = utils.failsafe(lambda response=response: response["id"])
        test_mode = utils.failsafe(lambda response=response: response["test_mode"])
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.AddField(
            ...
            name="test_mode",
            field=models.BooleanField(default=True, help_text="execution context", null=True),
        ),
