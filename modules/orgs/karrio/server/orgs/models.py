import importlib
from functools import partial
from django.db import models
from django.contrib.postgres import fields
from organizations.abstract import (
    AbstractOrganization,
    AbstractOrganizationUser,
    AbstractOrganizationOwner,
    AbstractOrganizationInvitation,
)

import karrio.server.orgs.serializers.base as serializers
import karrio.server.providers.models as providers
import karrio.server.documents.models as documents
import karrio.server.pricing.models as pricing
import karrio.server.manager.models as manager
import karrio.server.tracing.models as tracing
import karrio.server.events.models as events
import karrio.server.orders.models as orders
import karrio.server.core.fields as fields
import karrio.server.graph.models as graph
import karrio.server.audit.models as audit
import karrio.server.core.models as core
import karrio.server.user.models as auth
import karrio.server.data.models as data

AUTOMATION_MODULE_INSTALLED = (
    importlib.util.find_spec("karrio.server.automation") is not None
)
APPS_MODULE_INSTALLED = importlib.util.find_spec("karrio.server.apps") is not None


@core.register_model
class Organization(AbstractOrganization):
    id = models.CharField(
        max_length=50,
        primary_key=True,
        default=partial(core.uuid, prefix="org_"),
        editable=False,
    )

    config = models.OneToOneField(
        auth.WorkspaceConfig,
        on_delete=models.CASCADE,
        related_name="org",
        null=True,
    )
    metadata = models.JSONField(
        default=core.field_default({}),
        help_text="Organization defined metadata",
    )

    carriers = models.ManyToManyField(
        providers.Carrier, related_name="org", through="CarrierLink"
    )
    system_carriers = models.ManyToManyField(
        providers.Carrier, related_name="active_orgs"
    )
    carrier_configs = models.ManyToManyField(
        providers.CarrierConfig, related_name="org", through="CarrierConfigLink"
    )
    rate_sheets = models.ManyToManyField(
        providers.RateSheet, related_name="org", through="RateSheetLink"
    )

    parcels = models.ManyToManyField(
        manager.Parcel, related_name="org", through="ParcelLink"
    )
    pickups = models.ManyToManyField(
        manager.Pickup, related_name="org", through="PickupLink"
    )
    customs = models.ManyToManyField(
        manager.Customs, related_name="org", through="CustomsLink"
    )
    trackers = models.ManyToManyField(
        manager.Tracking, related_name="org", through="TrackerLink"
    )
    addresses = models.ManyToManyField(
        manager.Address, related_name="org", through="AddressLink"
    )
    shipments = models.ManyToManyField(
        manager.Shipment, related_name="org", through="ShipmentLink"
    )
    commodities = models.ManyToManyField(
        manager.Commodity, related_name="org", through="CommodityLink"
    )
    upload_records = models.ManyToManyField(
        manager.DocumentUploadRecord, related_name="org", through="UploadRecordLink"
    )
    manifests = models.ManyToManyField(
        manager.Manifest, related_name="org", through="ManifestLink"
    )

    templates = models.ManyToManyField(
        graph.Template, related_name="org", through="TemplateLink"
    )

    webhooks = models.ManyToManyField(
        events.Webhook, related_name="org", through="WebhookLink"
    )
    events = models.ManyToManyField(
        events.Event, related_name="org", through="EventLink"
    )

    orders = models.ManyToManyField(
        orders.Order, related_name="org", through="OrderLink"
    )

    document_templates = models.ManyToManyField(
        documents.DocumentTemplate, related_name="org", through="DocumentTemplateLink"
    )

    data_templates = models.ManyToManyField(
        data.DataTemplate, related_name="org", through="DataTemplateLink"
    )
    batch_operations = models.ManyToManyField(
        data.BatchOperation, related_name="org", through="BatchOperationLink"
    )

    logs = models.ManyToManyField(core.APILog, related_name="org", through="LogLink")

    auditlogs = models.ManyToManyField(
        audit.AuditLogEntry, related_name="org", through="AuditLogEntryLink"
    )

    tokens = models.ManyToManyField(auth.Token, related_name="org", through="TokenLink")

    surcharges = models.ManyToManyField(pricing.Surcharge, related_name="org")

    tracing_records = models.ManyToManyField(tracing.TracingRecord, related_name="org")

    metafields = models.ManyToManyField(core.Metafield, related_name="org")

    if AUTOMATION_MODULE_INSTALLED:
        import karrio.server.automation.models as automation

        workflows = models.ManyToManyField(
            automation.Workflow, related_name="org", through="WorkflowLink"
        )
        workflow_actions = models.ManyToManyField(
            automation.WorkflowAction, related_name="org", through="WorkflowActionLink"
        )
        workflow_connections = models.ManyToManyField(
            automation.WorkflowConnection,
            related_name="org",
            through="WorkflowConnectionLink",
        )
        workflow_events = models.ManyToManyField(
            automation.WorkflowEvent, related_name="org", through="WorkflowEventLink"
        )
        workflow_tiggers = models.ManyToManyField(
            automation.WorkflowTrigger,
            related_name="org",
            through="WorkflowTriggerLink",
        )
        shipping_rules = models.ManyToManyField(
            automation.ShippingRule, related_name="org", through="ShippingRuleLink"
        )

    if APPS_MODULE_INSTALLED:

        import karrio.server.apps.models as apps

        oauth_apps = models.ManyToManyField(
            apps.OAuthApp, related_name="org", through="OAuthAppLink"
        )
        app_installations = models.ManyToManyField(
            apps.AppInstallation, related_name="org", through="AppInstallationLink"
        )

    def is_owner(self, user):
        owner = getattr(self, "owner", None)
        return owner and super().is_owner(user)


@core.register_model
class OrganizationUser(AbstractOrganizationUser):
    id = models.CharField(
        max_length=50,
        primary_key=True,
        default=partial(core.uuid, prefix="usr_"),
        editable=False,
    )
    roles = fields.MultiChoiceField(
        choices=serializers.USER_ROLES,
        default=core.field_default(
            [
                serializers.UserRole.member.value,
                serializers.UserRole.developer.value,
            ]
        ),
    )
    metadata = models.JSONField(
        default=core.field_default({}),
        help_text="Organization user defined metadata",
    )

    def save(self, *args, **kwargs):
        if serializers.UserRole.member.value not in self.roles:
            self.roles.append(serializers.UserRole.member.value)

        if (serializers.UserRole.admin.value in self.roles) and not self.is_admin:
            self.is_admin = not self.is_admin

        return super().save(*args, **kwargs)

    def __str__(self):
        return f"{self.user.email} ({self.organization.name})"


@core.register_model
class OrganizationOwner(AbstractOrganizationOwner):
    def __str__(self):
        return "{0}: {1}".format(self.organization, self.organization_user.user.email)


@core.register_model
class OrganizationInvitation(AbstractOrganizationInvitation):
    is_owner = models.BooleanField(default=False)
    roles = fields.MultiChoiceField(
        choices=serializers.USER_ROLES,
        default=core.field_default(
            [
                serializers.UserRole.member.value,
                serializers.UserRole.developer.value,
            ]
        ),
    )


"""Models organization linking"""


class CarrierLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="carrier_links"
    )
    item = models.OneToOneField(
        providers.Carrier, on_delete=models.CASCADE, related_name="link"
    )


class CarrierConfigLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="carrier_config_links"
    )
    item = models.OneToOneField(
        providers.CarrierConfig, on_delete=models.CASCADE, related_name="link"
    )


class RateSheetLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="rate_sheet_links"
    )
    item = models.OneToOneField(
        providers.RateSheet, on_delete=models.CASCADE, related_name="link"
    )


class ParcelLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="parcel_links"
    )
    item = models.OneToOneField(
        manager.Parcel, on_delete=models.CASCADE, related_name="link"
    )


class PickupLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="pickup_links"
    )
    item = models.OneToOneField(
        manager.Pickup, on_delete=models.CASCADE, related_name="link"
    )


class CustomsLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="custom_links"
    )
    item = models.OneToOneField(
        manager.Customs, on_delete=models.CASCADE, related_name="link"
    )


class TrackerLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="tracker_links"
    )
    item = models.OneToOneField(
        manager.Tracking, on_delete=models.CASCADE, related_name="link"
    )


class AddressLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="address_links"
    )
    item = models.OneToOneField(
        manager.Address, on_delete=models.CASCADE, related_name="link"
    )


class ShipmentLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="shipment_links"
    )
    item = models.OneToOneField(
        manager.Shipment, on_delete=models.CASCADE, related_name="link"
    )


class UploadRecordLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="upload_record_links"
    )
    item = models.OneToOneField(
        manager.DocumentUploadRecord, on_delete=models.CASCADE, related_name="link"
    )


class ManifestLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="manifest_links"
    )
    item = models.OneToOneField(
        manager.Manifest, on_delete=models.CASCADE, related_name="link"
    )


class CommodityLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="commodity_links"
    )
    item = models.OneToOneField(
        manager.Commodity, on_delete=models.CASCADE, related_name="link"
    )


class TemplateLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="template_links"
    )
    item = models.OneToOneField(
        graph.Template, on_delete=models.CASCADE, related_name="link"
    )


class WebhookLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="webhook_links"
    )
    item = models.OneToOneField(
        events.Webhook, on_delete=models.CASCADE, related_name="link"
    )


class EventLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="event_links"
    )
    item = models.OneToOneField(
        events.Event, on_delete=models.CASCADE, related_name="link"
    )


class LogLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="log_links"
    )
    item = models.OneToOneField(
        core.APILog, on_delete=models.CASCADE, related_name="link"
    )


class AuditLogEntryLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="auditlog_links"
    )
    item = models.OneToOneField(
        audit.AuditLogEntry, on_delete=models.CASCADE, related_name="link"
    )


class TokenLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="token_links"
    )
    item = models.OneToOneField(
        auth.Token, on_delete=models.CASCADE, related_name="link"
    )


class OrderLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="order_links"
    )
    item = models.OneToOneField(
        orders.Order, on_delete=models.CASCADE, related_name="link"
    )


class TracingRecordLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="tracking_record_links"
    )
    item = models.OneToOneField(
        tracing.TracingRecord, on_delete=models.CASCADE, related_name="link"
    )


class DocumentTemplateLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="document_template_links"
    )
    item = models.OneToOneField(
        documents.DocumentTemplate, on_delete=models.CASCADE, related_name="link"
    )


class DataTemplateLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="data_template_links"
    )
    item = models.OneToOneField(
        data.DataTemplate, on_delete=models.CASCADE, related_name="link"
    )


class BatchOperationLink(models.Model):
    org = models.ForeignKey(
        Organization,
        on_delete=models.CASCADE,
        related_name="batch_operation_links",
    )
    item = models.OneToOneField(
        data.BatchOperation, on_delete=models.CASCADE, related_name="link"
    )


class MetafieldLink(models.Model):
    org = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="metafield_links"
    )
    item = models.OneToOneField(
        core.Metafield, on_delete=models.CASCADE, related_name="link"
    )


if AUTOMATION_MODULE_INSTALLED:
    import karrio.server.automation.models as automation

    class WorkflowLink(models.Model):
        org = models.ForeignKey(
            Organization, on_delete=models.CASCADE, related_name="workflow_links"
        )
        item = models.OneToOneField(
            automation.Workflow, on_delete=models.CASCADE, related_name="link"
        )

    class WorkflowActionLink(models.Model):
        org = models.ForeignKey(
            Organization, on_delete=models.CASCADE, related_name="workflow_action_links"
        )
        item = models.OneToOneField(
            automation.WorkflowAction, on_delete=models.CASCADE, related_name="link"
        )

    class WorkflowConnectionLink(models.Model):
        org = models.ForeignKey(
            Organization,
            on_delete=models.CASCADE,
            related_name="workflow_connection_links",
        )
        item = models.OneToOneField(
            automation.WorkflowConnection, on_delete=models.CASCADE, related_name="link"
        )

    class WorkflowEventLink(models.Model):
        org = models.ForeignKey(
            Organization, on_delete=models.CASCADE, related_name="workflow_event_links"
        )
        item = models.OneToOneField(
            automation.WorkflowEvent, on_delete=models.CASCADE, related_name="link"
        )

    class WorkflowTriggerLink(models.Model):
        org = models.ForeignKey(
            Organization,
            on_delete=models.CASCADE,
            related_name="workflow_trigger_links",
        )
        item = models.OneToOneField(
            automation.WorkflowTrigger, on_delete=models.CASCADE, related_name="link"
        )

    class ShippingRuleLink(models.Model):
        org = models.ForeignKey(
            Organization, on_delete=models.CASCADE, related_name="shipping_rule_links"
        )
        item = models.OneToOneField(
            automation.ShippingRule, on_delete=models.CASCADE, related_name="link"
        )


if APPS_MODULE_INSTALLED:
    import karrio.server.apps.models as apps

    class OAuthAppLink(models.Model):
        org = models.ForeignKey(
            Organization, on_delete=models.CASCADE, related_name="app_links"
        )
        item = models.OneToOneField(
            apps.OAuthApp, on_delete=models.CASCADE, related_name="link"
        )

    class AppInstallationLink(models.Model):
        org = models.ForeignKey(
            Organization,
            on_delete=models.CASCADE,
            related_name="app_installation_links",
        )
        item = models.OneToOneField(
            apps.AppInstallation, on_delete=models.CASCADE, related_name="link"
        )
