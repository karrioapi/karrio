...
def shipment_updated(sender, instance, created, raw, using, update_fields, *args, **kwargs):
    ...
    # Built-in webhooks are turned off on JTL deployments (BUILT_IN_WEBHOOKS=
    # False), where the events bridge owns event distribution; default on for
    # upstream karrio. Defer until the surrounding transaction commits so we
    # don't fire webhooks for rolled-back changes and don't block the request
    # thread on the AMQP attach inside the task backend's enqueue.
    if settings.BUILT_IN_WEBHOOKS:
        transaction.on_commit(
            functools.partial(tasks.notify_webhooks, event, data, event_at, context, schema=settings.schema)
        )
