...
class DocumentTemplateAdminForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "related_object": forms.Select(choices=[(c.name, c.name) for c in list(serializers.TemplateRelatedObject)]),
        }
...
class DocumentTemplateAdmin(admin.ModelAdmin):
    ...
    def full_preview_url(self, obj):
        ...
        if obj.pk:
            ...
            if request:
                ...
                return format_html(
                    '<a href="{}" target="_blank" rel="noopener noreferrer">{}</a>', absolute_url, absolute_url
                )
            else:
    ...
    def get_queryset(self, request):
        if settings.MULTI_ORGANIZATIONS:
            return models.DocumentTemplate.objects.all().filter(link__org__users__id=request.user.id)
