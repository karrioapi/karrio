...
REMOTE_STYLESHEET_URL = "https://cdn.jsdelivr.net/npm/bulma@0.9.3/css/bulma.min.css"
_stylesheets: list[weasyprint.CSS] | None = None
UNITS = {
...
def get_stylesheets() -> list[weasyprint.CSS]:
    """Load optional remote styles lazily with a local fallback.

    The remote Bulma stylesheet is useful for production rendering, but loading it
    during module import breaks tests and offline environments.
    """

    global _stylesheets

    if _stylesheets is not None:
        return _stylesheets

    stylesheets = list(INLINE_STYLESHEETS)

    # Offline/test environments should still be able to render PDFs with the
    # local baseline styles only.
    with contextlib.suppress(Exception):
        stylesheets.insert(0, weasyprint.CSS(url=REMOTE_STYLESHEET_URL))
...
class Documents:
    ...
    def generate(
        ...
        **kwargs,
        ...
        shipment_contexts = data.get("shipments_context") or lib.identity(
            get_shipments_context(data["shipments"]) if "shipments" in data and related_object == "shipment" else []
        )
        order_contexts = data.get("orders_context") or lib.identity(
            get_orders_context(data["orders"]) if "orders" in data and related_object == "order" else []
        )
        ...
        try:
            ...
            raise TemplateRenderingError(f"PDF generation error: {str(e)}", template_error=e) from e
    ...
    @staticmethod
    def shipment_params(shipment, **kwargs) -> dict:
        """Build the Jinja render context for a shipment document (shared by the
        slug-based and inline-body generation paths)."""
    ...
    def generate_shipment_document(slug: str, shipment, **kwargs) -> dict:
        """Generate a shipment document from a saved ``DocumentTemplate`` row (by slug)."""
        template = document_models.DocumentTemplate.objects.get(slug=slug)
        document = Documents.generate_template(template, Documents.shipment_params(shipment, **kwargs)).getvalue()
    ...
    @staticmethod
    def generate_shipment_document_from_body(
        template_body: str,
        shipment,
        *,
        doc_name: str = "commercial-invoice.pdf",
        doc_type: str = "commercial_invoice",
        metadata: dict | None = None,
        **kwargs,
    ) -> dict:
        """Generate a shipment document from an inline template **body** — no saved
        ``DocumentTemplate`` row required. This is the interface behind the
        system-wide default invoice (``PAPERLESS_DEFAULT_INVOICE_TEMPLATE`` holds the
        actual HTML/Jinja body), so a default always resolves without depending on
        tenant-owned template rows."""
        document = Documents.generate(
            template=template_body,
            data=Documents.shipment_params(shipment, **kwargs),
            related_object="shipment",
            metadata=metadata or {"doc_type": doc_type},
        ).getvalue()
...
def get_shipments_context(shipment_ids: str) -> list[dict]:
    ...
    return [
        dict(
            ...
            orders=orders_serializers.Order(get_shipment_order_contexts(shipment), many=True).data,
        )
...
def get_shipment_order_contexts(shipment):
    ...
    return order_models.Order.objects.filter(shipments=shipment).order_by("-order_date").distinct()
...
def get_orders_context(order_ids: str) -> list[dict]:
    if order_ids == "sample":
