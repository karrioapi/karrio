...
class SystemCarrierConnectionType(base.CarrierConnectionType):
    ...
    def id(self: providers.SystemConnection) -> str:
        # Admin graph (/admin/graphql) is staff-only at the route level —
        # raw SystemConnection PKs are appropriate here. Override the
        # tenant-graph default (which would encrypt it).
        return self.id
    ...
    def resolve_list(
        ...
        if _filter_data.get("carrier_name"):
            # carrier_name is a list[str] on CarrierFilter — match any of them.
            queryset = queryset.filter(carrier_code__in=_filter_data["carrier_name"])
...
class SystemRateSheetType(base.RateSheetType):
    ...
    def resolve_list(
        ...
        # service_rates (the ~300 KB rate matrix) is only rendered by the detail
        # view; defer it so the list doesn't load + json.loads it per row.
        queryset = filters.SystemRateSheetFilter(
            _filter.to_dict(),
            providers.SystemRateSheet.objects.all().defer("service_rates"),
        ).qs
