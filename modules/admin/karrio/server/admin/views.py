...
class AdminGraphQLView(views.GraphQLView):
    """GraphQL view for /admin/graphql that enforces staff-only access at the route.

    The base graph view only checks that the request is authenticated. Admin
    schemas expose system-wide data and must additionally verify that the
    caller is a staff user. Enforcing this here, before the schema executes,
    is the single source of truth for admin authorization — per-resolver
    decorators are not used; new admin queries/mutations inherit the gate by
    virtue of being mounted under this route.
    """

    def dispatch(self, request, *args, **kwargs):
        if lib.failsafe(lambda: self.should_render_graphiql(request)):
...
urlpatterns = [
    path(
        "admin/graphql/",
        csrf_exempt(AdminGraphQLView.as_view(schema=schema.schema)),
        name="admin-graph",
    ),
    path(
        "admin/graphql",
