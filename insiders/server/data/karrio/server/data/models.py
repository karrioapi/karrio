...
class BatchOperation(core.OwnedEntity):
    ...
    org = models.ManyToManyField(
        orgs.Organization, related_name="batch_operations", through="BatchOperationLink"
    )
...
class DataTemplate(core.OwnedEntity):
    ...
    org = models.ManyToManyField(
        orgs.Organization, related_name="data_templates", through="DataTemplateLink"
    )
...
class DataTemplateLink(models.Model):
    org = models.ForeignKey(
        orgs.Organization, on_delete=models.CASCADE, related_name="data_template_links"
    )
    item = models.OneToOneField(
