...
class FedexMapperPartial(FedexMapperBase):
    def parse_process_shipment_reply(
        ...
        shipment: T.ShipmentDetails = self._extract_shipment(details[0]) if len(
            details
        ) > 0 else None
        return (shipment, self.parse_error_response(response))

    def _extract_shipment(
        self, shipmentDetailNode: etree.ElementBase
    ) -> T.ShipmentDetails:
        ...
        return T.ShipmentDetails(
            ...
            tracking_numbers=reduce(
                lambda ids, pkg: ids + [id.TrackingNumber for id in pkg.TrackingIds],
                items,
                [],
            ),
            ...
            RequestedShipment=RequestedShipment(
                ...
                RequestedPackageLineItems=[
                    RequestedPackageLineItem(
                        ...
                        if "CustomerReferences" in pkg.extra
                        else None,
