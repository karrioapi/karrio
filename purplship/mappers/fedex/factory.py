...
def create_rate_request(mapper: Mapper, payload: E.quote_request) -> Rate.RateRequest:
    ...
    version = Rate.VersionId(ServiceId="crs", Major=22,
    ...
    totalWeight = reduce(lambda r, p: r + p.weight,
                         payload.shipment.packages, 0)
...
def create_track_request(mapper: Mapper, payload: E.tracking_request) -> Track.TrackRequest:
    version = Track.VersionId(
        ServiceId="trck", Major=14, Intermediate=0, Minor=0)
...
def create_shipment_request(mapper: Mapper, payload: E.shipment_request) -> Ship.ProcessShipmentRequest:
    ...
    def _create_party(data: E.party, account_number: str) -> Ship.Party:
        Party_ = Ship.Party(
            ...
            Address=Ship.Address(
                ...
                GeographicCoordinates=data.extra.get(
    ...
    Shipper_ = _create_party(
        payload.shipper, payload.shipment.shipper_account_number)

    Recipient_ = _create_party(
        payload.recipient, payload.recipient.extra.get('AccountNumber'))
    ...
    FreightShipmentDetail_ = Ship.FreightShipmentDetail(
        ...
        FedExFreightBillingContactAndAddress=payload.shipment.extra.get(
        ...
        DeclaredValuePerUnit=payload.shipment.extra.get(
            "DeclaredValuePerUnit"),
        DeclaredValueUnits=payload.shipment.extra.get(
            "DeclaredValueUnits"),
        TotalHandlingUnits=payload.shipment.extra.get(
            "TotalHandlingUnits"),
        ClientDiscountPercent=payload.shipment.extra.get(
            "ClientDiscountPercent"),
        PalletWeight=payload.shipment.total_weight,
        ShipmentDimensions=payload.shipment.extra.get(
            "ShipmentDimensions"),
        Comment=payload.shipment.extra.get("Comment"),
        HazardousMaterialsEmergencyContactNumber=payload.shipment.extra.get(
            "HazardousMaterialsEmergencyContactNumber"),
        HazardousMaterialsOfferor=payload.shipment.extra.get(
    ...
    if 'SpecialServicePayments' in payload.shipment.extra:
        FreightShipmentDetail_.SpecialServicePayments = Ship.FreightSpecialServicePayment(
            SpecialService=payload.shipment.extra.get(
                "SpecialServicePayments").get("SpecialService"),
            PaymentType=payload.shipment.extra.get(
                "SpecialServicePayments").get("PaymentType")
        )
    ...
    if 'LiabilityCoverageDetail' in payload.shipment.extra:
        FreightShipmentDetail_.LiabilityCoverageDetail = Ship.LiabilityCoverageDetail(
            CoverageType=payload.shipment.extra.get(
                "LiabilityCoverageDetail").get("CoverageType"),
            CoverageAmount=payload.shipment.extra.get(
                "LiabilityCoverageDetail").get("CoverageType")
        )

    for ref in payload.shipment.references:
        FreightShipmentDetail_.add_PrintedReferences(
            Ship.PrintedReference(Value=ref))

    RequestedShipment_ = Ship.RequestedShipment(
        ShipTimestamp=datetime.now(),
        DropoffType=payload.shipment.extra.get(
            "DropoffType") or "REGULAR_PICKUP",
        ServiceType=payload.shipment.extra.get(
            "ServiceType") or "INTERNATIONAL_PRIORITY",
        PackagingType=payload.shipment.extra.get("PackagingType"),
        TotalWeight=payload.shipment.total_weight or reduce(
            lambda t, p: t + p.weight, payload.shipment.packages, 0),
        ...
        RecipientLocationNumber=payload.shipment.extra.get(
        ...
        DeliveryInstructions=payload.shipment.extra.get(
            "DeliveryInstructions"),
        BlockInsightVisibility=payload.shipment.extra.get(
            "BlockInsightVisibility"),
        RateRequestTypes=payload.shipment.extra.get(
            'RateRequestTypes') or 'LIST',
    ...
    for id, pkg in enumerate(payload.shipment.packages):
        RequestedShipment_.RequestedPackageLineItems.append(
            Ship.RequestedPackageLineItem(
                ...
                VariableHandlingChargeDetail=pkg.extra.get(
                ...
                ItemDescriptionForClearance=pkg.extra.get(
                    "ItemDescriptionForClearance"),
                CustomerReferences=pkg.extra.get("CustomerReferences"),
                SpecialServicesRequested=pkg.extra.get(
    ...
    if 'Payor' in payload.shipment.extra or payload.shipment.paid_by is not None:
        if payload.shipment.paid_by == 'THIRD_PARTY':
            ResponsibleParty_ = _create_party(E.party(
                **payload.shipment.extra('Payor')), payload.shipment.billing_account_number)
        else:
            ResponsibleParty_ = Shipper_
        RequestedShipment_.ShippingChargesPayment = Ship.Payment(
            PaymentType=payload.shipment.paid_by,
            Payor=Ship.Payor(
                ResponsibleParty=ResponsibleParty_) if ResponsibleParty_ is not None else None
        )
    ...
    if payload.shipment.label is not None:
        RequestedShipment_.LabelSpecification = Ship.LabelSpecification(
            ...
            LabelStockType=payload.shipment.label.extra.get(
                'LabelStockType'),
            ...
            PrintedLabelOrigin=payload.shipment.label.extra.get(
                'PrintedLabelOrigin'),
            CustomerSpecifiedDetail=payload.shipment.label.extra.get(
    ...
    if 'SmartPostDetail' in payload.shipment.extra:
        ...
        RequestedShipment_.SmartPostDetail = Ship.SmartPostShipmentDetail(
            ProcessingOptionsRequested=smartP.get(
    ...
    if 'PickupDetail' in payload.shipment.extra:
        ...
        """
        pickup = E.Pickup.request(
            **payload.shipment.extra.get("PickupDetail"))
    ...
    if payload.shipment.customs is not None:
        RequestedShipment_.CustomsClearanceDetail = Ship.CustomsClearanceDetail(
            ...
            ClearanceBrokerage=payload.shipment.customs.extra.get(
                "ClearanceBrokerage"),
            CustomsOptions=payload.shipment.customs.extra.get(
                "CustomsOptions"),
            ImporterOfRecord=payload.shipment.customs.extra.get(
                "ImporterOfRecord"),
            RecipientCustomsId=payload.shipment.customs.extra.get(
                'RecipientCustomsId'),
            DutiesPayment=payload.shipment.duty_payment_account,
            DocumentContent=payload.shipment.customs.description,
            CustomsValue=payload.shipment.customs.extra.get(
                'CustomsValue'),
            FreightOnValue=payload.shipment.customs.extra.get(
                'FreightOnValue'),
            InsuranceCharges=payload.shipment.insured_amount,
            PartiesToTransactionAreRelated=payload.shipment.customs.extra.get(
        ...
        if 'CommercialInvoice' in payload.shipment.customs.extra:
            CommercialInvoice = payload.shipment.customs.extra.get(
                'CommercialInvoice')
            RequestedShipment_.CustomsClearanceDetail.CommercialInvoice = Ship.CommercialInvoice(
                ...
                TaxesOrMiscellaneousCharge=CommercialInvoice.get(
                    'TaxesOrMiscellaneousCharge'),
                TaxesOrMiscellaneousChargeType=CommercialInvoice.get(
                ...
                SpecialInstructions=CommercialInvoice.get(
                    'SpecialInstructions'),
                DeclarationStatement=CommercialInvoice.get(
                    'DeclarationStatement'),
                PaymentTerms=CommercialInvoice.get('PaymentTerms'),
                Purpose=CommercialInvoice.get('Purpose'),
                CustomerReferences=CommercialInvoice.get(
                    'CustomerReferences'),
    ...
    if 'VariableHandlingChargeDetail' in payload.shipment.extra:
        RequestedShipment_.VariableHandlingChargeDetail = Ship.VariableHandlingChargeDetail(
            FixedValue=payload.shipment.extra.get(
                "VariableHandlingChargeDetail").get("FixedValue"),
            PercentValue=payload.shipment.extra.get(
                "VariableHandlingChargeDetail").get("PercentValue"),
            RateElementBasis=payload.shipment.extra.get(
                "VariableHandlingChargeDetail").get("RateElementBasis")
        )

    if 'ExpressFreightDetail' in payload.shipment.extra:
        RequestedShipment_.ExpressFreightDetail = Ship.ExpressFreightDetail(
            PackingListEnclosed=payload.shipment.extra.get(
                "ExpressFreightDetail").get("PackingListEnclosed"),
            ShippersLoadAndCount=payload.shipment.extra.get(
                "ExpressFreightDetail").get("ShippersLoadAndCount"),
            BookingConfirmationNumber=payload.shipment.extra.get(
                "ExpressFreightDetail").get("BookingConfirmationNumber")
        )
    ...
    if 'ManifestDetail' in payload.shipment.extra:
        RequestedShipment_.ManifestDetail = Ship.ShipmentManifestDetail(
            ManifestReferenceType=payload.shipment.extra.get(
                "ManifestDetail").get('ManifestReferenceType')
        )

    return Ship.ProcessShipmentRequest(
        WebAuthenticationDetail=self.webAuthenticationDetail,
        ClientDetail=self.clientDetail,
        TransactionDetail=Rate.TransactionDetail(
            CustomerTransactionId="IE_v18_Ship"),
        Version=Rate.VersionId(
            ServiceId="ship", Major=21, Intermediate=0, Minor=0),
