...
def parse_tracking_summary(response: Element, settings: Settings) -> Tuple[List[TrackingDetails], List[Error]]:
    ...
    tracking: List[TrackingDetails] = reduce(_extract_tracking(settings), pin_summaries, [])
    return tracking, parse_error_response(response, settings)


def _extract_tracking(settings: Settings) -> Callable[[List[TrackingDetails], Element], List[TrackingDetails]]:
    def extract(tracking: List[TrackingDetails], pin_summary_node: Element) -> List[TrackingDetails]:
